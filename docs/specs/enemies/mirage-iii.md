...
- 互斥原型：Lancer；池角色：intercept。身份必须由主循环而非小数值差表达。
- 主循环：三角翼高速截击，越过目标后继续脱离。

## 2. 数据定义

| 项目 | 权威值 |
|---|---|
| 解锁 / Token / 上限 / 编成 | 2 / 3 / ∞ / 2–3 |
| 机体关键值 | 42；2200/900/225；5.8G；roll 3.4；accel 43 |
| 雷达 | 2800px/22°/3.8s |
| 武器与防御 | 机炮+敌方 V-tier 导弹；无 flare；普通敌机 HP 运行时封顶 75 |
| 飞行/生存/雷达基准 | F-104 低价截击机；不读取 resources/player/ |
...
| 敌版资源 | resources/enemy_mirage3.tres |
