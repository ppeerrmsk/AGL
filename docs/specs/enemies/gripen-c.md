...
# JAS 39 Gripen C 常规敌版

## 1. 设计意图

- 互斥原型：Schemer；池角色：ew。身份必须由主循环而非小数值差表达。
- 主循环：队级最多3目标、每机1锁，错峰发射后全队10s换位。

## 2. 数据定义

| 项目 | 权威值 |
|---|---|
| 解锁 / Token / 上限 / 编成 | 6 / 5 / 4 / 固定3 |
| 机体关键值 | 55；2100/920/185；7G；roll 4.0；accel 52 |
| 雷达 | 3800px/32°/3.2s |
| 武器与防御 | 机炮+敌方 V-tier 导弹；flare 1/fail 0.50；普通敌机 HP 运行时封顶 75 |
| 飞行/生存/雷达基准 | MiG-29 本体，机制换持续缠斗；不读取 resources/player/ |
...
| 敌版资源 | resources/enemy_gripen_c.tres |
