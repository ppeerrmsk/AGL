...
- **Litmus 自检**：
  - 信息察觉（原则 3）：专属涂装色 + 入场无线电台词（`ace_spawn`）+ 次级提示条 + Tab 标记 +
    FLR 计数（那枚必定躲的 flare 用没用）；红色警告横幅**不用**——那是 BOSS 专属（tier §2.6）；
  - 一击毙命（原则 1）：**本体一发死**（HP≤cap，不豁免——除 BOSS 外都一击必杀）；防御只有 1 枚必定躲 flare（骗 1 发 → 第 2 发死）；
...
### 2.2 生存模型 —— **一发死 + 1 枚必定躲**（2026-07-23 用户定档，推翻旧"命数"设计）
...
| `max_flares` | **1** | 4 |
| `burst_count` | 1 | 1 |
| 干扰成功率 | **1.00（必定躲那一发）** | 1.00 |
| `fail_chance` | 0（必定释放） | 0 |
| 补充 | 无（耗尽即防御归零） | 无 |
| 耗尽后 | 不解锁规避机动 | 同 |

**击杀序列（单机）**：第 1 发导弹 → 那枚 flare **必定**骗飞（jam 1.00 + fail 0）→ 第 2 发命中即死
（HP ≤75 < MRM 80）。**严格 2 发/架**，全队 5 机 = 10 发导弹当量。对比 BOSS：4 骗 + HP 100 残血 +
击坠 = 6 发/架 + 隐形。**BOSS 每维度都更强**（HP 豁免 / 命数 4× / 隐形），支援中队仅"必定躲 1 发"。

**与一般敌人的区别（用户明确）**：一般敌机保留"flare 也会被打中的概率"（走 `calc_jam_chance`
概率模型，可能失败）；王牌支援中队那 1 枚**必定**成功（`AceTier.is_ace → jam 1.00`，共享判定不变）。
即差异只在"必定 vs 概率"，数量都是 1 枚 → 都符合一发死（骗弹是干扰层，非加血）。
...
- [ ] **击杀序列**：对单架王牌连射导弹：第 1 发必被骗（无随机失败）、第 2 发命中**即死**（HP≤75）；
      迎头/近距第 1 发同样必被骗（jam 1.00 无角度/距离修正）
...
2. 支援档 = **一发死 + 1 枚必定躲 flare**（2026-07-23 用户定，推翻旧"2 命"）；ace_gun 强化机炮 +
