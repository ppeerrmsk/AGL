...
> **v14 已实现并验收。** G0 是全向锁定 / 离轴发射阶段，G1 恢复普通前向规则；G1 HP 按用户确认由 `400` 降为 `200`。当前四代 HP 为 `400 / 200 / 100 / 70`，每棵树总 HP `1760`，双根总 HP `3520`。
...
- G1 / G2 的爬升阶段为 `2.5s`：达到 `15,000m` 前仍可见、可锁定、可受伤，达到后才隐藏并免伤，然后进入独立高空等待。
- 爬升与俯冲不再首尾相接：完成爬升后先在不可见高空独立等待 `7–10s`，等待结束才锁存新的玩家落点并开始 4 秒俯冲预警。G1 首次由 G0 分裂生成时的隐藏下降属于登场契约，不计作循环技能，不取消。
...
| 证据 | 结果 |
|---|---|
| `hyper_a` | `98/98`：Registry、四代资源、Debug 九入口、G0/G1/G2/G3 为 400/200/100/70 HP、G1 首次隐藏 / 撞击揭示、G0 180° 半锥 / 全向四目标离轴实弹、G1 45° 前向锥 / 侧向成熟锁拒射、四代独立饱和齐射与特殊状态静默、40–50s 再入间隔、7–10s 高空等待、其它分裂体暂停倒数 / 至少 12s 错峰、6s SLOW、冲锋物理对线 / 伸展预警 / 延迟终点扇区、急刹扇区几何 / 实伤 / 单次结算、崩溃防护与 arrival 契约 |
...
| Visual | Black Star 横幅完整显示 `Blame it on the falling sky`；G2 冲锋线在 `1.2s` 只伸展约一半且终点扇区完全隐藏；线满 / 急刹专项在 `4.0s` 才显示从终点沿机头前方扩散、未越出判定扇区的波面；G1 隐藏下降、Debug 四卡与 16 G3 树 HUD 均完成 1920×1080 截图验收 |
