...
> 战场上有人在通信。击坠、下令、BOSS 登场时，屏幕上方跳出一句带呼号的可读无线电文字，同时播放
> 一段像人声但完全不可辨词句的电台通信纹理，让空战从"打靶"变成"有人的战场"。
...
无线电的**文字与声音职责严格分离**：屏幕文字负责具体语义、呼号和三语本地化；声音只负责“频道里有人
正在通信”的存在感与质感。声音不是台词配音，不能让玩家从音频中听出任何具体词句。

| 字段 | 值 |
|---|---|
| 播放总线 | `AudioManager.RADIO_BUS`（`"Radio"`，默认 -10 dB，已挂频带切割+轻失真效果链） |
| 当前占位音效 id | `radio_beep` |
| 当前占位文件 | `res://audio/sfx/radio_beep.wav` |
| 正式素材形态 | 一套跨语言共享的短“含混人声通信纹理”池，可叠加蜂鸣、静电与开关噪声 |
| 语言 | **无语言绑定**；中文、英文、日文界面共用同一套声音，不生成三套音频 |
| 可懂度硬门 | 隔离试听时应明确像人在通信，但**不得辨认任何单词、数字、呼号或句意** |
| 与文字映射 | 不逐句生成、不按 key 配音、不要求跟文字长度同步；同一声音变体可服务任意台词 key |
| 播放时机 | 每条台词**显示的同一帧** |
| 素材缺失行为 | **静默跳过，不 push_warning**（视觉文字部分独立可验收） |

生成与制作时使用无语义、非词汇化的含混音节或被严重遮蔽的人声节奏。不能用“换成玩家听不懂的真实外语”
代替不可懂度；如果仍能听出具体语言或词句，该候选不合格。母版保留人声纹理、蜂鸣和静电的独立层，
最终由 Radio Bus 统一做频带与失真，避免把语言或单句内容烤死进素材。
...
- [ ] 音效素材到位后，含混人声/蜂鸣/底噪与 BGM/SFX 的相对关系合适；隔离试听明确像人在通信，
      但听不出任何单词、数字、呼号、句意或具体语言。
...
### 阶段 3 — 待 playtest 后决定
- [ ] 真实无线电音效素材接入（当前留接口）
- [ ] 跨语言共享的无语义含混人声纹理池；**不做逐句 TTS / 配音，不按三语复制音频**
...
| 日期 | spec_version | 改动 |
|---|---|---|
| 2026-08-22 | 12 | 用户定档无线电声音语义：声音不是三语台词配音，而是一套跨语言共享、像人在通信但完全不可辨词句的含混人声纹理；屏幕文字独立承担呼号、句意与三语本地化。禁止逐句 TTS、语言绑定和可辨单词，蜂鸣/静电/人声纹理保留独立母版后统一走 Radio Bus。 |
