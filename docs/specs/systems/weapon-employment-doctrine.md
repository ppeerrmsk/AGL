...
扩展现有机制，不在纪律层开叉：
- 玩家方/第三方友军：尽可能准确不失误——无人为误差注入，不吃敌方“一次机会一梭”安全门。
- 敌方：`_ai_gun_burst_allowed` + 武器执行层统一实行“一次机会一梭、梭后停火 3.0s”节流 + PilotPersonality 误差注入
...
| 2026-07-12 | 7 | **火力纪律补丁：导弹超杀双门收紧**（log 181952 实证：6 枚 MRM"目标已消失"脱靶浪费、F-14 弹舱 2 枚全空转打直升机）：①TEAM_OVERKILL 记账**含自己在飞弹**（exclude=null——原排除自己 + max_inflight=2 允许单机对 70hp 小目标连发 2×80；jammed/丢制导仍不计账，flare 干扰后补射价值保留，高 HP 目标连发照常）；②**AI 机炮让路门**（原 GUN_ACTIVE 仅玩家机）：自机机炮正在输出且目标"单弹即超杀"的小目标 → 不发导弹让机炮收；高 HP 目标不触发保饱和火力。验收 --bench=fire_discipline §A + 回归门 22 项全绿。 |
