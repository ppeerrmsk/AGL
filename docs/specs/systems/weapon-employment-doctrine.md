...
5. **兜底**（用户定稿 4）：全部候选失格（都在转 CD/弹尽）→ **维持追击 + 按导弹纪律
   机动等待**（crank 保持雷达锁的宽松几何——CD 转好即刻能打；不再"机炮硬兜底"，
   也不是呆飞）。
...
### 阶段 2 — planner 接入（行为切换点）
Situation.primary_weapon → `_apply_combat_weapon` 重写为消费竞选结果；
weapon_mode 扩展 kind；统一提前点 helper 抽出（机炮修复公式上移）。

### 阶段 3 — LINE_UP intent + 电磁炮迁入 planner
新 intent + railgun 状态机的"时机决策"上收（发射执行留在 equipment）；
AF-03 摘掉 prefer_nose_aligned_weapon 旗迁入 planner 白名单。
