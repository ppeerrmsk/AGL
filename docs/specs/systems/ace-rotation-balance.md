...
- **固定遭遇节拍**：王牌不再按上一波终态加冷却漂移；每局固定在开局 3:30 后开启第一波，并在 BOSS 前最后 3:00 开启第二波。
...
| 字段 | 值 |
|---|---:|
| 第一波时间槽 | `game_time ≥ 210 s`（开局 3:30） |
| 第二波时间槽 | `WARZONE_PHASE_DURATION - game_time ≤ 180 s`（BOSS 前最后 3:00） |
| 抽取 | 每局开局 Fisher–Yates 洗牌 |
| 局内重复 | 禁止，无放回 |
| 连续两局首队 | session 内禁止相同；若洗牌首项相同，与后续随机项交换 |
| 同场上限 | 1 支 |
| 每局刷新上限 | 2 支 |
| 波间冷却 | 无；时间槽到达且场上没有非 BOSS 王牌即触发 |
| 槽位冲突 | 第二槽到达时若第一支仍在场，则第二槽保持待触发；第一支终态后下一帧立即刷新，不追加冷却 |
| ORION | 仍走宿敌独立轨道，不进入本轮换 |

第二槽必须按本局真实 `WARZONE_PHASE_DURATION` 的剩余时间计算：默认 600 秒战区在 420 秒（7:00）开启；生涯商店把战区延长到 630 秒时在 450 秒（7:30）开启。补给时间税若跨过阈值，则在跨过后的下一调度帧开启，不补算固定墙钟。槽位开放状态单向锁存：到点后即使击破上一支王牌令 `game_time` 倒拨 60 秒，也不能把已经开放的第二槽关回去。两个槽各消费一次，禁止因长时间停留在阈值之后重复刷新。
...
4. `game_time >= 210s` 时开放第一槽；`WARZONE_PHASE_DURATION - game_time <= 180s` 时开放第二槽。
5. 已开放槽数大于本局已刷新数、且场上没有非 BOSS 王牌时，从顺序表取下一支未出现中队；第二槽被占场阻塞时保持待触发。
6. profile 热更新导致顺序表无合法项时，退化为当前候选首项，事件不得卡死。
...
- [x] 六支中队在 210 秒第一槽都有被抽资格；局内无重复；连续两局首队不同。
- [x] 默认 600 秒战区在 209.9 秒不刷、210.0 秒开放第一槽；419.9 秒仍仅一槽、420.0 秒开放第二槽。
- [x] 630 秒延长战区的第二槽固定在 450.0 秒开放；调度只看真实剩余 180 秒，不写死 420 秒。
- [x] 每局最多刷新两支；第二槽到达但第一支仍在场时保持待触发，终态后下一帧立即刷新且无额外冷却。
- [x] 第二槽开放后单向锁存；王牌击破奖励令 `game_time` 倒拨 60 秒也不会撤销待触发槽。
...
- [x] profile 统一 210 秒进入第一时间槽，加入 flare 与 balance 字段。
...
| 日期 | spec_version | 改动 |
|---|---:|---|
| 2026-08-23 | 3 | 用户定稿为两个固定时间槽：第一波开局 3:30，第二波为 BOSS 前最后 3:00；删除终态后冷却与固定 540 秒截止，第二槽占场时保持待触发，每局最多两支。 |
