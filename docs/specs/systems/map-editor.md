...
| 字段 | 值 | 说明 |
|---|---|---|
| 世界范围 | ±7500 px（30×30 km，随 map-expansion 主开关走） | 与 MapBoundary 一致 |
| 地形格边长 | 100 px（= 200 m） | 150×150 格覆盖全图；海岸线精度 ≈ 官方 OSM 烘焙抽稀后水平 |
| 笔刷尺寸 | 1 / 3 / 5 / 9 格（直径） | 圆形笔刷 |
| 云 mask 格边长 | 234 px（64×64 格覆盖全图） | 云是软边界，低分辨率足够 |
...
editor_cells             → 由多边形栅格化反推（格心 point-in-polygon，150×150 一次性）
```
...
- [ ] 笔刷/橡皮涂陆地/城区 + 实时轮廓预览 + 试飞按钮
- [ ] 保存/另存为/自动保存/恢复/退出确认（§2.4）
