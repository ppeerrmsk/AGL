...
| 新机体的专属技大概率遇到 | 每架机每局在“首次符合条件的三轴卡片事件”独立掷 **90%**；命中才显示第四槽 | 起手机也算本局驾驶的第一架机；每次进化到新机可获得该新机自己的独立机会 |
...
| 状态 | 含义 | 转移 |
|---|---|---|
| `UNSEEN` | 本局尚未为该机处理机会 | 首次符合条件的三轴事件时掷 90% |
...
- [ ] 已购 `signature_f15`：本局首次符合事件只掷一次 90%；命中时显示 3+1，普通三轴卡内容与无第四槽对照组一致。
...
- [x] 确认 90% / 每机每局一次、Tier 价格 500~900、AWACS 3000、四分页分类。
...
- [x] 增加逐机逐局状态账本与 90% 单次 roll；换机后以新 node 独立记账。
- [x] 第四卡选择复用现有 `_apply_upgrade_choice`、轴点与 `milestone_plus` 路径；前置技能不阻止出示。
- [x] 扩展 `sig_skills` bench 覆盖 41 映射、F-14 特例、90% 边界与前置语义。
