...
| 功勋商店购买后才有机会刷 | 43 条专属技全部默认锁定；亲自获得过对应机体后才揭示并可购买；购买后永久获得“入第四槽资格” | 购买不直接把技能带进局，仍须在局内选卡 |
...
本 spec 获批后，覆盖 `aircraft-signature-skills` 中“签名技直接进入普通池、×2.5 抽取权重”的旧获取方式；43 条技能效果、继承、轴归属与稀有度不变。
...
- [x] 三轴普通池显式排除全部 43 条签名技，退役签名 ×2.5 权重消费。
