...
港湾正式 PNG 上仅为 `AF_HANEDA` 补画四条跑道；木更津、調布与图 2/图 3 不追加该覆盖层。
跑道中线直接取冻结的东京湾 OSM 跑道数据，避免凭示意图手调造成位置、角度或尺寸漂移：

| 跑道 | 起点（px） | 终点（px） | 长度（px / m） | 宽度（px / m） |
|---|---:|---:|---:|---:|
| A | `(40.19, -6025.35)` | `(790.91, -7098.64)` | `1310 / 2620` | `28 / 56` |
| B | `(1159.45, -5362.92)` | `(410.03, -6658.89)` | `1497 / 2994` | `30 / 60` |
...
- 战区气氛层的 SPG 不是机场 TGT，也不参与占领转换。敌方 SPG 中弹后仍为 `HOSTILE`；解放时只撤离气氛演员，再独立生成 §2.4 的绿色 AA/SAM，禁止复用或翻转旧敌方实例。
...
| 2026-08-10 | 7 | 港湾正式 PNG 为羽田补画四条 OSM 对齐跑道；主地图静态绘制，Tab 使用独立小缓存，位置、角度、长度与宽度均先经 PNG 叠图核对。 |
