...
| **城区直升机** | 低概率出现在远端城区的 3 架 CH-47 逃跑组，带 2~4 机战斗机护卫；玩家从战术地图主动寻找并截击（护卫编成规则见 [zone-reward-docking §2.7](zone-reward-docking.md)） |

**城区直升机事件规则**：

| 规则 | 内容 | 为什么 |
|---|---|---|
| **低频调度** | 每个 60–120 s 调度窗仅有 **25%** 概率生成；开局首窗沿用 36–96 s；一局最多生成 **2 次** | 保留偶遇感，避免周期计时器把背景事件变成固定主线 |
| **禁止并存** | 上一组 CH-47 尚有成员存活/撤离时不生成下一组，30 s 后再检查；次数只在真正刷出机群后计入 | 同屏只允许一组，避免任务状态被新组覆盖、战术地图出现两组目标 |
| **全图探索生成** | 候选从**全图**城区选取，不再限定玩家附近；城区中心距玩家至少 **6000 px（12 km）**、距地图边界至少 **4000 px（8 km）**、不在 AVAILABLE/SELECTED 战区圈内且不在主视野内。无合格城区则 25 s 后重试 | 任务应先存在于地图中，由玩家主动打开战术地图寻找；边界净空保证发现后仍有截击时间 |
...
> 全歼判定走轮询式击杀追踪（与既有击杀检测同模式），不挂信号；只持当前一组 CH-47 的短生命周期引用，结算/撤离后立即清空。
...
| 日期 | spec_version | 改动 |
|---|---|---|
| 2026-08-20 | 4 | 城区直升机由周期必刷收口为每窗 25% 概率、整局最多 2 次且禁止并存；出生候选改为全图远端城区（距玩家至少 12 km、距边界至少 8 km），让玩家从战术地图主动寻找。 |
