...
kind: balance
status: draft        # ⏳ v7 五档 41 机矩阵待用户 review 后实装。规范玩家机战斗力曲线 + 数据解耦。
schema_version: 1
spec_version: 1
owner: noelu
...
## 2. 数值总表（有效值口径，5 档 41 机，⚠ v7 待用户 review）
...
| **A-10** | 6 | 攻击/肉 | **160** | ～1200～ | 800 | 64 | 7.2 | 3000 | 30° | 2.8 | 3 | **全谱最强机炮**；默认无火箭，Hydra 仅从战区奖励取得；A-6E 直系 |
...
- [ ] **debug 测试场全谱（v7，用户令）**：主菜单 B 的 boss debug 链路可选**全谱 41 机**（等级/属性门槛全豁免），
...
- [ ] 阶段 1：建 `resources/player/` 41 份 params（照 §2 直写）+ X-02 补机炮 + 新资产（欧系 9 款/YF-23/J-36/X-77/X-90/AX-00 等卡片与 i18n）。
