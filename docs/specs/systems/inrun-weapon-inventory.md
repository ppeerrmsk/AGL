...
- **新方向（用户 2026-07-19，重点调整）**：
  ...
  4. ~~**不必剥武器**：进化树上飞机的固有签名武器**保留**（不做"全谱只留机炮导弹"的剥离方案）。~~
     ...
     火箭/电磁炮/激光/僚机/雷/QMAAM 一律**不烤入机体**，纯靠 §2.2 的战区奖励 + 签名技能获取。
...
| **特殊武器（外部装备）** | **火箭** / 电磁炮 / 激光 / 忠诚僚机（中程僚机）/ QMAAM / 尾部漂浮雷 / 后续新增 | **玩家武器库**，到手即永久（局内）；**不烤入任何机体**（2026-07-23） |

### 2.2 获取来源（都在局内；2026-07-23 起收敛为两条，均**非自带**）
...
2. **签名技能**：某些签名技能主动给武器（sig_x02 突击翼龙→电磁炮 / sig_f47·sig_x90→忠诚僚机）——
   这是玩家**选卡换来**的，不算"机体自带"。
   ~~机型固有入手/首驾入库~~（作废——特殊武器不再烤进 `player_*.tres`，故无"首驾即得"）。
