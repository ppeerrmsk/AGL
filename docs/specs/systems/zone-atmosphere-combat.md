...
> 普通地图约 30% 的已激活正式战区拥有集中调度的低成本战斗；决战地图的已激活战区全部保留这种氛围。普通 1★/2★战区在玩家选择或进入前不生成演员，玩家进入后敌军把玩家当成最高优先级威胁，而气氛组永远不能代替玩家完成战区目标。

## 1. 设计意图（Why）

- **体验目标**：玩家明确选择的战线从边缘进场并形成持续战争；未关注战区不在画外消耗实体与弹道预算。
...
- **反模式规避**：不靠加厚敌人 HP 削弱玩家；不让瞬时镜头边界反复切断已激活战斗；不让 AI 击杀提前完成任务；未激活战区不靠零伤害弹丸伪装成低成本；不复活废弃沙盒路径。
...
| 字段 | 值 | 说明 |
|---|---:|---|
| 普通地图出现率 | 30% / 战区 | 每个合资格战区首次生成时独立抽取一次，结果缓存到本局结束；重刷和等级刷新不重抽 |
| 决战地图出现率 | 100% / 战区 | 当前决战地图 id 为 `ocean_islands_preview`；预览期没有战区，正式毕业后自动沿用全覆盖规则 |
| 调度频率 | 2 Hz | 一个正式战区氛围控制器集中处理全部战区 |
| 真实伤害进入距离 | 1500 px（3 km） | 玩家进入后，新发武器按近距倍率快照伤害 |
| 真实伤害退出距离 | 1800 px（3.6 km） | 300 px 迟滞避免边界抖动 |
| 已激活战区离镜开火 | 保留 | 玩家已经选择/进入的战线保持连续，不随瞬时镜头边界开关；未激活普通战区为 0 实体 |
...
本节只适用于已经按 [未关注战区的最简世界模拟](offscreen-world-simulation.md) 进入 `LIVE` 的战区。伤害权限在武器发射瞬间快照：在途弹丸不会因为玩家跨过 3/3.6 km 边界而中途变成实弹或空包弹。零伤害普通弹走既有 `visual_only` 快路径，跳过单位碰撞、建筑碰撞和近炸运算；导弹仍保留离架、飞行、曳光与爆炸表现，但零伤害导弹不创建持续 AOE。
...
- 气氛直升机只选择敌对地面目标：气氛 `GroundUnit`、普通地面单位、攻城坦克，以及一体化巨炮单体。双方直升机即使同时存在也互不攻击，因为 Aircraft 永远不是合法目标。
...
- 气氛直升机可以正常击毁敌对气氛地面演员；攻击正式任务 TGT/挂点时继续走非致死入口，最低保留 1 HP，不能替玩家清掉攻城坦克、一体化巨炮或其它任务目标。
...
|---|---:|---:|---:|---|
| 火炮 | 4.5–5.5 s，稳定错峰 | 2.2 s | 60 | 瞄准发射瞬间的目标位置，落点沿随机方向偏移 0–80 px；到达时目标距落点 ≤24 px 才算可信直击并一发击毁 60 HP 的专用 SPG，否则只播放近失爆点 |
...
zone_spawn_finished(zone_id):
    require zone simulation state == LIVE
    if mission_type == bomber_escort: skip
    enabled = decisive_map or cached_roll(zone_id, chance=0.30)
    if not enabled: skip
...
同一个战区只允许一条登记记录。普通地图的抽取结果按 `zone_id` 缓存在当前 `ZoneMission` 生命周期内：同一战区后续调试重刷、等级刷新或任务类型安全降级都沿用第一次结果，不因刷新而突然出现或消失。决战地图跳过随机门，所有合资格战区直接登记。调试重刷、等级刷新、战区完成、失败、阶段取消时先注销旧记录，恢复既有敌军的普通伤害/目标选择，再按“视线内不凭空消失”规则撤走新增友军。
...
- [x] 普通地图仅约 30% 的已激活 air/squadron、ground/airfield、naval 战区登记气氛层；未激活普通战区为 0 实体；每区只抽一次且刷新不重抽；决战地图已激活战区全部登记；bomber_escort 始终不额外刷演员。
...
- [x] 专用 SPG 为 60 HP；近距火炮使用 80 px 散布与 24 px 直击窗口，窗口外不磨血，窗口内 60 点伤害可明确击毁；画外仍为 0 伤害。
...
| 日期 | spec_version | 改动 |
|---|---:|---|
| 2026-08-23 | 13 | 按用户新的世界模拟裁决，普通 1★/2★气氛战斗只在战区选择/进入并生成后登记；未关注战区不再用画外零伤害弹丸维持“全图战争”。已激活战线短暂离镜仍保持连续，3★全局威胁继续走专属例外。 |
| 2026-08-22 | 12 | 跟随三级巨炮一体化裁决更新气氛目标语义：整座巨炮现在只有一个 GroundUnit/TGT，友军气氛火力仍只能把它压到 1 HP，不能替玩家完成任务。 |
