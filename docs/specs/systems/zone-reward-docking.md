...
> "追加武器限副系统类"的边界已被扩——武器子池新增**火箭弹/电磁炮/激光**（依据
...
> - **航母整局保证**：pity 机制——前 `CARRIER_PITY_ROLLS`(=4) 次 roll 内航母若未自然出现 → 下次强制发，
>   确保每局一定出现一次航母奖励。

| 奖励 | 内容 | 权重（★ / ★★ / ★★★，待调参） |
|---|---|---|
| **航母增援** | §2.4；全局限 2 次登舰；**整局唯一 + pity 保证必现一次** | 0 / 20 / 45 |
| **+1 僚机** | §2.5（ACE 升级 + 爆出僚机，**2026-07-24 改一次给 2 架**）；**不去重=可重复保底** | 40 / 45 / 40 |
| **追加武器** | §2.6（副系统三选一）；**整局每种唯一** | 60 / 35 / 15 |
...
领取时（2026-07-24 修订：一次给 2 架）：
  ...
  在 ace 侧后爆出 2 架新僚机（出现特效 + 自动入队，squad 数量 +2）
  编队上限：9（squad-upgrade-ownership 上限）——能塞几架塞几架，一架都塞不下才降级追加武器
