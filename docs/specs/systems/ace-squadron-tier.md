...
**flare 统一铁律（2026-07-27 用户定档）**：王牌中队**每架一律 1 枚 flare**，除非该队 spec
**特别声明**。已特别声明的例外只有两个方向：**BOSS 档 4 枚**（多命是 BOSS 专属待遇，本节上表）
与**零 flare 机动规避型**（无 flare 的补偿是导弹规避机动，见 §3.4 例外条款；首例 2ndWave
的 Teacher）。新队默认 1 枚，不要自行发明第三档。
...
| 特高 | **0.50** | 零 flare 机动规避型（Teacher 类——闪避是它唯一的防御轴） |
...
|---|---|---|---|---|---|---|---|---|
| **MARATHON** | 马拉松 | Su-35 ×5 | 斗士 | **中期**（2026-07-28 用户改档） | 猩红 `#FF2E3D` | 不闭合的跑道环 + 终点线竖杠 | PACER | `marathon` |
| **2NDWAVE** | 第二波 | F-4E ×1 + F-15 ×4 | 混编（斗士长机 + 骑士学员） | 早期 | 电紫 `#B44DFF` | 双叠浪线，后浪高过前浪 | TEACHER | `2ndwave` |
| **ORION** | 猎户座 | 原创机体 Cre ×1（宿敌单机，§3.8） | 斗士（独狼） | **中期（独立轨道）** | **无——伪装普通敌橙**（§3.8 豁免） | 猎户座腰带三星连线 | Cre-XX（机号即呼号） | `orion`（计数即档案） |
| **GIMMICK** | 把戏 | F-16 ×2 + Mirage 2000 ×2 | 混编（狙击 + 斗士） | 中期 | 洋红 `#E23A8E` | 交叉互指的双箭头（调包） | BLUFF | `gimmick` |
| **GOOFIGHTERS** | 怪火 | Su-47 ×2 | 斗士（格斗弹 + 眼镜蛇） | 中期 | 深紫罗兰 `#7B3FE4` | 两簇并飞的鬼火圆点 | WISP | `goofighters` |
| **VULTURE** | 秃鹫 | MiG-31 ×8 | 骑士 | **后期** | 酒红 `#8E2450` | 三线构成的俯冲展翼 V + 翼下一点 | CARRION | `vulture` |
...
| 项 | 规格 |
|---|---|
| 段数 | = 编成机数（MARATHON 5 段 / 2NDWAVE 5 段 / VULTURE 8 段），一段 = 一架 |
...
### 2.9 登场时段档位（2026-07-27 用户定档）

用户对各队标注的登场时机是**刷新时机建议**，核心诉求：**避免比较强的中队一开始就出现**。
落成两档（profile 字段，调度器按当前 game_time 从可用池轮换）：

| 档 | 进池时间（草案） | 队 |
|---|---|---|
| 早期 | `game_time ≥ 240 s`（现状首支门槛） | 2NDWAVE |
| 中期 | `game_time ≥ 320 s` | **MARATHON**（2026-07-28 用户定档：Su-35 中期才出现）、GIMMICK、GOOFIGHTERS |
| 后期 | `game_time ≥ 400 s` | VULTURE |

触发时从**已进池**的队伍里按轮换指针选（指针跨局轮转、同局不重复同队）。其余调度
规则不变（间隔 150 s / 540 s 截止 / 同场 ≤1 支 / BOSS 阶段不触发）。
...
**例外条款（2026-07-27 用户定档）——零 flare 机动规避型**：spec **特别声明**的王牌个体
可以走相反的防御模型：**没有 flare，靠机动规避导弹**（beam/notch 等既有规避行为对其开启），
同时挂特高档机炮闪避（§2.2 分档表 0.50——被机炮追也会闪）。这是"防御预算换轴"而非加强：
放弃确定性命数（1 骗必躲），换成持续但不确定的机动周旋。适用面收窄为**个体**（一队里至多
长机一架），首例 2NDWAVE 的 Teacher（F-4E）。整队零 flare 机动规避**不允许**——那会把
"命数可数"的 tier 读数体系整个废掉。
...
首例 GOOFIGHTERS（[events/ace-goofighters](../events/ace-goofighters.md) §2.3）。
...
- [x] 调度器时段档（240/320/400）+ 轮换指针（跨局 session 内）+ 同局不重复
- [x] lore/中文名三语入 i18n（`ACE_SQUAD_*_NAME` / `_LORE` ×6 队）
- [x] Tab 标记改中队主色 + 代号标签
- [x] 档案页王牌区块 UI：**击破解锁制**（未击破 = 灰徽章剪影 + "???"，遭遇过附遭遇次数
      "它认识你了"；击破 = 代号/中文名/lore 全文/首破日期/击破数）；ORION 额外显示"下一架"机号。
      ⚠ 2026-07-28 用户扩展需求后，该页**泛化为全敌人图鉴**（六队成为其中一组），
      入口改「敌人图鉴」，规格移交 [career-archive §2.6](career-archive.md)
- [x] 徽章线框绘制：`AceEmblemIcon`（六队几何线框：跑道环/双叠浪/猎户三星/调包双箭头/
      双鬼火/俯冲 V；静态绘制仅变更时重画）——血条代号旁小图 + 档案页大图两处挂载
