...
> 咬住你就不松口；每一枚热诱弹都只给它一次短暂的脱离机会，真正能否躲开取决于它有没有做出有效 break。
...
> **热诱弹 = 有限 break 尝试。**投焰后仍会被导弹追踪；只有敌机在 1.25 s 内真正改变航迹，且操控等级判定通过，导弹才失导。

这条设计仍保持**读数清晰**：玩家可数剩余投焰尝试，也能直接观察敌机有没有转向、横移、增速或换高度。
高等级王牌成功率更高，但不再把一次投焰等同于确定的一条命；近距追上或动作失败都能正常命中。
...
- **单杠杆**：王牌中队的导弹防御仍只有有限热诱弹尝试，不叠护甲 / 隐藏减伤；耐久差异优先调尝试数与可观察的操控水平。
- **效果即反馈**：热诱弹生效时导弹明显被骗飞，这就是反馈本身。**不加"剩余命数"HUD 元素**。
- **动作优于按钮**：概率只决定敌机能否把动作执行到位，失导仍必须由真实 break 兑现；轨迹未变时投焰不会抹掉玩家导弹。
...
> tier 的 LOD 豁免 / 无缩放 / 强 AI；那 1 枚 flare 只提供一次高成功率 break 尝试，血量与多次尝试数不继承。
>
> **尝试次数档位**：`max_flares` 按定位分档——**BOSS 档 4 次 + HP 100 豁免**（Wraith 等，本节数值）/
> **支援档 1 枚 + HP 一发死**（非 BOSS）。BOSS 每个耐久维度都严格更强（§3.6）。

| 字段 | 王牌中队（BOSS 档） | 普通敌机（对照） | 说明 |
|---|---|---|---|
| `max_flares` | **4**（BOSS 专属；非 BOSS 支援档 1） | 1 | 热诱弹 break 尝试次数；不是确定命数 |
| `burst_count` | **1** | 1 | 每次投放只消耗一次尝试 |
| `cooldown` | 1.2 s | 1.2 s | 两次投放最小间隔 |
| `reload_time` | — | — | **不适用**：`enable_flare_reload = false`，耗尽永不补充 |
| break 执行成功率 | `0.15 + 0.80 × effective_skill` | 同一公式 | 王牌有效操控 ≥0.90 时为 87%–95%；仍须真实动作达门，见 §3.3 |
| `fail_chance` | **0.00** | 0.05~0.85 按机型 | 王牌不会漏按投焰，但仍可能 break 执行失败 |
| `head_on_fail_reduction` | **0.00** | 0~0.25 | 同上，`fail_chance` 归零后本项无意义 |
| 投放距离 | **400 m** | 240 m | `nervousness = 0.5` → `lerp(200, 600, 0.5)` |
| 耗尽后行为 | **不解锁额外规避机动**，下一发导弹没有 flare break 窗 | — | 见 §3.4 |
...
与 §1.3"确定性优于概率"的关系：flare 数量仍是可数的有限尝试，但结果改由**可观察的真实 break +
等级概率**共同决定。玩家能从敌机是否转向、横移、增速或换高度判断结果，不再面对投焰瞬间无条件免疫；
完整公式和动作门以 [flare-evasion-coupling](flare-evasion-coupling.md) 为权威。
...
#### 非 BOSS 王牌专属 BGM 生命周期（2026-08-22 用户定档）

非 BOSS 王牌中队拥有一个 **4 首曲目的共用专属池**，但**入场不立即切歌**：入场仍由上面的无线电、
提示条和 Tab 标记负责。音乐唯一触发点是 **§2.8 的 Ace 中队血条从隐藏变为可见**；该浮现
仍由 `battle_joined`（任一成员首次开火、首次受击或已被击毁）判定。血条浮现的同一上升沿，
从 `ace_battle_01`～`04` 的可用曲目中**等概率随机抽 1 首**，以 **2.0 秒等功率交叉淡化**切入。
每次遭遇只抽一次、只播这一首，**不循环，也不在播完后接池中下一首**。音乐不得另建第二套接敌判断。

| 边沿 | 音乐行为 |
|---|---|
| 王牌生成 / 进场追击 | 保持普通战斗歌单，不切歌 |
| Ace 中队血条浮现 | 同一帧随机切入 `ace_battle_01`～`04` 之一，loop 关 |
| 交战中 / 被打断后重新接敌 / 普通弹尽撤离 | 当前专属曲尚未自然结束时继续播放，不因短暂脱离重抽或重启 |
| 抽中的 Ace 曲自然播完（Ace 仍存活也一样） | 立即淡入普通双曲歌单；本次遭遇不再抽第二首 Ace 曲 |
| 全灭 / WhiteTea 投降中和 / 全员逃离 / 事件取消清理 | 若当前仍由王牌曲占有，则 2.0 秒交叉淡化回普通双曲歌单 |
| BOSS 解锁或 BOSS 曲已经接管 | 立即放弃王牌曲所有权；BOSS 音乐优先，禁止抢回普通歌单 |
| Game Over / 场景退出 | 只释放事件所有权，由终局/下一场景接管，不擅自重启战斗歌单 |

素材缺失时必须 **fail-open**：随机池只纳入 `AudioManager.has_music(id) == true` 的曲目；部分缺失
就从剩余曲目随机，全池为空才保持当前普通歌单。不得先关闭 playlist、不得警告刷屏、不得留下
“当前 id 已切但实际无声”的假状态。专属曲成功
取得所有权后，终态归还必须幂等；同一帧重复进入 terminal / finish 只执行一次。
...
|---|---|---|---|---|---|---|---|---|
| **MARATHON** | 马拉松 | Su-35 ×5 | 斗士 | **固定两槽** | 猩红 `#FF2E3D` | 不闭合的跑道环 + 终点线竖杠 | PACER | `marathon` |
| **2NDWAVE** | 第二波 | F-4E ×1 + F-15 ×4 | 混编（斗士长机 + 骑士学员） | 固定两槽 | 电紫 `#B44DFF` | 双叠浪线，后浪高过前浪 | TEACHER | `2ndwave` |
| **ORION** | 猎户座 | 原创机体 Cre ×1（宿敌单机，§3.8） | 斗士（独狼） | **中期（独立轨道）** | **无——伪装普通敌橙**（§3.8 豁免） | 猎户座腰带三星连线 | Cre-XX（机号即呼号） | `orion`（计数即档案） |
| **GIMMICK** | 把戏 | F-16 ×2 + Mirage 2000 ×2 | 混编（狙击 + 斗士） | 固定两槽 | 洋红 `#E23A8E` | 交叉互指的双箭头（调包） | BLUFF | `gimmick` |
| **GOOFIGHTERS** | 怪火 | Su-47 ×2 | 斗士（格斗弹 + 眼镜蛇） | 固定两槽 | 深紫罗兰 `#7B3FE4` | 两簇并飞的鬼火圆点 | WISP | `goofighters` |
| **VULTURE** | 秃鹫 | MiG-31 ×8 | 骑士 | **固定两槽** | 酒红 `#8E2450` | 三线构成的俯冲展翼 V + 翼下一点 | CARRION | `vulture` |
| **WhiteTea** | WhiteTea | F-CK-1 ×3 | 机炮骑士（joust + 单次 J-turn） | **固定两槽** | 覆盆子红 `#C73567` | 三片茶叶 + J 形叶钩 | TEA | `whitetea` |
...
### 2.9 固定两波轮换（2026-08-23 用户定档）

六支非宿敌中队都在 `game_time ≥ 210 s` 进入第一槽候选。新局以无放回洗牌决定顺序，session
内连续两局的首队不得相同；同局不重复。每局固定两槽：开局 3:30 第一波、BOSS 前最后 3:00
第二波；同场仍 ≤1 支，第二槽到达时若第一支仍在场则保持待触发，第一支终态后不追加冷却。
强弱不再用分队时段遮蔽，而由 [ace-rotation-balance](ace-rotation-balance.md) 的 60~90 秒标准
击破预算约束。BOSS 阶段不触发新队。
...
收到"有导弹锁定本机且距离 ≤ 400 m"
  ...
  └─ 否则
       flares_remaining -= 1
       开始 1.25 s 可命中的 flare break（见 §3.3）
       进入 cooldown 1.2 s
```
...
Wraith 的具体节奏由 [wraith-squadron](../bosses/wraith-squadron.md) 持有：开战后首次等待
60.0s，每次 5.5s cloak 结束后重新进入严格 60.0s CD，可重复使用；取消导弹来袭绕过 CD。
当前操控玩家机进入任一 Wraith 1000px 内时禁止启动或提前结束 cloak，淡入/淡出各 1.0s。

### 3.3 干扰判定 —— 等级 + 真实 break

全体敌机统一继承 [flare-evasion-coupling](flare-evasion-coupling.md)：投焰时按有效操控水平采样
`P = 0.15 + 0.80 × S`，但导弹保持制导、警告与碰撞资格；只有 1.25 s 内完成规定的转向、横移、
增速或高度变化，且等级判定通过，才正式写入失导。王牌 `S≥0.90` 因而有 87%–95% 执行率，
但轨迹没变或动作完成前被追上时仍会命中。

### 3.4 导弹规避（明确不做）

王牌中队平时**不执行 beam / notch 等持续规避机动**。防御手段只有两层：**有限 flare break + 隐形**。

理由：规避机动会让王牌中队频繁脱离交战去做几何动作，直接破坏 tier 定义里
"对玩家攻击欲极强、咬住不放"的核心特质。唯一例外是**实际投焰后最长 1.25 s 的局部 break**：
它只为兑现本次诱饵，成功/失败即返回原攻击循环。热诱弹耗尽后不解锁持续规避作为最后挣扎。
...
1. **建 spec**：`docs/specs/events/ace-<name>.md`（以 [events/ace-support-squadron](../events/ace-support-squadron.md)
   ...
   混编按混编条款）**/ 生存档（默认 = 一发死 + 1 次 flare break 尝试；偏离必须特别声明）/
...
- [ ] **尝试次数可数**：BOSS 最多消耗 4 次 flare break、非 BOSS 王牌最多 1 次；次数不等于保证存活次数。
- [ ] **轨迹不变可命中**：王牌投焰后若 1.25 s 内没有真实转向/横移/增速/高度变化，导弹不失导。
- [ ] **高等级但非无敌**：有效操控 0.90–1.00 时成功率为 87%–95%，动作达门前近距导弹仍能命中。
...
- [ ] **点名铁律隐形让位**：给僚机点名一架王牌 → 它隐形瞬间僚机 `combat_target` 清空（停止空转/停火）、
      ...
      玩家无需重新点名；全程无 ENGAGE↔脱离的逐 tick 抖动，RTS 铁律/分火/防守/齐射也不能在隐形中重挂 `combat_target`。
- [ ] **Wraith cloak 节奏与近距揭露**：开战后 60s 才首次使用；结束后 60s 内来袭导弹不触发，
      满 60s 可再次使用；当前操控玩家机进入 1000px 时禁止或打断 cloak。
...
- [ ] **王牌 BGM 边沿**：入场追击阶段普通歌单不断；Ace 中队血条浮现的同一帧从四首中随机
      抽一首、2.0s 切入且不循环；单曲播完或敌对中队全灭/中和后淡入普通双曲歌单；短暂脱离
      与普通撤离不重抽、不重启。
- [ ] **BGM 权威与缺素材**：四首部分缺失时只抽可用曲，全缺时当前歌单不中断；Boss 解锁/接管后王牌终态
      不抢回普通曲；取消、重复 finish、Mode `queue_free()` 后跨下一帧均不残留所有权或报错。
...
- [x] Ace 中队血条浮现时从 4 首池随机抽 1 首 one-shot；自然播完或全灭/中和/逃离/取消后幂等归还普通歌单，Boss 优先，缺素材不中断
...
- [x] 调度器固定 3:30 / BOSS 前 3:00 两槽 + 新局无放回洗牌 + 连续两局首队防重复 + 同局不重复
...
| tier 回归测试 | `scripts/tests/test_ace_tier.gd`（`--bench=ace_tier`，含专属 BGM 登记与缺资源安全门） |
...
| 王牌专属 BGM（§2.6） | `scripts/events/ace_reinforcement_event.gd`（血条/终态所有权）、`scripts/survivor/survivor_mode.gd`（四曲随机、自然结束、Boss 优先与普通歌单恢复）、`scripts/audio/audio_manager.gd`（`ace_battle_01..04` / `music_track_finished` / playlist crossfade） |
...
| 日期 | spec_version | 改动 |
|---|---|---|
| 2026-08-23 | 20 | 用户取消敌机投焰瞬时无敌：王牌/BOSS 的 flare 从确定命数改为有限 break 尝试；实际投焰后最多 1.25s 做局部机动，只有真实轨迹达门且等级判定通过才失导。 |
| 2026-08-23 | 19 | 非宿敌王牌改为固定两槽：3:30 第一波、BOSS 前最后 3:00 第二波；删除前队终态后冷却与 540s 截止。 |
| 2026-08-22 | 18 | 收到四首 Ace 阶段曲：每次血条浮现从可用四曲中等概率抽一首 one-shot；该曲自然播完或 Ace 队终止即淡入普通歌单，本次遭遇不续播第二首。 |
| 2026-08-22 | 17 | 非 BOSS 王牌的中队血条浮现时，同一上升沿切入共用专属 BGM；全灭/中和/逃离/取消后幂等恢复普通歌单，Boss 与终局权威更高；素材缺失保持原歌单。 |
| 2026-08-22 | 16 | Wraith cloak 实战削弱：可重复使用但首次与每次结束后均严格等待 60s，取消来袭导弹提前触发；1000px 内禁止/打断 cloak，渐变延长至 1.0s；玩家 `combat_target` 写入权威拒绝隐形目标，堵住 RTS tick 重挂。 |
