...
### 2.1 尺寸决策表（战场核心 60×60 km；当前真边界 64×64 km）
...
| **×2（核心，已拍板）** | **60×60 km** | **15000** | 180 s | ≈4720 px（9.4 km） | 9.9% | ~120 s |
| **当前真边界** | **64×64 km** | **16000** | 192 s | 核心布局不变 | 核心布局不变 | ~126 s |
...
| 项 | 原 30km 值 | 当前值 |
|---|---|---|
| `MapBoundary.WORLD_SIZE_M` | 30000 | **64000**（真边界；60km 核心入口由 `CORE_HALF_PX=15000` 独立保留） |
| `PLAYER_START_OFFSET_PX` | (0, 6400) | (0, **13900**)（距核心入口 1100px，保持旧开局遭遇语义） |
| 相机裁切 | 世界边界外放 margin | `get_camera_bounds()` 在真边界内收 **32px**，任何 zoom 不采样底图外 |
| 边界警距 | 1000px | 对 60km 核心入口保持 1000px；AI 对 64km 真边界使用 300px |
...
### 2.6 三图外缘空域（2026-08-24）

东京湾、沙漠铁路、海洋群岛三张正式底图都在旧 `±15000 px` 玩法边界之外保留了栅格
overscan；三图共同最窄覆盖仍到 `±16525.5 px`。本次只消费其中保守的 `1000 px`：

| 项 | 权威值 | 语义 |
|---|---:|---|
| 旧战场核心半径 | `15000 px` | 保留为外缘空域入口，不再是瞬时终止线 |
| 新世界半径 | `16000 px` | `WORLD_SIZE_M = 64000`，四边各扩 `1000 px = 2 km` |
| 外缘决策倒计时 | `2.5 s` | 进入外缘空域后连续计时；返回核心区立即取消并重置 |
| AI 软返场触发余量 | `300 px` | 普通敌机与战术脱离可进入大部分外缘带，只在真边界前收容 |
| 相机内容内缩 | `32 px` | 视口边缘与电影抖动仍不能采到三图栅格覆盖之外 |

倒计时归 `MapBoundary` 单一状态机所有：进入任一边的外缘带开始，离开外缘带即取消；只有
连续耗尽后才发既有边界决策事件。正常阶段仍提供补给 / 调头 / 撤退，BOSS 阶段沿用补给禁用，
因此只剩调头或撤退。玩家不能因单帧越线立刻被打断，也不能在入口线来回抖动重复弹窗。

相机钳制改用新世界矩形向内 `32 px` 的安全裁切矩形，删除旧“允许看出界外 5km”的相机余量；
任何 zoom 下视口四边都必须留在正式底图共同覆盖内。Tab、栅格 LOD、UGC 文档尺度、增援周长、
退场点和世界边界查询继续从 `MapBoundary.WORLD_HALF_PX` 派生，三张官方地图不分叉实现。

AI 的普通边界纪律、恐惧逃向和 joust 脱离统一使用新真边界前 `300 px` 软余量；飞机类 BOSS
同样允许进入外缘带，再在 `300 px` 触发、`800 px` 解除，并把返场目标放到核心线内侧。
撤离 / 增援的显式跨线航路继续豁免普通收容，保证物理入场与离场不被抢写。
...
- [ ] i18n：外缘接近、正常/BOSS 倒计时与决策面板文案三语齐全
- [ ] 三图外缘空域：进入旧 `±15000 px` 线后 `2.5 s` 内不弹决策；返回核心区取消；耗尽后只触发一次
- [ ] 相机在最小/最大 zoom、自由平移与电影偏移下均不显示正式底图外的黑边
- [ ] 普通敌机、fear/joust 与飞机类 BOSS 能进入外缘带，并只在新 `±16000 px` 真边界前按统一余量返场
- [ ] 三张官方地图共用同一边界实现，preview 文档尺度均为 `64000 m`
...
### 阶段 5 — 三图外缘空域
- [x] 东京湾 / 沙漠铁路 / 海洋群岛统一 `WORLD_SIZE_M` 60000→64000；两张 preview 文档同步
- [x] 旧核心线外连续 `2.5s` 倒计时；返回核心取消；耗尽后沿用补给 / 调头 / 撤退决策
- [x] 相机内容矩形严格内收 32px；三张正式栅格均通过 64km 覆盖审计
- [x] 普通敌机、joust/fear 与飞机 BOSS 只在新真边界前返场，并保留越线当帧硬钳底线
...
| 2026-08-24 | 8 | 三图统一消费底图 overscan：旧 60km 核心外增加四边各 2km 外缘空域，新真边界为 64km；进入外缘连续 2.5s 后才打开补给/调头/撤退决策，返回核心取消；相机安全裁切与普通/BOSS AI 边界纪律同步认新边界。 |
