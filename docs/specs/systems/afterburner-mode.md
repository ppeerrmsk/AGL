...
> 把语义模糊的"规避模式"改造成显眼的小队级充能资源"加力模式"：条充满后一键激活，全队 6 秒极速冲刺 + 机炮完全打不中 + 滚转躲导弹，代价是 6 秒内无法攻击 + 30 秒充能。给玩家一个明确的"脱离-重整-再入"战术按钮。

## 1. 设计意图（Why）

- **体验目标**：
  - 现状痛点：规避模式（E 键开关）名字和作用都让人疑惑——"规避了什么？"收益不可见（+20% 机炮闪避是暗数值）、无成本无限开关导致没有使用决策，玩家几乎不用。
  - 改造后：**名字直白（加力）、收益直观（全队瞬间拉满速 + 敌人打不中）、成本清晰（一条看得见的充能条）**。被导弹雨围剿时按下去 = 全队脱离火网的"逃命键 / 重新进场键"。
  - 击杀充能形成正循环：打得凶 → 充得快 → 更敢往火网里进场。
- **Litmus 自检**（引 [DESIGN_PHILOSOPHY.md](../../DESIGN_PHILOSOPHY.md)）：
  - **单杠杆**：一条资源、一个按钮、一个固定窗口。无档位、无滞回、无部分激活。
  - **效果即反馈**：速度陡增（加力焰）+ 子弹全闪避触发滚转动画 + 导弹被甩偏飞——效果本身就是反馈，不加中介 HUD 数字。充能条是资源本身的可视化，不是解释性 UI。
  - **不破坏"一击毙命"张力**：不是无敌泡泡——导弹保留 10% 极限命中、窗口仅 6 秒、期间全队丧失攻击能力（有真实代价）。
  - **输入语法**：E / 按钮 = 全队广播（符合"轮盘/模式键带动全体"的操作语法，不设计部分队员加力）。
- **反模式规避 / 底层复用铁律**（用户点名"底层代码什么都不要变"）：
  - `evasion_mode` 字段名、set/传播链（`escort_cover_active` 广播）、AI 自保 `enter_evade` 语义、§1.2 技能钩子、TORP/WMN 投放、无线电 break 呼叫**全部保留原样**。
...
| 常量 | 值 | 说明 |
|---|---|---|
| `CHARGE_MAX` | 30.0 s | 满格容量。激活条件 = charge ≥ CHARGE_MAX（满才能放）。 |
| `CHARGE_RATE` | 1.0 /s | 被动充能速率（空格 → 满 ≈ 30 秒，用户定值）。 |
| `KILL_CHARGE` | 4.0 s | 小队任一成员击杀 1 个敌人（空中或地面）→ +4 s 充能。 |
| `WINDOW_DURATION` | 6.0 s | 激活后的加力窗口时长，固定，不可提前取消、激活中按键无效。 |
| 初始 charge | `CHARGE_MAX`（满） | 开局即可用一次，保证新机制第一时间被发现。可调。 |
| 激活消耗 | 全部（charge → 0） | 窗口开始瞬间清零；窗口内击杀照样 +4（为下一轮攒）。 |
| 窗口内被动充能 | 0 | ACTIVE 期间被动充能暂停，窗口结束恢复。 |

### 2.2 窗口内强 buff（挂 `Aircraft.afterburner_window_active`，全队生效）

| 项 | 值 | 说明 |
|---|---|---|
| 机炮闪避 | **100%**（无条件闪避，绕过全局 dodge cap 0.85） | 每次闪避照旧触发滚转动画（沿用既有节流）。理由：6 s 限时 + 30 s 资源代价，换取 cap 的 15% 命中窗口豁免。 |
...
| 加速度 | `ACCEL_MULT = 3.0` | 窗口内加速度 ×3。没有它，典型加速度从巡航加到顶速需 ≈7 s，6 s 窗口内根本"快不起来"。失速地板照旧（不破坏"转弯不得自陷失速"）。 |
...
| 状态 | 进入条件 | 期间行为 | 退出 |
|---|---|---|---|
| CHARGING | 初始（若非开局满格）/ 窗口结束 | charge += CHARGE_RATE × dt；击杀 +KILL_CHARGE；clamp 到 CHARGE_MAX | charge = CHARGE_MAX → READY |
| READY | charge 满 | 等待玩家触发（E / HUD 按钮） | 触发 → ACTIVE |
| ACTIVE | 玩家触发 | 窗口倒计时 6.0 → 0；被动充能暂停；击杀仍 +4（存入下一轮）；再按 E 无效 | 计时归零 → CHARGING（charge 从击杀存量起充） |

- 升级 UI 暂停（get_tree().paused）期间模块不被驱动 → 充能与窗口计时自然冻结。
- 触发时若处于 CHARGING：无操作（按钮/条的状态显示即反馈，不做额外弹窗）。

### 3.2 激活流程（触发瞬间）

```
try_activate(leader):
  if charge < CHARGE_MAX or window_active: return false
  charge = 0; window_left = WINDOW_DURATION
  window_members = [leader] + 所有 squad.leader == leader 的存活僚机（快照，不追新）
  for m in window_members: m.afterburner_window_active = true
  leader.set_evasion_mode(true)        # 既有链路原样：清指令、planner EVADE(max+AB)、
                                       # 传播 escort_cover_active 给僚机、radio break 呼叫、
                                       # evasion_modifiers cd 缩放、隐身/overstock 技能计时启动
  return true
...
- 窗口成员为**激活瞬间快照**：中途换帅（1-4 切控）、新僚机入场都不改变本窗口 buff 归属；成员遍历带 `is_instance_valid` 守卫。
- 模块**不缓存长机引用**（`try_activate` 传参即用），规避 SEAM-019 换帅悬挂引用问题。

### 3.3 窗口内交互规则（与既有系统的叠加）

| 场景 | 行为 |
|---|---|
| 玩家窗口内下移动/攻击命令 | 照现状：命令下达会退出长机 `evasion_mode`（S 型自动机动、TORP 投放等技能钩子停止）。但 `afterburner_window_active` 独立倒计时——强 buff（闪避/躲弹/静默/满速地板/加速）**保持满 6 秒**。点哪就全速飞哪。 |
...
| 机制 | 关系 | 裁定 |
|---|---|---|
| 全局机炮闪避 cap 0.85 | **结构冲突** | 窗口内短路为 1.0（唯一绕 cap 通道，6s 限时资源代价换取）。 |
...
| `evasion_overstock`（每 4s +1 弹） | 缩水但成立 | 6s 窗口 ≈ 每次 +1 发。文案不变（数值仍准确）。 |
| `evasion_stealth`（进入 2s 后隐身） | 成立 | 6s 窗口 → 4s 隐身段。 |
...
| 玩家命令铁律（commanded_target） | 兼容 | 窗口静默不清除点名目标，仅暂停开火 6s（玩家自己按的全队防御指令，优先级成立）。 |
| 敌机 AI `evasion_mode` | 隔离 | 强 buff 只认窗口标志；敌机永不置窗口标志。 |
| 无线电 break 呼叫 / kill feed | 保留 | `evasion_started` 上升沿照旧（激活加力时长机喊 break，语义仍通）。 |
...
- **新模块** `scripts/survivor/afterburner_charge.gd`（`AfterburnerCharge extends RefCounted`，模块化约定）：持 charge / window_left / window_members；方法 `update(delta)`、`try_activate(leader)`、`on_kill_charge()`、查询器 `ratio()` / `is_ready()` / `is_window_active()` / `window_ratio()`。不持长机引用。
...
- **接线（生存层）**：survivor_mode 建实例、`_process` 驱动、E 键与 HUD 按钮改走 `try_activate`、击杀 handler 调 `on_kill_charge`。
- **HUD**：战术按钮区 `_btn_evasion` 上方加充能条（StyleBoxFlat 线框，宽同按钮，高 ~10px；充能=暗橙填充，READY=亮橙+满格，ACTIVE=亮青倒计时收缩）；按钮文案三态（§2.3 key）。

## 5. 验收标准（Acceptance / Litmus）

- [ ] 开局条满；按 E 全队进入加力：速度明显陡增至顶速（加力焰亮）、6 秒后自动退出、条清零开始重充，~30 秒充满。
- [ ] 窗口内敌机机炮对全队 0 伤害（每次闪避有滚转动画）；EventLogger 无玩家队 GUN 受伤条目。
- [ ] 窗口内无 flare 被导弹追：约 9/10 弹在命中瞬间被甩偏（AB_MISSILE_DODGE 日志 + 偏飞），偶发命中存在。
- [ ] 窗口内全队机炮/导弹/火箭静默（含被点名 commanded_target 的僚机）；窗口结束点名目标自动恢复开打。TORP/WMN 照旧投放。
- [ ] 窗口内下移动命令：飞机满速飞向目标点，强 buff 不掉。
- [ ] 击杀充能：窗口外击杀敌机 charge 可见 +4s 跳增；地面击杀同样。
- [ ] 充能未满按 E 无反应；ACTIVE 中按 E 无反应。
- [ ] AI 自保规避（玩家托管被咬 / 敌机 EVADE）不触发窗口、不清资源；敌机规避时玩家机炮命中率与改造前一致（+20% 闪避不变）。
- [ ] 沙盒 E 键行为不变。
...
### 阶段 1 — 资源模块 + 生存层接线
- [x] `afterburner_charge.gd`：常量 + 状态机 + `try_activate/update/on_kill_charge/查询器`。
- [x] survivor_mode：建实例（字段初始化）、`_physics_process` 驱动（早退后 → 升级暂停冻结）、E 键改 gate、`_on_radio_kill_recorded` 处 +4s、spawner 地面击杀挂点 +4s、窗口到期 `_end_window` 对称清理。
