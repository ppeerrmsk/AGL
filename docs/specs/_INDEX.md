...
| [systems/zone-atmosphere-combat](systems/zone-atmosphere-combat.md) | system | done | ✅ | **正式战区氛围战斗**：空战复用既有目标；陆战专用固定阵营 SPG 对 SPG并在三种阵型、槽位与轨道相位中一次性随机，AA/SAM 严格只对空；AI 互锁不生成玩家锁框；港池水面排除并要求 50px 连续陆地；海战只补友军并复用正式敌舰。画外持续开火但零杀伤，3/3.6km 迟滞进入实伤。 |
| [systems/bomber-escort-zone](systems/bomber-escort-zone.md) | system | in-progress | ✅ | **可选战区任务：轰炸机护送**：正式局 150s+Lv5 后先广播、6 秒后生成；整局保底 1 次、20% 第 2 次且最多 2。七线等时、实体陆地目标、场外 3×B-1B + 2×F-4E；成功只给特殊 XP，FAILED 无痕。 |
...
| [systems/boss-clear-progression](systems/boss-clear-progression.md) | system | done | ✅ | **按各 BOSS 历史击败次数切换编成/机制**：初见为教学层；首败后 Wraith 接战追加 2 架雷达静默 YF-23 可选狙击支援，LADON 航母由 0CG+2DDG+6FFG 强化到 2CG+2DDG+8FFG，Mother Goose 解锁 MQ-111/112；MQ-111 激光保留减速并累计消耗导弹拦截 HP，过热循环与玩家 X-02 完全一致。历史次数 ≥2 暂沿用强化层 1，等待后续设计。 |
...
| [systems/career-shop](systems/career-shop.md) | system | in-progress | ✅ | **起手机解锁与生涯商店**（depends_on career-archive）：F-86/A-10/F-15 三项空中支援授权各自每局首次合资格事件出动一次；机场防空网购入后仍为每局每座解放机场追加一次性 SAM×1。原 `airfield_sam_network` 局内技能删除；正式局查购买态，非正式局 fail-open；差既有定向回归。 |
...
| [bosses/mother-goose](bosses/mother-goose.md) | boss | done | ✅ | Mother Goose 飞行翼母舰：10 挂点 + 弱点、JAM 力场、指定猎杀、UAV 蜂群、MQ-X 精英；VLS 在 3000m 内停火、飞行 8000m 后形成 800m AOE；MQ-111 激光可真实拦截导弹并完全继承玩家激光过热规则；登场先播系统入侵身份横幅再切母机镜头 |
| [bosses/hypersonic-splitter](bosses/hypersonic-splitter.md) | boss | done | ✅ | **Black Star / Hyper-A 高超音速分裂体 BOSS**：两架根机先后以 4 秒 `30,000m→5,500m` 状态栏 / AOE 预警再入，各自三次二分 `1→2→4→8`，整场 30 节点、16 个 G3。G0/G1/G2/G3 为 `800/300/100/70 HP`、`8/4/2/1` 锁、全代 4 发导弹弹匣且 0 flare；G0 点射激光，G0/G1 冲刺左右各 5 火箭，G2 冲刺无火箭，G3 机炮狗斗无冲刺。G1/G2 可爬升至 15km 外隐身免伤再入；G0–G2 冲刺后靠近当前操控机并自施标准 `SLOW 5s`。树 HUD、统一身份演出、七个 Debug 直达、生命周期 / 16 G3+Sentinel 压测与 Visual 已落地；全部节点及最终胜利 0 XP。 |
...
| [enemies/deadair](enemies/deadair.md) | enemy | in-progress | ✗ | **DEADAIR「断讯」累积 JAM 支援 Schemer**：3000m 移动场；友方单位 8s 累积后持续 JAM，制导导弹按 4× 在 2s 失导；HP55 无武装核心 + 2 动态护卫，鼓励玩家压入一次机炮攻击跑；与 Snowblind 在场互斥。**实现与验收中**。 |
...
| [skills/berserk-virus](skills/berserk-virus.md) | skill | done | ✅ | **狂化病毒**：次世代斗士全队技能；僚机不可主动接管并锁定 FREE，仍服从其它命令；常驻机动与 weapon/flare CD 强化，击杀进入标准 BLOODLUST。 |
...
| [weapons/rocket-ripple-trajectory](weapons/rocket-ripple-trajectory.md) | weapon | done | ✅ | 敌我飞机火箭改为左右挂点逐发交替：先沿出膛航向平行直飞 180m，再于 320m 内 smoothstep 渐进展开到各资源原散布半角；伤害、引信、射程与最终扇面不变。专项 46/46、相关回归、45 架压力与真实 Visual 均通过。 |
...
| [systems/zone-reward-arsenal](systems/zone-reward-arsenal.md) | balance | done | ✅ | 首批 A/B 奖励目标延迟到 60s+Lv3，先广播、6 秒后生成；保留武器/次世代双保底。四类奖励、七件武器、航母第 4 次保底与 F6 直发契约不变。 |
...
| [events/ace-support-squadron](events/ace-support-squadron.md) | event | in-progress | ✅ | **MARATHON / 马拉松**：Su-35×5 斗士，1 flare/架、猩红涂装、PACER 长机；10 DU+25s access=预计 TTK 75s；统一 240s 洗牌轮换；全灭 +60s 作战时间。已购 `support_ace_f15` 时仅本局首次事件派 Hound-1/2 两架 3000m 雷达 F-15，并播固定双句，终态物理撤离。**差 playtest** |
...
| [systems/map-expansion](systems/map-expansion.md) | system | done | ✗ | 地图扩展 + 战区重排（**60×60km ×2 已手改落地**，用户二次复审决定保留）：主开关/出生点/60km 重烘焙（矢量+正式 PNG 底图）/战区 ×2 重排（B 落湾里经 land_mask 网格扫描修正 (6000,-11000)）；无头回归 `tests/test_map_expansion.gd` 全绿（几何 15/15 + 陆地占比 + BOSS 锚点）。**差 playtest（≥3 战区/局节奏）；编辑器 converter 只改 gameplay 矢量数据，正式 PNG 按 2026-08-08 裁决保留** |
...
| [systems/radio-chatter](systems/radio-chatter.md) | system | done | ✅ | 无线电通讯（皇牌空战式）：一次一条、绝不打断；JSON + translations.csv 驱动。奖励目标与护送任务均新增 scripted 通报，并在目标实体生成前 6 秒入队。**差音效素材 + playtest 调频度** |
