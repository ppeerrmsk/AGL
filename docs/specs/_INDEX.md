...
| [systems/battlefield-atmosphere-experiment](systems/battlefield-atmosphere-experiment.md) | system | in-progress | ✅ | **真实生存局海陆空气氛实验**：F5 分为空战（3v3 固定翼、双轰炸、复用现有 Apache 轮廓 AH-64 且禁火箭）、炮战（3v3、3m/s 解析式轨道火炮）和海战（复用现有 DDG+FFG，必然命中的低伤害舰炮、无鱼雷）；AI 演员伤害 ×0.10，玩家保持满伤害，2Hz 控制器只在本次实验域内改派。实施中。 |
