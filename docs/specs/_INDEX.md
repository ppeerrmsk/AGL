...
| [bosses/wraith-squadron](bosses/wraith-squadron.md) | boss | draft | ✗ | WRAITH 中队（F-47）战术规格，depends_on ace-squadron-tier。目标=**本作最强敌人之一，强度来自四机协同的两难而非数值**：KNIGHT×2 逼你转弯 / SNIPER×2 惩罚你转弯。角色真实化（取代 `combat_specialty` 只写不读 + `f47_role` 只读不写两个死 meta）；队级战术 **PERCH 高位 → BRACKET 诱敌包夹（两翼分离轴 ≥60°，BAIT 不开火）→ PRESS → RESET**；**退化检测**（平均机头偏角 >50° 持续 6s 强制重整）根治绕圈；执行精度失误三项（瞄准 ±1.2° / 机动惩罚 / 25% 减速迟滞，**敌机现为零瞄准误差**）。**待 review** |
...
| [systems/afterburner-mode](systems/afterburner-mode.md) | system | in-progress | ✅ | 加力模式（规避模式资源化改造）：小队充能条（30s 充满 / 击杀 +4s / 开局满格）满格一键 → 全队 6s 强 buff——100% 机炮闪避（唯一绕 dodge cap 通道）+ 90% 滚转甩导弹（走 flare-jam 偏飞契约）+ 禁攻击（TORP/WMN 例外）+ 满速地板 & accel ×3；底层 evasion_mode 链路全保留（AI 自保 / 敌机行为不变、不触发不耗资源），16 项冲突裁定见 §3.5。**代码全落地 + i18n 三语，差 §5 playtest** |
...
| [systems/skills-720-rework](systems/skills-720-rework.md) | system | draft | ✅ | 720 技能整改批（用户逐条改表）：**"+1 轴进度"系统**（选卡奖励里程碑进度非技能点，13 条；预留档通道；cap 2 待二选一）+ 归属词汇 v6（王牌层为 AoE 控场强技收敛回归/A10 限定=exclusive_to/需要词条=requires_skill/队级单实例 1→8）+ 新增 27（含 R 键手动闪避/僚机阵亡触发×3/技能计数缩放×4/轮盘联动×2）/ 改动 ~35 / 移除 railgun_damage；实现对照四档（纯数据/复用钩子[加力充能·击杀归因·轮盘状态·recompute]/追加功能/新机制）；排查双项（数据链生效+僚机锁可射、寒蝉友军 JAM bug）；任务拆分 T0~T6。**待 review** |
...
| [systems/ui-transition](systems/ui-transition.md) | system | draft | ✅ | 表演导演系统（转场/镜头/时间/演出）：根治"升级面板啪地跳出来"+"BOSS 默默飘进战场"。地基=**TimeAuthority**（时间请求栈，最小值获胜，`Engine.time_scale`+`get_tree().paused` 唯一写入点，收编现散在 4 文件 9 处）+ **SequencePlayer**（step 时间线，数值全外置 `sequences.json`，F8 热重载）+ **7 条通道**（time/camera/overlay/panel/**radio**/**stage**/**actor**）。实例①升级**急刹车** 0.44s：时间 1.0→0.05 急停 → 镜头推近 ×1.12 → 卡片错开 0.07s 弹入（走 `scale`+`modulate`，**不用 position**，Container 会覆写）。实例②**Wraith 登场** 6.4s：**空舞台隔离**（`hard_pause` 冻世界 + 演员 `process_mode=ALWAYS` + 其余单位淡到 0/地图压 25%，用户"假空间"方案，不建第二 Viewport 不重父级）→ 四机菱形错开淡入列队飞入（复用 Poltergeist FADING 模式 + **`AIDirective.follow_path` 已造好但全项目零使用的现成钩子**，真物理禁 K 帧）→ 三句既有台词 → 推近 → 世界淡回。**关键安全约束**：演员指令**委托 `GameEvent` 下发**复用其 owner/cleanup，绝不造第二套所有权（泄漏=BOSS 永不参战且不报错）；三类泄漏（时间/舞台/演员）全覆盖无头断言 + 7s 硬超时收尾。**决策**：不可跳过 + 7s 硬上限（无尽波次会看几十遍）、无线电编排权归演出但渲染仍走无线电条（否则叠字+破"绝不打断"契约）、双 overlay 层 16/25（避免压暗无线电）、事件计时改 unscaled delta、不造通用 beat API（样本仅 1）、编队特技推迟阶段 3。**阶段 1 地基+升级，阶段 2 Wraith 演出，阶段 3 特技/其余 BOSS/场景切换** |
...
| [systems/ace-squadron-tier](systems/ace-squadron-tier.md) | system | in-progress | ✗ | **敌方**王牌中队分层标准（勿与 ace-system 的玩家 ACE 混淆）：王牌中队=宽泛 tier（最强机种/不吃 LOD/极强攻击欲/无等级缩放），BOSS=其子集（更强+专属演出+击败即过关）。核心生存模型=**热诱弹即命数**（4 命 / 每次 1 枚 / 必定成功 / 不补充 / 耗尽即必死，不解锁规避机动）；血量豁免 ENEMY_HP_MISSILE_CAP 但只到"残血"档；专属 `ace_gun.tres` 修正"BOSS 火力不如杂兵"；隐形一致性铁律（补齐 7 条绕过 is_lock_immune 的索敌通路）。**阶段 1+2 已落地**：隐形通路全补齐 + `evade_missiles` 死配置删除 + 机炮前置解弹速统一；新建 `AceTier` 模块（`scripts/survivor/ace_tier.gd`）收敛 LOD/缩放/HP cap 三处散点判定，`max_hp=100`（高于全部玩家导弹伤害 → 耗尽热诱弹后必经残血），`--bench=ace_tier` 20 断言全绿。**差阶段 3（热诱弹命数资源 + jam=1.00）/ 阶段 4（ace_gun.tres）/ playtest** |
...
| [systems/aa-fire-awareness](systems/aa-fire-awareness.md) | system | in-progress | ✅ | 僚机对地面机炮火力警觉：①被 AA/CIWS 机炮命中 → 打断 SETUP/RUN 强转 EGRESS + 机头转出 45° 后 AB 全速脱离（EGRESS 加速为敌我通用改进）；编队/巡航被打不脱队只 AB 直线冲刺 2.5s；②STANDOFF inner 环抬到目标对空火力半径 ×1.25（CIWS 舰 2200→2500m）+ F-Pole：弹在飞/TEAM_OVERKILL 时不压入、环外 crank 等命中。单杠杆=只对"被命中"反应，不做火力圈预判扫描。**代码全落地 + --bench=surface_pass §E 8 断言（28/28 绿），差生存 playtest/压测** |
