...
| [systems/player-instrument-hud](systems/player-instrument-hud.md) | system | in-progress | ✅ | **玩家与僚机仪表 HUD**：右侧玩家仪表高 `24u`；SPD、G 整数位与 FLR 当前值共用 `49 × 54 px` 固定字号数字框；AUTOPILOT 锁定 `3u × 6u` 并由动态空框板吸收 R 键；默认武器区使用上下两条有框 `0.5u` 装饰行、独立优先标题、两个 `2u` 槽以及贴近名称框的竖向装填条；玩家/僚机仪表随 HUD 阅读顺序在本局首次可见时闪两次后常亮，差 Godot 4.7.1 生存模式实机节奏确认。 |
...
| [bosses/hypersonic-splitter](bosses/hypersonic-splitter.md) | boss | draft | ✗ | **高超音速分裂体（原创名称待定）机制 BOSS 概念占位**：画线高速贯穿、连续冲刺后散热减速；每个 G0–G3 个体被击破都一分为二并弱一代，形成 `1→2→4→8→16` 异步分裂树。完整战斗共 31 个节点，只有 16 个 G4 终末体全部消灭才胜利。各代数值、命名、攻击并发与奖励规则待后续拍板，尚未实现。 |
...
| [systems/ui-transition](systems/ui-transition.md) | system | in-progress | ✅ | 表演导演系统（转场/镜头/时间/演出）：TimeAuthority + SequencePlayer + time/camera/overlay/panel/radio/stage/actor/audio 通道。升级急刹 0.44s；所有注册 BOSS 生成后立即使用统一 arrival 系统。Wraith 保留 6.7s 四机交汇专属分镜；CSG 为旗舰镜头+3 句无线电+回玩家（6.7s）；Mother Goose 为母机镜头+2 句无线电+回玩家（5.1s）。演出收尾立即 ENGAGED；缺序列或被 UI 转场覆盖时 fail-open 接战并亮血条，统一受 7s 硬上限与时间/舞台/演员三类泄漏兜底约束。玩家 HUD 框板另以固定登记表按屏幕空间顺序 `0.02s` 错峰并发启动，每块 `0.50s` 内两闪；右下仪表按功能框、僚机行与三轴格独立登记并直接开关源绘制，不使用黑底遮罩，首次绘制前默认关闭以杜绝总体 `1px` 描边。 |
...
| [systems/radio-chatter](systems/radio-chatter.md) | system | done | ✅ | 无线电通讯 v8（皇牌空战式）：一次一条、绝不打断；JSON + 独立 `radio.csv` 驱动。已按 2026-08-10 Notion 修订补齐 WRAITH/CSG/Goose、八方向轰炸护送、海空地战区与机场解放触发；三语与聚焦回归已过。**差音效素材 + 4.7+ 实机试听调频度** |
