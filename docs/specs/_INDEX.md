...
| [systems/radio-chatter](systems/radio-chatter.md) | system | in-progress | ✅ | 无线电通讯（皇牌空战式）：屏幕上方"呼号 ▸ 台词" + Radio 总线电台底噪（该总线早已建好、此前从未被使用）。**核心契约=一次一条、绝不打断**（优先级只管排队与满队淘汰）+ 类别冷却防刷屏 + 队列上限 3 + 过期丢弃（BOSS 剧本序列豁免，否则挑衅收尾句会被砍）。阵营色引 FactionPalette（玩家蓝/ALLY 绿/敌橙·精英红）。8 类触发：BOSS 登场多句队内对话（**范例**，三 BOSS 各一套）/ BOSS 交战 / 击坠回报 / 弹射呼叫 / break 规避 / RTS 五条指令回令（追击·包围·掩护·集合·撤离，中队级只回一句）/ 敌方累计减员三档哀嚎 / 僚机归队。入队即快照呼号+颜色，不持有 Aircraft 引用。--bench=chatter 50 断言 + 回归门 25 项 PASS。**差音效素材 + playtest** |
