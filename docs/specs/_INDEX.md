...
| [weapons/airburst-aa-gun](weapons/airburst-aa-gun.md) | weapon | in-progress | ✓ | **远距空爆高炮**：450m/s 三连发炮弹按冻结引信飞行，220m/75 一次性对空 AOE；组/单发随机偏角封顶 7°/1.5°，既允许整组落空又保证明显朝预瞄空域发射；爆点为白橙放射火光 + 黑灰烟团。普通战区最多一门并替换现有 AA 槽。已实现，待实机 QA。 |
...
| [systems/zone-air-support-naval-safety](systems/zone-air-support-naval-safety.md) | system | done | ✅ | **战区友军空中支援 + 对舰水域硬闸**：保留现有任务池；已购生涯授权后，对空 ACTIVE 按星级生成 2/3/4 架 F-86，非机场对地 ACTIVE 生成 2 架纯机炮、只攻 GroundUnit 的 A-10；统一第三方收益隔离与物理撤离。对舰继续执行 40px 全水硬闸。定向回归 129/129；49 架三支援压力样本 146 headless FPS。 |
...
| [systems/boss-hunter-doctrine](systems/boss-hunter-doctrine.md) | system | done | ✅ | **BOSS 猎手准则 v4**：废除 `ANCHOR_HOLD`，ENGAGED 后持续追玩家；出生点位于玩家机头前方 12km，BOSS 圈跟存活成员质心。飞机类 BOSS 自管**世界边缘收容**（2000px 触发→3000px 安全带、越线钳回 40px），仅守世界外框、不是锚点 leash。**2026-07-29 用户统一接战时机**：全部 BOSS 在 spawn 后立刻走表演导演 `<boss_id>_arrival`，镜头回玩家后立即 ENGAGED；旧进圈/贴近/被锁/受伤 T1~T4 退出主流程，缺序列 fail-open 直接接战。CSG 猎手性由最多 8 架舰载机承担；Mother Goose 巡逻环跟随玩家。附带 `BossEncounter.set_player_ref()` 活引用契约与 CSG 水面摆位/承伤规范。 |
...
| [systems/evolution-attribute-gates](systems/evolution-attribute-gates.md) | system | done | ✅ | 三轴点数、进化门与全队里程碑主体已完成。**v14（2026-08-03）**：斗士 2 点射程、3/4 点 HP/装甲；6 点 G 从不可感知的 +0.2 强化为 **+2.0**，8 点补失速包络。骑士导弹挂载/多目标仍压在 6/8 点后期兑现；策士不变。 |
...
| [systems/aircraft-signature-skills](systems/aircraft-signature-skills.md) | system | done | ✅ | 机体签名技能效果表（用户 722 表）：41 机每机 1 条专属技；全 CLASSIFIED ×1；获得后跟玩家走（换机重放不查门控）；轴归属 + milestone_plus（数组化支持 AX-00 双轴）；三个新事件点、致死拦截、超速截击、作战云、传感器融合、生成类等效果已落地。**2026-08-01 起获取方式改由 aircraft-signature-progression 接管**：不再进普通池，原 ×2.5 权重方案废弃；洋红专属卡框语义保留。 |
| [systems/aircraft-signature-progression](systems/aircraft-signature-progression.md) | system | done | ✅ | 41 条机体专属技已改为“亲自获得机体 → 功勋商店揭示/购买 → 每机每局一次 90% 第四槽机会”，普通三轴三卡不被挤占，跳过后本局消失；未知机体只显无信息 `???`。生涯商店已重构为战术学说/机体专属/战场支援/机体与后勤四分页；AWACS 以 3000 功勋商品门控现有支援事件。普通 4 级与机体专属卡入场后边框按自身语义色闪亮一次（专属优先去重）。自动 bench 已闭环，余实机视觉/概率手感微调。 |
...
| [systems/zone-reward-arsenal](systems/zone-reward-arsenal.md) | balance | done | ✅ | 战区奖励军械库（2026-07-22 用户）：奖励 roll 扩第四类 **nextgen 次世代技术**（★15/★★30/★★★40，候选=全仓 4 条 NEXT_GEN 技能按可用性/stacks/跨区去重过滤，空则降级 weapon）；**2026-08-01 每局保底**：开局 A/B 随机各出 1 个武器类 + 1 个技能类（至少各一，后续仍按原权重）。fear_on_lock/data_link 补 evolved 出卡池，**顺带修复 missile_swarm/evasion_stealth 720 批孤儿**；武器子池 +**火箭弹/电磁炮/激光**（equipment 泛化 + weapon_inventory 换机继承，★ 区无电磁炮/激光，已持有同类过滤）；修订 zone-reward-docking §0"副系统限定"边界（依据 inrun-weapon-inventory 既定方向）；ZoneRewardRegistry 死表维持退役。**已落地 + 200 局 roll 回归绿，差 playtest** |
