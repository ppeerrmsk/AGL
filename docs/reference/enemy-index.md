...
| `YF23(29)` | YF-23 Black Widow II | **Wraith 通关强化专属支援** | `enemy_yf23.tres` + `f47_missile.tres`（AIM-260）+ `default_flare.tres`；无机炮；传感器隐形 | 0（事件供给，不进随机池/无缩放） | **2** | Wraith 历史击败 ≥1 | `F47AceSquad.engage()` 在 Wraith 队形后方生成左右各一架；`boss_support`、无永久免锁、接触建立后正常可锁定、可选击毁、不进 BOSS 血条/胜利判定 | `_create_enemy` YF23 case | `bvr_only` 4–6km + 高空 + TS_BOSS 追当前玩家；spec [systems/boss-clear-progression](../specs/systems/boss-clear-progression.md) / [systems/enemy-sensor-stealth](../specs/systems/enemy-sensor-stealth.md) |
...
|---|---|---:|---|
| 极光长枪 | `Tier3SuperCannonPart` | 1 | 圆形底座 + 加长旋转炮管绘制为一个 GroundUnit/TGT；从 2.5–44k px 随机瞄准存活长机/僚机，预警结束同拍结算整条 AOE，高速弹迹只作表现；自身无 AA，近身完全依赖周围普通 AA/SAM |
...
- **1 枚热诱弹 + fail_chance**：`AircraftFlares.update` 在来袭导弹进入 release 距离时做一次纪律判定；敌机实际不投概率为配置值 ×10%，失误后这枚导弹不再触发 flare。`enable_flare_reload=false`（默认）保证 1 枚用完就没了；投出后仍须完成真实 break 才能失导。
...
- **光学隐形**：开战后首次等待 **60s**；每次使用结束后进入严格 **60s CD**，可重复使用、无随机抖动，来袭导弹不能提前触发。持续 5.5s，淡入淡出各 1.0s；当前操控玩家机进入任一成员 1000px 内时禁止启动或提前显形。效果：淡出消失 + 雷达锁定清除 + 导弹丢失制导 + 无法选中 + 子弹穿透；左下雷达仍保留匿名位置提示。由 `ace_squad.gd` 的 `_cloak_enter/_cloak_update/_cloak_exit` 管理，全队共享计时器
