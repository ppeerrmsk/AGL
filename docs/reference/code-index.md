...
| 功能 | 位置 |
|------|------|
| 物理主循环 | `aircraft.gd:910` _physics_process |
...
| 高度更新 | `aircraft/aircraft_physics.gd:344` update_altitude |
| 失速检查 | `aircraft/aircraft_physics.gd:385` update_stall |
| G力计算 | `aircraft/aircraft_physics.gd:407` update_g_load |
| 飞行员耐力 | **代码已移除**（保留概念：将来拟真战役模式再启用；`i18n` 的 `UPGRADE_PILOT_STAMINA_*` 三语文本刻意保留） |
| 位移应用 | `aircraft/aircraft_physics.gd:440` apply_movement |
| 最大 bank 角（物理瞬时上限） | `aircraft/aircraft_physics.gd:449` max_bank_angle |
| 有效最大G（**buff 注入点**，SEAM-001） | `aircraft/aircraft_physics.gd:492` effective_max_g；瞬时结构 G = `:503` effective_max_g_instant |
| 角点速度 V=V_stall×1.2×√G | `aircraft/aircraft_physics.gd:518` corner_speed_kmh |
| 失速速度 V_stall×√G | `aircraft/aircraft_physics.gd:605` stall_speed |
| 高空最大速度衰减 | `aircraft/aircraft_physics.gd:626` max_speed_at_altitude |
| 空气密度比 σ=e^(-alt/8500) | `aircraft/aircraft_physics.gd:648` air_density_ratio |
| 旋翼机速度向量/机头解耦运动 | `aircraft/aircraft_physics.gd:1645 update_rotorcraft` |
| 高度档位切换（生存模式） | `aircraft.gd:1478` set_target_tier |

## 主动特殊机动（R 五选一）

| 功能 | 位置 |
|------|------|
| 共享状态/命中资格 | `aircraft.gd:2142 is_active_special_maneuver` / `aircraft.gd:2146 can_accept_new_hit` |
| 位移滚转激活与安全侧评分 | `aircraft.gd:2244 _try_start_displacement_roll` |
| 垂直越过激活与高度边界 | `aircraft.gd:2269 _try_start_vertical_break` |
| 动作推进与 10Hz AI 自动触发 | `aircraft.gd:2377 _update_active_special_maneuver` |
| 垂直越过俯仰投影（纵轴收缩/尾焰锚点） | `aircraft_renderer.gd:679 draw_muzzle_flash` / `aircraft_renderer.gd:690 draw_afterburner_glow` / `aircraft_renderer.gd:850 draw_aircraft_icon` |
| 五向 R 固定优先级入口 | `aircraft.gd:2397 try_manual_maneuver` |
| 小队共享冷却账本 | `squad.gd:40 active_maneuver_cooldown_s` |
| 玩家仪表唯一技能/冷却只读状态 | `aircraft.gd:2163 equipped_r_maneuver_id` / `aircraft.gd:2177 r_maneuver_cooldown_total` / `aircraft.gd:2192 r_maneuver_cooldown_remaining` |
| 常规高度物理让位门 | `aircraft/aircraft_physics.gd:344 update_altitude` |
| 弹体/光束命中前资格查询 | `bullet_manager.gd:783 can_accept_new_hit` / `missile_manager.gd:410 can_accept_new_hit` / `equipment/laser_equipment.gd:224 can_accept_new_hit` / `equipment/railgun_equipment.gd:465 can_accept_new_hit` |
| 技能数据/五向互斥 | `survivor/survivor_data.gd:1741 displacement_roll` / `survivor/survivor_data.gd:1753 vertical_break` |
| 技能授予 | `survivor/survivor_player.gd:708 displacement_roll` / `survivor/survivor_player.gd:711 vertical_break` |
| 聚焦回归 | `tests/test_skills_720.gd:323 _test_t5_mechanisms`（轨迹、边界、命中、共享冷却、AI 自动） |

## 能量管理

| 功能 | 位置 |
|------|------|
| 能量管理总入口 | `aircraft/aircraft_physics.gd:845` update_energy_management |
| 加力燃烧器开关 | `aircraft/aircraft_physics.gd:811` set_afterburner |
| 燃油消耗 | `aircraft/aircraft_physics.gd:825` update_fuel |
...
| 设定战斗目标 | `aircraft.gd:1852` set_combat_target |
| 清除战斗目标 | `aircraft.gd:1864` clear_combat_target |
| CombatParams 获取 | `aircraft.gd:1919` _combat_params |
| 自动扫描机炮目标 | `aircraft/aircraft_weapons.gd:140` auto_gun_scan（普通机炮守交战纪律；玩家全队炮艇各自扫描最近 Aircraft/GroundUnit 且不锁 planner 目标池） |
| 编队 / LOD 提前返回路径的炮艇 tick | `aircraft/aircraft_weapons.gd` update_passive_gunship + `aircraft.gd` LOD0/1/2 formation 分支 |
...
| 机炮射程（像素） | `aircraft.gd:1948` _gun_range_px |
| 子弹生成 | `bullet_manager.gd:141` spawn_bullet |
| 子弹物理+命中检测 | `bullet_manager.gd` _physics_process |
| 曳光弹绘制 | `bullet_manager.gd` _draw（区分 is_rocket）|
| 真命中火星（闪避不触发；每目标 110ms CD；12 组封顶；两次批量绘制） | `bullet_manager.gd:932` _emit_hit_spark / `bullet_manager.gd:922` _update_hit_sparks / `bullet_manager.gd:950` _draw |
...
| 火箭弹发射主逻辑（齐射排队 + 距离/角度过滤）| `aircraft/aircraft_weapons.gd:627` update_rocket |
| 单发火箭出膛 | `aircraft/aircraft_weapons.gd:717` _launch_rocket |
| 火箭弹生成（BulletManager 共享） | `bullet_manager.gd:224` spawn_rocket |
...
| 敌方火箭弹 tier 资源表（V1~V8，等级成长唯一杠杆）| `survivor/survivor_data.gd:3652` ENEMY_ROCKET_TIERS → `resources/weapons/enemy_rocket_v1.tres` ~ `enemy_rocket_v8.tres` |
| └ tier 注入点（`_create_enemy` 内按等级 duplicate 挂载）| `survivor/survivor_spawner.gd:3771` ENEMY_ROCKET_TIERS |
| └ 齐射数实际取 `burst_count_max`（`burst_count_min` 目前未被读取）| `scripts/rocket_params.gd` burst_count_max / `aircraft/aircraft_weapons.gd:627` update_rocket |
...
| 功能 | 位置 |
|------|------|
| 武器模式切换（MISSILE/GUN） | `aircraft/aircraft_weapons.gd:753` update_weapon_mode |
| 战术偏好武器模式（带机炮回退） | `aircraft/aircraft_weapons.gd:806` _update_weapon_mode_tactical |
...
| 导弹发射主逻辑 | `aircraft/aircraft_weapons.gd:861` update_missile |
| 导弹阻断日志（被锁/RNG/能量等原因） | `aircraft.gd:2695` _log_msl_block |
| 齐射空手日志（auto_fire 实际值 + 各过滤踢除计数） | `aircraft.gd:2713` _log_salvo_skip |
| 单枚发射 | `aircraft/aircraft_weapons.gd:1073` _fire_missile_at |
| 多目标齐射（有效锁数截断 + 正常冷却） | `aircraft/aircraft_weapons.gd:1109` _fire_multi_lock_salvo · `aircraft/aircraft_weapons.gd:1291` _salvo_fire_count |
| 最优目标选择（评分） | `aircraft.gd:2773` _select_best_missile_target |
| 射程包线检查 | `aircraft/aircraft_combat_tracking.gd:632` is_in_missile_envelope |
| 导弹阶段判定（接近/照射/保持） | `aircraft.gd:1982` _get_missile_phase |
| 是否应该用机炮 | `aircraft.gd:2009` _should_use_gun |
| Crank 状态查询 | `aircraft.gd:2020` is_cranking |
| 导弹射程（像素） | `aircraft.gd:1954` _missile_range_px |
| 有效导弹射程=min(导弹,雷达)像素 | `aircraft.gd:1964` _effective_missile_range_px |
| 有效射程（像素） | `aircraft.gd:1976` _effective_range_px |
| 导弹飞行物理+PN制导 | `missile.gd:149` _physics_process |
| 导弹低空制导衰减 | `missile.gd:690` _guidance_degradation |
| 导弹生成 | `missile_manager.gd:41` spawn_missile |
| 导弹命中检测 + 连锁弹头直穿 | `missile_manager.gd:337` _physics_process |
...
| 在飞导弹查询 | `missile_manager.gd:165` has_active_missile_at |
| 近炸引信 AOE 生成 | `missile_manager.gd:504` _spawn_aoe |
| AOE 区域更新+伤害 | `missile_manager.gd:528` _update_aoe_zones |
| AOE 红圈渲染 | `missile_manager.gd:590` _draw |
| 弹跳目标查找 | `missile_manager.gd:732` _find_bounce_target |
...
| 释放热诱弹（target_missile 可选，针对性释放）| `aircraft/aircraft_flares.gd:236` release |
| 干扰成功率计算 | `aircraft/aircraft_flares.gd:474` calc_jam_chance |
| 粒子更新 | `aircraft/aircraft_flares.gd:507` _update_particles |
| 固定十枚视觉波 + 同帧计数 | `aircraft/aircraft_flares.gd:538` _queue_visual_burst / `aircraft.gd` flare_visual_burst_emitted |
| 失误概率 / 对头减免（FlareParams 字段） | `flare_params.gd:19-22` fail_chance / head_on_fail_reduction |
| 规避态（底层 evasion_mode：导弹来袭 S 型 + 降高度；AI 自保 + 加力模式共用的底座）| `aircraft.gd:2443` _update_evasion |
| 规避态开关（调用方：AI 自保 enter_evade / 玩家 E 键经 AfterburnerCharge.toggle） | `aircraft.gd:2044` set_evasion_mode |
| 加力模式充能资源（充能制：有能量即开/耗尽自动关/再按关闭，spec afterburner-mode） | `survivor/afterburner_charge.gd:66` toggle |
| 加力标志（全队强 buff，生存层写入） | `aircraft.gd:622` afterburner_window_active |
| 加力机炮 100% 闪避（绕 dodge cap 短路） | `aircraft.gd:2974` effective_dodge |
| 加力滚转甩导弹（90% → is_flare_jammed） | `missile_manager.gd:431` AB_MISSILE_DODGE |
| 加力速度地板 + 加速 ×3 | `aircraft/aircraft_physics.gd:548` AB_WINDOW_ACCEL_MULT |
| 玩家仪表 HP/加力/装填统一绿黄红进度与 0.5s 闪烁 | `survivor/player_instrument_panel.gd` progress_color / _draw_progress_bar / _draw_afterburner / _draw_weapon_row |
...
| R 统一机动入口（眼镜蛇/J-Turn/胆大妄为） | `aircraft.gd:2397` try_manual_maneuver；`survivor/survivor_mode.gd:2028` KEY_R |
| 当前操控机手动 / AI 僚机自动分流 | `aircraft.gd:873` is_manual_maneuver_controlled；`:1604` _update_cobra_skill；`:1666` _update_evasion_herbst_skill；`:1693` _update_manual_dodge_skill |
...
| 锁定免疫检查 | `aircraft.gd:3465` is_lock_immune |

## 伤害与击毁

| 功能 | 位置 |
|------|------|
| 受伤（导弹） | `aircraft.gd:2904` take_damage |
| 受伤（机炮，含闪避；返回 bool 表示是否实际结算） | `aircraft.gd:2945` take_bullet_damage |
| 内部伤害应用 | `aircraft.gd:3040` _apply_damage |
| └ 敌方护卫反应（被护送对象挨打 → 护卫同拍脱队扑攻击者） | `aircraft.gd:3192` _alert_escort_guards（`TS_DIRECTIVE` + `clear_formation` + `enter_engage_state`）|
| └ 护卫名单字段 | `aircraft.gd:311` escort_guards（登记方 `survivor/survivor_spawner.gd:2091` _spawn_flee_escort）|
| 地面撞击检查 | `aircraft.gd:3225` _check_ground_crash |
| 击毁流程 | `aircraft.gd:3235` _start_destroy |
| 坠落动画 | `aircraft.gd:3247` _update_destroy |
| 基类伤害 | `combat_unit.gd:273` take_damage |

## 雷达系统

| 功能 | 位置 |
|------|------|
| 雷达锥判定（飞机） | `aircraft.gd:3286` is_in_radar_cone |
| 雷达锥判定（地面单位） | `ground_unit.gd:293` is_in_radar_cone |
...
| Simple AI 交战 | `ai_controller.gd:2038` _try_engage_simple |
...
| 巡逻逻辑 | `ai_controller.gd:2094` _process_patrol |
...
| **BOSS 登场→接战统一契约** | `events/boss_encounter_event.gd` — spawn 后立即播放 `<boss_id>_arrival`；演员取 `get_display_members()`（Wraith 全队 / CSG 旗舰 / Goose 母机）；镜头回玩家后立即 ENGAGED；缺序列或被 UI 转场覆盖均 fail-open 接战并亮血条；ENGAGED 后猎手持续追玩家（spec ui-transition §2.0 / boss-hunter-doctrine） |
...
| └ 水域/全舰 TGT 回归测试 | `tests/test_naval_zone_water.gd:22 run`（`--bench=naval_zone_water`，27 断言：战区 E 三档 40px 复核 + 4/5/6 编成 + 真实六舰实例化 + 旗舰速杀负例 + 无线电跳过已释放舰船 + 缩编/fallback + CSG 锚点） |
| **战区海上任务舰队** | `survivor/zone_mission.gd:959 _spawn_naval_formation`（4/5/6 艘；先规划后实例化；`land_hits != 0` 绝不创建）/ `survivor/zone_mission.gd:1033 build_naval_target_roster`（安全方案全舰 TGT）/ `survivor/zone_mission.gd:1046 safe_naval_plan`（缩圈→逐艘减护卫→单旗舰→零舰船退化为空战） |
| └ 编成/几何常量 | `survivor/zone_mission.gd:902 NAVAL_FLEET_COMPOSITIONS` / `survivor/zone_mission.gd:925 NAVAL_RING_RADIUS` / `survivor/zone_mission.gd:917 NAVAL_RING_CANDIDATES` / `survivor/zone_mission.gd:923 NAVAL_ESCORT_OFFSETS`（1★/2★/3★ 占地 2150/2228/2512） |
...
| 交战主逻辑 | `ai_controller.gd:2148` _process_engage |
...
| 地面/防空跨帧目标有效性边界 | `ground_unit.gd:118` _live_combat_unit_ref / `ground_unit.gd:128` _live_aircraft_ref |
| 地面自动目标选择 | `ground_unit.gd:133` _update_target_selection |
| 地面机炮战斗 | `ground_unit.gd:168` _update_combat |
| 地面机炮射击 | `ground_unit.gd:203` _update_gun |
| SAM 只对空选敌 / 导弹发射 | `sam_unit.gd:28` _update_target_selection / `sam_unit.gd:51` _update_sam_missile |
| AA 炮塔转向 | `aa_gun_unit.gd:76` _update_turret |
...
| 远距空爆高炮（陆基/舰载共用冻结解与偏角采样 + 火光/烟团 VFX） | `airburst_aa_unit.gd:99 sample_burst_solution` / `airburst_aa_unit.gd:119 sample_shell_solution` / `bullet_manager.gd:202 spawn_airburst_shell` / `bullet_manager.gd:571 _explode_airburst_shell` / 战区替换 `survivor/zone_mission.gd:479 _spawn_ground_garrison` |
...
| 状态过滤（船只接受 JAM）| `scripts/naval/naval_unit.gd:568` apply_status |
| 弱点暴露判定 | `scripts/naval/naval_unit.gd:578` _check_weak_point_reveal |
...
| 舰载 Flak hover 包线 / 挂点符号 | `scripts/naval/naval_unit.gd:812 _draw_flak_envelopes` / `scripts/naval/naval_unit.gd:899 _draw_alive_mount` |
| 子弹命中船（机炮 hull 0.15× / 弱点可磨）| `scripts/bullet_manager.gd:604` _physics_process 的 NavalUnit 分支 |
| 火箭弹命中船（hull 0.5× / 可磨弱点）| `scripts/bullet_manager.gd:604` _physics_process 的 is_rocket 分支 |
| 火箭弹 AOE 命中船 | `bullet_manager.gd:471` _explode_rocket |
| 导弹近炸 AOE 命中船（含 alt_ok 例外）| `missile_manager.gd:528` _update_aoe_zones |
...
| 技能 → 状态词条映射（keywords + EXTRA + OVERRIDE）| `survivor/survivor_data.gd:2545` status_notes_of |
| Aircraft.apply_status 覆写（UAV 滤 FEAR / OVERLOAD 钩子）| `aircraft.gd:2824` apply_status |
| NavalUnit.apply_status 覆写（只接受 JAM）| `scripts/naval/naval_unit.gd:568` apply_status |
...
| 功能 | 位置 |
|------|------|
| 相机跟随插值 | `survivor/survivor_mode.gd:2412` _process |
| 物理主循环（总入口） | `survivor/survivor_mode.gd:2433` _physics_process |
| 选中列表清理 | `survivor/survivor_mode.gd:2602` _cleanup_references |
| 飞机列表同步 | `survivor/survivor_mode.gd:2609` _update_aircraft_list |

#### 雷达锁定
| 功能 | 位置 |
|------|------|
| 全局锁定计算 | `survivor/survivor_mode.gd:2664` _update_radar_locks |
| 近距捕获（距离归一化倍率） | `survivor/survivor_data.gd` `close_range_lock_mult` → `survivor/survivor_mode.gd` `_update_radar_locks` |

#### 动态性能 / LOD / 清理
| 功能 | 位置 |
|------|------|
| FPS 采样与动态上限调整 | `survivor/survivor_spawner.gd:337` update_fps_sampling |
| 平均 FPS 查询 | `survivor/survivor_spawner.gd:358` _get_avg_fps |
| 屏幕外 AI/物理降频 | `survivor/survivor_mode.gd:2989` _update_offscreen_lod |
| 已坠毁敌机清理 | `survivor/survivor_mode.gd:3132` _cleanup_destroyed_enemies |
| 远距清理（释放 Token） | `survivor/survivor_spawner.gd:3140` _update_far_cleanup |

#### 猎手系统
| 功能 | 位置 |
|------|------|
| 猎手指派主循环 | `survivor/survivor_spawner.gd:3240` _update_hunters |
| BOSS 世界边界物理硬护栏（不接管战术/火控） | `survivor/survivor_spawner.gd:3351` enforce_boss_world_boundary |
| 空闲敌机航点围绕玩家 | `survivor/survivor_spawner.gd:3441` _update_enemy_waypoints |
| 获取 AI 控制器 | `survivor/survivor_mode.gd:3139` _get_ai |
| 导弹上限查询（飞向玩家数）| `survivor/survivor_mode.gd:3303` _count_missiles_targeting_player |
| 筛选未发射敌机 | `survivor/survivor_mode.gd:3312` _get_enemies_without_active_missile_at_player |
| 敌人数统计 | `survivor/survivor_spawner.gd:3744` _count_enemies |

#### 刷怪 & Token 烈度控制
| 功能 | 位置 |
|------|------|
| 刷怪主逻辑（每波间隔/FPS闸/Token 预算）| `survivor/survivor_spawner.gd:644` _update_spawner |
| 当前 Token 预算（随等级增长）| `survivor/survivor_spawner.gd:387` _get_token_budget |
| 重算场景 Token 占用 & 每类数量 | `survivor/survivor_spawner.gd:401` _recalc_token_usage |
| 指定类型是否可生成（预算+实例上限）| `survivor/survivor_spawner.gd:413` _can_spawn_type |
| 按等级选敌机类型（概率 + Token 约束）| `survivor/survivor_spawner.gd:439` _pick_enemy_type |
| └ BOSS/事件专属机型排除（后期随机桶，F-47 / F-14 Poltergeist）| `survivor/survivor_spawner.gd:608` BOSS_ONLY_TYPES（表在 `survivor/survivor_data.gd:3452` BOSS_ONLY_TYPES）|
| 敌人作战高度分档（按机型查权重表，替代均匀 1/3 随机）| `survivor/survivor_spawner.gd:2604` pick_altitude_tier → `survivor/survivor_data.gd:3458` ENEMY_ALTITUDE_WEIGHTS + `survivor/survivor_data.gd:3503` pick_altitude_tier |
| └ 巡逻高度跟随抽到的档（经 Situation.combat_altitude_m 影响战术层交战高度）| `survivor/survivor_spawner.gd:2615` patrol_altitude_for_tier → `survivor/survivor_data.gd:3492` TIER_PATROL_ALTITUDE + `survivor/survivor_data.gd:3498` patrol_altitude_for_tier |
| AF-03 解锁/概率常量（旅途随机池入口） | `survivor/survivor_data.gd:3250` AF03_UNLOCK_LEVEL |
| AF-03 战区池入口（type 17 行） | `survivor/survivor_data.gd:3681` ZONE_ENEMY_TABLE |
...
| DEADAIR 特殊包（HP55 无武装本体 + 两个动态护卫；与 Snowblind 在场互斥） | `survivor/survivor_spawner.gd` `_support_field_blocked` / `_spawn_deadair_squad` |
| DEADAIR 5Hz 累积 JAM（单位 8s；导弹 4×=2s；离圈宽限/衰减/清弹） | `survivor/deadair_controller.gd` `tick` / `next_unit_exposure` / `next_missile_exposure` / `_tick_units` / `_tick_missiles` |
...
| F-22 队级不同目标四锁 + 0.15s 齐射 + 12s 脱离 | `survivor/f22_multilock.gd` `register` / `allocate_unique_targets` / `tick`；登记机/齐射目标 freed-slot 净化 `_logic_step` / `_update_execution` |
| Gripen C/E 队级三目标；Rafale/F-35 单机双目标 | `survivor/schemer_multilock.gd` `register` / `tick` / `_plan_team_three` / `_plan_per_aircraft`；登记机/齐射目标 freed-slot 净化 `_prune_and_restore` / `_update_queues`；回归 `tests/test_spawn_pool.gd` `_test_multilock_freed_reference_safety` |
...
| 单机生成（J-7 / MiG-31 / F-4E 35%）| `survivor/survivor_spawner.gd:1283` _spawn_single |
| 编队生成（MiG-29 / F-86 / MiG-23 / F-100 / A-7 / Q-5 / MQ-109 / MQ-110 / F-4E）| `survivor/survivor_spawner.gd:1324` _spawn_squad |
| 指挥 UAV 小队生成（Sentinel + MQ-109 僚机）| `survivor/survivor_spawner.gd:1508` _spawn_commander_squad |
...
| 创建敌机实体（参数/AI/缩放/Token meta）| `survivor/survivor_spawner.gd:2315` _create_enemy |
...
| 无效分队清理 | `survivor/survivor_spawner.gd:3775` _cleanup_squads |
...
| 功能 | 位置 |
|------|------|
| 击杀检测 & 经验奖励 & 回血（×xp_mult×sig×机体 xp_gain_mult）| `survivor/survivor_spawner.gd:3536` _detect_kills |
...
| 玩家升级回调（普通三轴 + 可选专属第四槽）| `survivor/survivor_mode.gd:3416` _on_player_leveled_up → `:3576` _append_signature_offer |
| 升级选中回调（选任意卡结算当前专属机会）| `survivor/survivor_mode.gd:3929` _on_upgrade_selected |
| 玩家死亡 | `survivor/survivor_mode.gd:4006` _on_player_died |

#### 4 级金卡软 pity

| 功能 | 位置 |
|------|------|
| 倍率 `1+2m` / 三卡后清零或累加 / 奖励与第四槽隔离 | `survivor/survivor_data.gd:54` CLASSIFIED_PITY_WEIGHT_PER_MISS；`:2642` classified_pity_weight_multiplier；`:2647` classified_pity_next_misses；`:3115` pick_card_for_axis；`survivor/survivor_mode.gd:89` _classified_pity_misses；`:3484` _roll_axis_cards |

#### 生涯档案（spec career-archive，2026-07-26）
| 功能 | 位置 |
|------|------|
| 档案 AutoLoad（schema/记录 API/成就/写盘） | `meta/career_archive.gd` 全文件 |
| 入档守卫（bench/boss_debug 排除） | `survivor/survivor_mode.gd:5098` archive_enabled |
| 空中击坠入档（归因过滤+enemy_type 键） | `survivor/survivor_spawner.gd:3536` _detect_kills（空中/地面两分支各一处 record 调用） |
| 停机计数 | `survivor/survivor_mode.gd:5009` _on_dock_docked 头部 |
| BOSS 接战/击败入档 | `survivor/survivor_mode.gd:5231` on_boss_engaged / `:5302` on_boss_victory |
| BOSS 轮换 + 通关次数 history 注入 | `survivor/survivor_mode.gd:5208` _update_boss_phase → `events/boss_encounter_event.gd` `_start`（spawn 前注入 `defeat_counts`） |
| 轮换算法（纯函数） | `survivor/boss_registry.gd:65` pick_for_map / `:95` pick_by_rotation / `:107` rotation_candidates |
| 结算面板 BOSS 名（"XX 已被击毁"） | `survivor/boss_registry.gd:43` name_key_for → `survivor/survivor_hud.gd:1881` show_victory（boss_id 空 → 通用文案） |
| 忠诚僚机奖池门控（构造时注入；缺键 fail-closed） | `survivor/zone_data.gd:823 _assign_reward`（武器子池过滤）+ `survivor/survivor_mode.gd:5084 _build_reward_roll_context`；回归 `tests/test_zone_rewards.gd:98 _test_achievement_reward_gate` |
| 成就 toast | `survivor/survivor_mode.gd:5103 _on_achievement_unlocked` |
| 删存档登记 | `main_menu.gd:455` `_on_reset_save_pressed` 内 `CareerArchive.debug_reset` |
...
| AWACS 正式局权益消费点 | `survivor/survivor_mode.gd:4602 _update_ally_events` |
| 起手机门控 | `survivor/survivor_select.gd` _effective_list / _unlock_hint_for（boss debug 放行） |
| 停靠送僚机门控 + 首停上新 toast | `survivor/survivor_mode.gd:5009` _on_dock_docked 内 |
...
| 功能 | 位置 |
|------|------|
| ESC 分流（开面板 / 结算态直退 / 选卡中不响应） | `survivor/survivor_mode.gd:1963` _unhandled_input |
| 创建 + 接线（bench 跳过） | `survivor/survivor_mode.gd:496` |
| 确认退出回调（不结算功勋） | `survivor/survivor_mode.gd:4030` _on_pause_quit_to_menu |
| 退出序列（clear_all + stop_music + 切场景） | `survivor/survivor_mode.gd:4037` _quit_to_main_menu |
...
| 应用升级（修改 aircraft.params）| `survivor/survivor_player.gd:439` apply_upgrade |
...
| HP 查询 | `survivor/survivor_player.gd:929` get_hp |
...
| 战区临时支援（已购授权后：每局首次 `air/squadron` 派 2/3/4 架 F-86、首次 `ground` 派 2 架 A-10；两项额度分账；ALLY、短标签、战区 leash、统一物理撤离） | `survivor/zone_mission.gd:1131 _start_air_support_if_needed` / `survivor/zone_mission.gd:1227 _try_spawn_air_support` / `survivor/zone_mission.gd:1300 _create_a10_support` / `survivor/zone_mission.gd:1352 _begin_air_support_egress` / `ai_controller.gd:1079 acquire_target` 的 `air_targets_only` + `ground_targets_only` 门；测试 `tests/test_zone_air_support.gd:33 run` |
| 王牌截击支援（已购授权后：每局首次非 BOSS 王牌轮换派 Hound-1/2 两架 F-15；友军实例雷达 3000m、固定双句无线电，只对空并优先本事件王牌；事件终态物理撤离） | `events/ace_reinforcement_event.gd:186 _spawn_ally_support` / `events/ace_reinforcement_event.gd:308 _maintain_ally_support_targets` / `events/ace_reinforcement_event.gd:345 _begin_ally_support_egress` / `events/ace_reinforcement_event.gd:377 _tick_ally_support_egress`；测试 `tests/test_zone_air_support.gd:33 run` |
| └ 第三方 ALLY 击杀收益隔离（任务销毁照常；不给玩家 XP/击杀数/回血/连击/教程进度） | `survivor/survivor_spawner.gd:3536 _detect_kills` 的 `third_party_kill` |
| └ 三支援最坏压力样本 | `survivor/survivor_mode.gd:1108 _bench_force_zone_support`（`--bench=zone_support_stress`：Lv15 + 31 敌 + Sentinel 完整机群 + 8 F-86 + 2 A-10） |
| └ 对舰最坏压力样本 | `survivor/survivor_mode.gd:1191 _bench_force_naval_zone`（`--bench=naval_zone_stress`：39 架飞机 + 3★ 六舰 + 18 挂点代理，持续覆盖 VLS/CIWS/Flak） |
| └ 王牌截击压力样本 | `survivor/survivor_mode.gd:5407 _bench_force_ace_support`（`--bench=ace_support_stress`：Lv15 + 31 敌 + Sentinel 完整机群 + MARATHON×5 + F-15×2） |
...
| └ 无头回归 | `scripts/tests/test_boss_phase.gd`（bench key `boss_phase`，29 断言）|
...
| CH-47 受击散开（scatter_on_damage meta + flock_members 串联）| `survivor/survivor_spawner.gd:2155` spawn_heli_flee |
...
| 新地图云层 / 沙漠沙尘暴（spec weather-clouds-and-sandstorm） | `weather_system.gd` `sandstorm_speed_kmh` / `sandstorm_progress` / `sandstorm_center_world` / `set_debug_midgame` / `sample_density` / `sample_sandstorm_density` / `sample_obscurant_density` / `_draw_sandstorm` / `_sandstorm_edge_wave` / `_draw_sandstorm_fill` / `_draw_sandstorm_streamlines` / `_draw_sandstorm_observations` / `_draw_sandstorm_front`（官方沙漠 180km/h、10km 宽；气象图式矢量纹样、曲边 Alpha 羽化，不复用云贴图）；`ugc/map_document.gd` cloud schema；`resources/maps/*_preview.aglmap`；`aircraft.gd` `_update_cloud_state`（focused bench 注入同一 WeatherSystem 验真实对象状态）；`survivor_mode.gd` `_cached_is_in_cloud` / `debug_skip_to_midgame`；`missile.gd` 云中累计失导；`missile_manager.gd` `get_in_cloud` / 命中 miss；`survivor_debug_zone.gd` `_on_skip_midgame`；bench `weather` / `map_preview_desert` |
...
| 友军航母（南入北上/甲板 DockPoint/限 2 次/友军专属 300 hull/击沉清零；敌方 CV 资源仍 1200）| `survivor_mode.gd:4719 _summon_reward_carrier` / `survivor_mode.gd:4793 _depart_friendly_carrier` |
| 玩家触发的友军设施区域仇恨（机场 2000px/航母 2500px 激活；1 Hz；H→Q 限额；8s 退出；`SCORED < BOSS < ASSET < DIRECTIVE < COMMANDED`）| `survivor/friendly_asset_aggro.gd:76 tick` + `ai_controller.gd:977 TargetSource` / `ai_controller.gd:1072 get_target_source` / `ai_controller.gd:1079 acquire_target` + `combat_unit.gd:70 META_FRIENDLY_ASSET_GROUP` + `naval/mount_target.gd:38 _ready`；测试 `test_friendly_asset_aggro.gd:10 run` |
...
| 析构时清理 | `survivor/commander_aura.gd:255` _exit_tree |
| 查找 AI 控制器 | `survivor/commander_aura.gd:262` _find_ai |
...
| 玩家 HUD 框板本局首次两闪 | `ui/hud_first_reveal_sequencer.gd` `register_panel` / `register_callback_panel` / `set_panel_available` / `set_panel_sort_position` / `update`；`ui/hud_board_visibility.gd` + `resources/shaders/hud_board_visibility.gdshader` 直接开关复合控件源框板且新区域默认关闭；`survivor/survivor_hud.gd` `_setup_hud_first_reveal` / `_sync_hud_first_reveal_targets` 在首次绘制前预同步；玩家仪表、僚机行、三轴格逐框登记 |
...
| E/G/F/Q/T 键、高度两态与缺失武器跳过 | `survivor/survivor_mode.gd:1963` _unhandled_input / `_cycle_player_altitude_preference` / `_cycle_player_weapon_preference` |
| 王牌中队交战血条（分段命条） | `survivor/survivor_hud.gd:1416` _build_ace_panel |
| Debug 面板文字更新 | `survivor/survivor_hud.gd:1818` _update_debug_panel |
| 游戏结束画面 | `survivor/survivor_hud.gd:1867` show_game_over |
...
| 玩家队反查（扫 `_spawner.get_squads()` 找 leader==玩家）| `survivor/survivor_hud.gd:1592` _get_player_squad |
| 存活僚机列表 | `survivor/survivor_hud.gd:1607` _get_wingmen |
| **玩家队装配 + 登记进 spawner 队表**（唯一入口，幂等）| `survivor/survivor_mode.gd:1704` _ensure_player_squad |
| 起手僚机（`wingman_count>0`，仅 F-14 走）| `survivor/survivor_mode.gd:1743` _spawn_starting_wingmen |
| 懒建队消费方：+1 僚机奖励 / 停靠送僚机 / 双子星克隆 | `survivor/survivor_mode.gd:4872` _claim_wingman_reward |
...
| scope 查询 | `survivor/survivor_data.gd:2506` upgrade_scope |
| 品类数组查询 | `survivor/survivor_data.gd:2571` upgrade_classes |
| "+1 轴进度"目标轴查询 | `survivor/survivor_data.gd:2578` milestone_plus_of |
| 王牌字段型 stat 白名单 | `survivor/survivor_data.gd:2600` ACE_FIELD_STATS |
| 归属生效纯谓词 | `survivor/survivor_data.gd:2614` upgrade_applies_to_machine |
...
| 定向应用（借指针走同 match） | `survivor/survivor_player.gd:393` apply_upgrade_to |
| 王牌剥离（切控迁移逆操作） | `survivor/survivor_player.gd:406` strip_upgrade_from |
...
| 队存活成员枚举 | `survivor/survivor_mode.gd:3617` _squad_members_alive |
| 单机品类身份（meta profile_id） | `survivor/survivor_mode.gd:3630` _class_identity_of |
| 队品类并集（卡池门控） | `survivor/survivor_mode.gd:3639` _squad_present_classes |
| 升级归属分流入口 | `survivor/survivor_mode.gd:3649` _distribute_upgrade |
| "+1 轴进度"发放点 | `survivor/survivor_mode.gd:3728` _grant_milestone_plus |
| 生效子集 meta 重建 | `survivor/survivor_mode.gd:3739` _refresh_squad_effective_stacks |
| 王牌字段技切控迁移 | `survivor/survivor_mode.gd:3766` _migrate_ace_field_upgrades |
| 新僚机入队补挂 build | `survivor/survivor_mode.gd:3782` _apply_build_to_new_member |
...
| 功能 | 位置 |
|------|------|
| 备用弹仓（弹尽概率回满） | `survivor/skill_hooks.gd:349` try_gun_reserve_mag |
| 副武器（装填期免耗弹窗口） | `survivor/skill_hooks.gd:366` in_free_missile_window |
| QAAM 嗜血 / 适应回能（击杀钩子内） | `survivor/skill_hooks.gd:210` 附近 dispatch_on_kill 720 批段 |
| AB 充能静态引用注入 | `survivor/skill_hooks.gd:215` afterburner |
| 僚机阵亡 watcher（0.5s 沿检测） | `survivor/survivor_mode.gd:3808` _tick_squad_watch |
| 复仇之战/刺客复仇/黑匣子分发 | `survivor/survivor_mode.gd:3861` _on_squad_member_down |
| 奖励升级队列/呈现 | `survivor/survivor_mode.gd:3878` _queue_bonus_upgrade |
...
| 阵地转移/保卫阵地/座舱护甲减伤 | `aircraft.gd:3040` _apply_damage 720 批段 |
| 撤离/防守物理注入 | `aircraft/aircraft_physics.gd:473` _g_buff_mult 与 EVAC_SHIFT_SPRINT_BONUS |
...
| 功能 | 位置 |
|------|------|
| R 统一机动（眼镜蛇/J-Turn/胆大妄为） | `aircraft.gd:2397` try_manual_maneuver |
| 胆大妄为动作（i-frame + 滚转 + 投焰） | `aircraft.gd:2428` do_manual_dodge |
| R 键输入入口 | `survivor/survivor_mode.gd:2028` KEY_R 分支 |
...
| 导弹二段推进（续推+渐强） | `missile.gd:80` _second_stage_g_mult 与动力阶段 elif |

### 722 批 机体签名技能（41 机每机一条；spec aircraft-signature-skills）

| 功能 | 位置 |
|------|------|
| 数据表 42 条 sig_* + F-14 围猎特例 | `survivor/survivor_data.gd:1769` sig_f15 起 |
| milestone_plus 数组化 | `survivor/survivor_data.gd:2584` milestone_plus_list_of |
| apply 专用分支（722 段） | `survivor/survivor_player.gd:848` sig_relaxed_stability 起 |
| 技能杂项 tick（CD/VIFFing/近太空/三发推力/超速截击） | `aircraft.gd:1507` _update_sig_skills |
| 超速截击选目标（机头前半球 + 当前雷达锥双硬门） | `aircraft.gd:1566` _sig_mig31_pick_target |
| STEALTH 上升沿装填（先敌开火） | `aircraft.gd:2867` _sig_f22_reload_all |
| 隐身多锁齐射消费点 | `aircraft.gd:2883` effective_max_locks |
| 致死拦截（钛浴缸/复活判序） | `aircraft.gd:3108` _try_sig_death_save |
| 负面状态免疫早退（电战预算） | `combat_unit.gd:162` apply_status（头部 sig_status_immune 早退） |
| 全频段压制流速（被锁敌负面 ×0.6） | `status_effects.gd:113` sig_x13_active + tick 内 x13_suppress |
| 锁定管线集中注入（6 技 + viggen 出锥 grace） | `survivor/survivor_mode.gd:2664` _update_radar_locks（722 段在 in_cone/出锥两分支） |
| 一次性特判（f47/x02/ax00） | `survivor/survivor_mode.gd:3666` _dispatch_sig_oneshot |
| 签名 drone 生成（f47/x90，不进离屏 despawn 体系） | `survivor/survivor_mode.gd:3708` _sig_spawn_loyal_drone |
| 联合突击差量重算 | `survivor/survivor_mode.gd:3837` _update_sig_gcap |
| 奖励僚机生成体（双子星复用；尾部 build 补挂） | `survivor/survivor_mode.gd:4893` _spawn_reward_wingman |
| 鲸群血量均摊 | `survivor/skill_hooks.gd:67` whale_pod_share |
| 作战云广播（中继直通防双乘） | `survivor/skill_hooks.gd:90` broadcast_combat_cloud |
| 三发推力触发（突击命令两入口） | `survivor/skill_hooks.gd:110` try_trigger_j36_assault |
| 特殊机动完成事件（急停/落叶飘） | `survivor/skill_hooks.gd:128` on_special_maneuver_done |
...
| f35 越肩发射豁免 | `aircraft/aircraft_weapons.gd:1058` _sig_f35_relay_ok |
| 夜枭静默弹过滤（规避+投焰单点覆盖） | `ai/missile_evasion.gd:253` sig_silent 过滤 |
| 超越地平重索敌 | `missile.gd:411` _sig_find_retarget |
| 验收 bench（77 断言，含两条新签名技） | `tests/test_sig_skills.gd` `_test_new_signatures`；--bench=sig_skills |
| **43 机映射 + F-14 特例 + sig 判别式** | `survivor/survivor_data.gd` `signature_upgrade_id_for_aircraft` / `is_signature_upgrade` |
| **普通随机池排除统一谓词** | `survivor/survivor_data.gd:2697` is_normal_random_candidate；消费点 `survivor_mode.gd:3460` _roll_upgrade_choices / `survivor_mode.gd:3484` _roll_axis_cards / `zone_data.gd:754` _nextgen_candidates |
| **每机每局第四槽调度** | `survivor/survivor_mode.gd:121` SignatureOfferState 账本 / `:3567` _current_evolution_node_id / `:3576` _append_signature_offer / `:3929` _on_upgrade_selected |
...
| 2/3/4/6/8/10 基准里程碑表 | `survivor/survivor_data.gd:3006` MILESTONE_TABLE（斗士 6 点 `max_g +2.0`） |
| Tab 里程碑数值格式（闪避百分比 / G 一位小数） | `survivor/tactical_map.gd:1484` _fmt_milestone_value |
...
| └ 注入点（注 `_squad_members_alive`） | `survivor/survivor_mode.gd:502` milestone_targets_provider |
...
| └ 新僚机入队调用点 | `survivor/survivor_mode.gd:3801` apply_all_milestones_to |
| 清账重挂（换型；**每机恰好一次**，重复调用会叠两遍） | `survivor/survivor_player.gd:303` reapply_all_milestones_to |
| └ 进化换型调用点（对 `_squad_members_alive()` 逐机） | `survivor/survivor_mode.gd:4448` reapply_all_milestones_to |
...
| 功能 | 位置 |
|------|------|
| 43 节点 / 124 边 / 具体三轴门 SSOT | `resources/evolution/evolution_tree.json` |
| 运行时门判定（仅具体三轴 + F/A-18E any） | `survivor/evolution_system.gd` `gates_passed` `gates_missing` |
| 8 点内全分配枚举、永久不可达边=0 | `tests/test_attribute_gates.gd` `_test_evolution_gates`；bench `attr_gates` |
| 43 节点详情、pip 与 124 条正交树边 | `tests/test_evolution_detail.gd` `_test_gate_pips` `_test_tree_routes_and_current_marker`；bench `evo_detail` |
| 三轴静态成长 + 主导弹/flare 分档与跨 Tier 边审计 | `scripts/tools/audit_evolution_growth.py` `main` `tier_summary` `ordnance_violations` `render_report` |
...
| 2Hz 实验内固定翼/旋翼机重指派 | `survivor/battlefield_atmosphere_experiment.gd:985` _refresh_assignments |
...
| 功能 | 位置 |
|------|------|
| ZoneMission 生成后登记 / 完成失败重刷取消时注销 | `survivor/zone_mission.gd` `_register_zone_atmosphere` / `_retire_zone_atmosphere` / `_retire_all_zone_atmosphere` |
| 空战零增员复用正式敌军；陆战双方固定阵营专用 SPG；海战只补友军 | `survivor/zone_atmosphere_combat.gd` `register_zone` / `_spawn_artillery_group` / `_find_land_artillery_plan` / `artillery_formation_offsets` / `_mark_actor` / `_spawn_allied_naval`；`survivor/atmosphere_artillery_unit.gd` `configure_ellipse`；`combat_unit.gd` `META_FACTION_CONVERSION_LOCKED`；`events/faction_transition.gd` `convert` |
...
| 2Hz、1500/1800px 伤害迟滞、发射快照与轰炸豁免 | `survivor/zone_atmosphere_combat.gd` `update` / `_apply_damage_state`；`combat_unit.gd` `ambient_damage_multiplier` |
| 跨帧演员引用净化（先判对象与实例有效，再做类型判断） | `survivor/zone_atmosphere_combat.gd` `_valid_combat_unit` / `_live_combat_unit`；回归 `tests/test_zone_atmosphere_combat.gd` `_test_live_ally_lookup` |
...
| 广播后 6 秒生成；独立航线目录、安全陆地目标、七线统一 12500px 航程、场外直线入口、75 HP 单弹目标、3×B-1B + 2×F-4E | `survivor/zone_mission.gd` `MISSION_SPAWN_RADIO_LEAD_S` / `_ensure_spawned_for_active_zones` / `BOMBER_ESCORT_ROUTE_CATALOG` / `build_bomber_escort_route` / `_prepare_bomber_escort` / `_start_bomber_escort`；`survivor/survivor_mode.gd` `_on_zone_mission_spawn_announced` |
| 150s、每机一弹、成功优先、全灭/离场失败、在途炸弹窗口与地图只读状态 | `survivor/bomber_mission.gd` `setup` / `_update_outcome` / `_resolve_failure` / `get_live_center` / `get_live_heading` / `get_alive_bomber_count` / `get_alive_escort_count` / `get_phase_key` |
| 纵队槽位与自带护航 | `survivor/survivor_spawner.gd` `spawn_bomber_mission` / `bomber_formation_offset` / `_spawn_bomber_escort_fighters` |
| 地图失败无痕 / OPTIONAL 类别 + 航线 + 高对比固定 TGT/HP 条/右栏状态 + 可钳边移动纵队标记 / 准确 XP 预告 | `survivor/tactical_map.gd` `_should_hide_zone` / `_zone_primary_center` / `_draw_bomber_route_and_target` / `_draw_bomber_escort_marker` / `_refresh_info` / `_zone_id_at` |
| 成功绕过普通奖励只发专属 XP；失败不发收益 | `survivor/survivor_mode.gd` `_on_zone_mission_completed` / `_grant_bomber_escort_xp` / `_on_zone_mission_failed` |
...
| 功能 | 入口 |
|---|---|
| 五份领域源表与新增文本路由 | `i18n/README.md`；`interface.csv` / `gameplay.csv` / `skills.csv` / `meta.csv` / `radio.csv` |
| 分表清单、四列审计、跨表 key 唯一与预期资源路径 | `scripts/i18n_catalog.gd` `csv_paths` / `read_source` / `audit` / `expected_translation_paths` |
| 逐表生成 15 份三语资源 | `scripts/tests/build_translations.gd` `run` / `_build_source`；`bench/run.cmd i18n_build 5 120 Shadow` |
| 运行时注册与 locale 切换 | `project.godot` `[internationalization]`；`scripts/locale_manager.gd` `set_locale_persistent` / `trm` |
| 结构回归门 | `scripts/tests/test_career_archive.gd` `_test_csv_columns`：五表集合、1360 key、跨表唯一、15 资源注册 |
...
| **改台词文案** | `i18n/radio.csv` 的 `RADIO_*` 行（改完需重建翻译资源） |
...
| 按 trigger 选词；仅 `_FMT` key 执行 `%` 格式化，普通句可与格式句共池 | `radio_chatter.gd:233` say |
...
| 世界坐标量化为八方向 i18n key | `radio_chatter.gd:323` direction_key |
...
| 关注点 | 位置 |
|------|------|
| **硬规则**：no_pilot 永不说话 | `aircraft.gd:383` can_speak_on_radio |
| 等级门字段 | `aircraft.gd:379` has_radio_voice |
| 常规敌机赋值（与 no_pilot 同处） | `survivor/survivor_spawner.gd:1796` |
| Mother Goose 蜂群 UAV | `survivor/mother_goose_uav_swarm.gd:250` |
| Mother Goose MQ-X | `survivor/mother_goose_boss.gd:502` no_pilot / `:503` has_radio_voice |
...
| 字段声明 | `survivor/survivor_mode.gd:158` _radio |
| BOSS 登场挑衅 | `events/boss_encounter_event.gd:112`（`_start` 内，紧邻 WARNING 横幅） |
| BOSS 交战 | `survivor/survivor_mode.gd:5231` on_boss_engaged |
| WRAITH 首次真实成员减员 | `survivor/f47_ace_squad.gd:58` _on_member_destroyed（基类钩子 `survivor/ace_squad.gd:302`） |
| CSG 定期 F/A-18 首次成功通报 / 航母沉没 Phase 2 | `survivor/carrier_strike_group.gd:321` _say_periodic_fa18_radio / `:384` _say_phase2_radio |
| Mother Goose MQ-X 实际释放后 Phase 2 | `survivor/mother_goose_boss.gd:448` _spawn_mqx_pair / `:480` _say_phase2_radio |
| 轰炸护送双句 + 实际目标八方向 | `survivor/survivor_mode.gd:4129` _say_bomber_escort_available |
| 海/空战区首次进入（真实敌军 / 真实有人僚机） | `survivor/survivor_mode.gd:4228` _on_zone_mission_triggered；单位查询 `survivor/zone_mission.gd:1445` get_live_hostile_target |
| 地面战完成且气氛友军存活 | `survivor/survivor_mode.gd:4255` _on_zone_mission_completed；快照 `survivor/zone_mission.gd:1452` get_surviving_atmosphere_ally / `survivor/zone_atmosphere_combat.gd:213` first_live_ally |
| 机场解放三选一，代入真实长机呼号 | `survivor/survivor_mode.gd:4677` _liberate_airfield |
| 击坠回报 / 弹射 / 减员计数 | `survivor/survivor_mode.gd:5252` _on_radio_kill_recorded |
| break 规避呼叫（真·躲导弹） | `survivor/survivor_mode.gd:5277` _on_radio_evasion_started |
| 加力冲刺呼叫（玩家主动加力，非躲导弹） | `survivor/survivor_mode.gd:5285` _on_radio_afterburner_engaged |
| 僚机归队 | `survivor/survivor_mode.gd:5294` _on_radio_wingman_joined |
...
| 关注点 | 位置 |
|------|------|
| 无头数据/队列/三语/八方向回归（101 断言） | `scripts/tests/test_radio_chatter.gd`，`--bench=chatter` |
...
| 统一机型尺寸幂律 + 高度倍率 | `aircraft_renderer.gd:152 altitude_base_scale` / `:111 visual_model_scale`；真实尺寸字段 `aircraft_params.gd` |
| 指挥型图标 | `aircraft_renderer.gd:1036` draw_commander_icon |
| 雷达锥绘制 | `aircraft_renderer.gd:371` draw_radar_cone |
| 跨帧玩家机引用净化（HUD / 锁定线 / 绘制消费者） | `aircraft_renderer.gd:24 safe_aircraft_ref` / `:23 safe_player_ref`；`survivor/survivor_hud.gd:20 _safe_player_aircraft`；终局断引用 `survivor/survivor_mode.gd:4006 _on_player_died` |
...
| 数据标签（剥离窗口 stretch 后按真实相机 0.35/0.40 简略档 + Alt 临时完整；当前操控机显示纯机型编号 + 完整呼号，长机名后缀缩首字母） | `aircraft_renderer.gd` `label_lod_scale_for` / `label_lod_scale` / `next_compact_label_state` / `should_draw_compact_label` / `draw_data_label_compact` / `aircraft_model_designation` / `compact_aircraft_name` / `controlled_identity_label`；地面单位 `ground_unit.gd` `_should_draw_compact_data_label` |
| 机头闪光 | `aircraft_renderer.gd:679` draw_muzzle_flash |
| 加力火焰 | `aircraft_renderer.gd:690` draw_afterburner_glow |
| 热诱弹粒子 | `aircraft_renderer.gd:725` draw_flare_particles |
| 目标连线（普通=当前操控机 icon_color；双击突击=独立黄线；单层中细线） | `aircraft_renderer.gd:1986` draw_target_line |
...
| 飞机旁状态栏：FLR 弹尽装填行红/琥珀双色闪烁 | `aircraft_renderer.gd:200` _wpn_color（FLR_RELOAD） |
| 玩家仪表：热诱弹十星投放/短冷却/长装填同步 | `survivor/player_instrument_panel.gd` flare_lit_star_count / _draw_flare_stars |
| 雷达小地图：来袭导弹常亮脉冲标记 + 警示牌/外圈 | `survivor/survivor_hud.gd:1910` RadarDisplay |
| 预测轨迹 | `aircraft_renderer.gd:2095` draw_predicted_path |
| 战术提示弹窗 | `aircraft_renderer.gd:1966` draw_tactic_popup |
| 机炮意图锥（敌方威胁锥 / 友方 hover 参考锥；仅 Mother Goose 蜂群隐藏） | `aircraft_renderer.gd:537` should_show_enemy_gun_threat / `:543` draw_gun_cone |
| └ 威胁锥淡出系数（开火淡出 / 停火淡回） | `aircraft.gd:554` _gun_threat_fade |
| └ 淡出淡回时长常量 | `aircraft.gd:555` GUN_THREAT_FADE_OUT_TIME / `aircraft.gd:556` GUN_THREAT_FADE_IN_TIME |
| └ 每帧推进（威胁条件成立时按 is_firing 增减） | `aircraft.gd:3484` _update_gun_threat_indicator |
| └ 威胁条件中断复位（保留"立即归零防抖"语义） | `aircraft.gd:3510` _reset_gun_threat |
...
| HUD 首次显现（空间顺序、`0.02s` 错峰并发、逐框 `0.50s` 两闪、无黑底源开关、首帧默认关闭） | `ui/hud_first_reveal_sequencer.gd` `HudFirstRevealSequencer`；`ui/hud_board_visibility.gd` `HudBoardVisibility`；`survivor/survivor_hud.gd` `_setup_hud_first_reveal` / `_sync_hud_first_reveal_targets` |
...
| 主菜单 Y2K 军用 CRT 与双栏终端布局 | `main_menu.gd` `_build_terminal_header` / `_build_system_panel` / `_add_mode_button` / `_refresh_terminal_palette`；机壳 `ui/main_menu_crt_shell.gd`；凸面后处理 `ui/main_menu_crt_effect.gd` + `resources/shaders/main_menu_crt.gdshader`；航路示意 `ui/main_menu_scope_display.gd`；可视验收 `tests/main_menu_visual_qa_runner.gd`（bench `main_menu_visual`） |
...
| Game Over 显示 | `survivor/survivor_hud.gd:1867` show_game_over |
...
| 炮艇模式 / 重型机炮 | `survivor/survivor_player.gd` `apply_upgrade`；`aircraft/aircraft_weapons.gd` `auto_gun_scan`（全队独立、最近 Aircraft/GroundUnit、不锁 planner 目标池）、`update_gun`（整梭扫描目标锁存）、`_fire_gun_round`（炮口随射向）、`update_ciws`（独立正面锥）；`aircraft_renderer.gd` `draw_gun_cone` |
...
| 功能 | 位置 |
|---|---|
| 内置地图数据 | `resources/maps/desert_railway_preview.aglmap` / `ocean_islands_preview.aglmap`：60×60 km MapDocument，zones 为空；沙漠仅中央 15 体块油田，海洋仅西北孤岛 16 体块火箭基地与 1 条配套跑道，另定义地理、云、出生点、palette 和各自 basemap 路径 |
| PNG 底图 | 原始 `desert_railway_bg.png/.json` / `ocean_islands_bg.png/.json` 均为 zoom 13、17×17 张 `@2x` 瓦片拼成的 8704² 无标注底图；运行时使用 `*_bg_v2.png/.json`，由 `refine_preview_basemaps.py` 增强沙漠地貌并清理群岛土地覆盖直角色块 |
...
| 主图与 Tab 同源色板 | `survivor/map_feature_renderer.gd` `ugc_palette` `_ugc_color`；`survivor/tactical_map.gd` `ugc_palette` `_map_color`，UGC 模式禁用东京湾 PNG/Aqua-Line |
| 正式底图失败反馈 | `survivor/map_feature_renderer.gd` `basemap_load_failed` / `_report_basemap_error` → `survivor/survivor_mode.gd` `_on_basemap_load_failed` / `_show_basemap_error`；正式图 fail-open 到旧矢量层并显示本地化红色 toast，UGC 纯矢量不误报 |
| PNG 上方机场覆盖 | `survivor/map_feature_renderer.gd` `_draw_ugc_airports` / `survivor/tactical_map.gd` `_draw_ugc_airports`：仅 UGC/内置预览图把 MapDocument 跑道静态绘制在 PNG 上方；Tab 几何按面板尺寸缓存，东京湾不重复描画 |
| 港湾羽田跑道覆盖 | `survivor/map_geography.gd` `HANEDA_RUNWAYS` 存冻结 OSM 四跑道中线/宽度；`survivor/map_feature_renderer.gd` `_draw_haneda_airport` 静态叠在正式 PNG 上方；`survivor/tactical_map.gd` `_rebuild_haneda_runway_cache` / `_draw_haneda_airport` 以独立八端点缓存绘制 Tab，不作用于图2/图3或任务判定 |
...
| 载入与空场探针 | `bench/bench_runner.gd` `PREVIEW_BENCH_MAPS`；`bench/run.cmd map_preview_desert 5 120 Shadow` / `map_preview_ocean` |
...
| 四档静态 triangle-array renderer | `survivor/map_vector_preview_renderer.gd` `setup` `prewarm_lod` `update_lod` `_build_packet_definitions`；v7 主图固定 Operational 单根，Tab 强制 LOD_TAB；自交主水环回退 `_triangulate_polygon` → `Geometry2D.offset_polygon` 拆环 |
...
| PNG/矢量同负载性能门 | `survivor/survivor_mode.gd` `MAP_BENCH_SCENARIOS`；`bench/run.cmd map_png_stress 120 300 Shadow Visual` / `map_vector_stress`；两侧包含共享 `BuildingRenderer`。V44 实测 PNG 117.91 FPS / 最低 38.12 / 3 低帧，矢量 119.33 / 最低 29.87 / 1 低帧；后半程战斗结果不同，只裁定未触发 -15 FPS 硬门 |
| V45 最终生产裁决 | `docs/specs/systems/pure-vector-map-preview.md`：用户整图视觉验收未通过；正式主地图与 Tab 保留 8704×8704 PNG + shader，V44 代码/数据/QA 仅作冻结 debug 研究，未来重启须另立 approved spec |
