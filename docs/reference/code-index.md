...
> ```
> 退出码 1 = 有锚点对不上。写锚点时**带上符号名**（`aircraft/aircraft_physics.gd:345 update_speed`），
...
| 目标航向计算 | `aircraft/aircraft_physics.gd:80` update_target_heading |
| 实飞 / 预测共享目标航向状态 | `aircraft/aircraft_physics.gd` `_target_heading_state` |
| 滚转角更新（G 限制由 tactical_aggression 插值调节）| `aircraft/aircraft_physics.gd` `update_bank` |
| 实飞 / 预测共享滚转公式（小角参考滤波、解析 LOS 前馈、连续 P 死区、方向锁、坡度帽、滚转权限、回正、EMA 积分） | `aircraft/aircraft_physics.gd` `_filter_heading_reference` / `compute_target_bank` / `_resolve_turn_lock_state` / `_bank_limit_rad` / `_bank_roll_rate_rad_s` / `_apply_bank_recovery` / `_integrate_bank_state`；回归 `tests/test_turn_physics.gd`（bench `turn_physics`） |
| 右键急刹拖拽转向（捕获鼠标 / 相对位移、12px 死区 / 110px 满舵、持续 G 坡度、失速禁用、实飞预测同源、机场面板接管释放、速度/射程摇杆、真实机炮射界） | `survivor/survivor_mode.gd` `_begin_hard_brake` / `_update_hard_brake_steer` / `_set_hard_brake` / `_open_evolution_offer` → `aircraft/aircraft_physics.gd` `brake_steer_input_from_dx` / `_brake_steer_target_bank` / `_speed_target_ms`；表现 `survivor/brake_steering_overlay.gd` → `survivor/survivor_hud.gd` `begin_brake_steering` / `set_brake_steering_stall_locked` / `set_brake_steering_flight_data`，飞机前方复用 `aircraft_renderer.gd` `should_show_gun_reference` / `draw_gun_cone`；右键 `SquadCommandController.cancel` 清旧 combat/commanded target，但机炮/炮艇自动扫描继续；spec `systems/brake-steering` |
| 航向更新 ω=g×tan(bank)/speed | `aircraft/aircraft_physics.gd:323` update_heading |
| 实飞 / 预测共享航向积分 | `aircraft/aircraft_physics.gd` `_integrate_heading_rad` |
| 速度更新（含高G阻力、高度耦合） | `aircraft/aircraft_physics.gd:345` update_speed |
| 实飞 / 预测共享速度公式（目标约束 + 积分） | `aircraft/aircraft_physics.gd` `_speed_target_ms` / `_integrate_speed_ms` |
| 高度更新 | `aircraft/aircraft_physics.gd:437` update_altitude |
...
| 失速检查 | `aircraft/aircraft_physics.gd:510` update_stall |
| G力计算 | `aircraft/aircraft_physics.gd:532` update_g_load |
| 飞行员耐力 | **代码已移除**（保留概念：将来拟真战役模式再启用；`i18n` 的 `UPGRADE_PILOT_STAMINA_*` 三语文本刻意保留） |
| 位移应用 | `aircraft/aircraft_physics.gd:565` apply_movement |
| 最大 bank 角（物理瞬时上限） | `aircraft/aircraft_physics.gd:574` max_bank_angle |
| 有效最大G（**buff 注入点**，SEAM-001） | `aircraft/aircraft_physics.gd:619` effective_max_g；瞬时结构 G = `:630` effective_max_g_instant |
...
| 角点速度 V=V_stall×1.2×√G | `aircraft/aircraft_physics.gd:711` corner_speed_kmh |
| 失速速度 V_stall×√G | `aircraft/aircraft_physics.gd:790` stall_speed |
| 高空最大速度衰减 | `aircraft/aircraft_physics.gd:819` max_speed_at_altitude |
| 空气密度比 σ=e^(-alt/8500) | `aircraft/aircraft_physics.gd:841` air_density_ratio |
| 旋翼机速度向量/机头解耦运动 | `aircraft/aircraft_physics.gd:1728 update_rotorcraft` |
| 高度档位切换（生存模式） | `aircraft.gd:1773` set_target_tier |

## 主动特殊机动（R 五选一）

| 功能 | 位置 |
|------|------|
| 共享状态/命中资格 | `aircraft.gd:2439 is_active_special_maneuver` / `aircraft.gd:2443 can_accept_new_hit` |
| 位移滚转激活与安全侧评分 | `aircraft.gd:2537 _try_start_displacement_roll` |
| 垂直越过激活与高度边界 | `aircraft.gd:2562 _try_start_vertical_break` |
| 动作推进与 10Hz AI 自动触发 | `aircraft.gd:2658 _update_active_special_maneuver` |
| 垂直越过俯仰投影（纵轴收缩/尾焰锚点） | `aircraft_renderer.gd:1118 draw_muzzle_flash` / `aircraft_renderer.gd:1133 draw_afterburner_glow` / `aircraft_renderer.gd:1304 draw_aircraft_icon` |
| 五向 R 固定优先级入口 | `aircraft.gd:2678 try_manual_maneuver` |
| 小队共享冷却账本 | `squad.gd:40 active_maneuver_cooldown_s` |
| 玩家 HUD 唯一技能/冷却只读状态 | `aircraft.gd:2456 equipped_r_maneuver_id` / `aircraft.gd:2470 r_maneuver_cooldown_total` / `aircraft.gd:2485 r_maneuver_cooldown_remaining` |
| 常规高度物理让位门 | `aircraft/aircraft_physics.gd:437 update_altitude` |
...
| 功能 | 位置 |
|------|------|
| 能量管理总入口 | `aircraft/aircraft_physics.gd:1040` update_energy_management |
| 加力燃烧器开关 | `aircraft/aircraft_physics.gd:1006` set_afterburner |
| 燃油消耗 | `aircraft/aircraft_physics.gd:1020` update_fuel |
...
| 设定战斗目标 | `aircraft.gd:2134` set_combat_target |
| 清除战斗目标 | `aircraft.gd:2153` clear_combat_target |
| CombatParams 获取 | `aircraft.gd:2212` _combat_params |
...
| 机炮射程（像素） | `aircraft.gd:2241` _gun_range_px |
...
| └ tier 注入点（`_create_enemy` 内按等级 duplicate 挂载）| `survivor/survivor_spawner.gd:4303` ENEMY_ROCKET_TIERS |
| └ 齐射数实际取 `burst_count_max`（`burst_count_min` 目前未被读取）| `scripts/rocket_params.gd` burst_count_max / `aircraft/aircraft_weapons.gd:814` update_rocket |
...
| 功能 | 位置 |
|------|------|
| 武器模式切换（MISSILE/GUN） | `aircraft/aircraft_weapons.gd:950` update_weapon_mode |
| 战术偏好武器模式（带机炮回退） | `aircraft/aircraft_weapons.gd:1003` _update_weapon_mode_tactical |
...
| 导弹阻断日志（被锁/RNG/能量等原因） | `aircraft.gd:2976` _log_msl_block |
| 齐射空手日志（auto_fire 实际值 + 各过滤踢除计数） | `aircraft.gd:2996` _log_salvo_skip |
| 单枚/齐射共享发射提交与一轮 cooldown/reload | `aircraft/aircraft_weapons.gd:1295 _emit_missile` / `aircraft/aircraft_weapons.gd:1327 _finish_main_missile_cycle`；包装入口 `aircraft/aircraft_weapons.gd:1286 _fire_missile_at` |
| 多目标齐射（有效锁数截断 + 正常冷却） | `aircraft/aircraft_weapons.gd:1338 _fire_multi_lock_salvo` · `aircraft/aircraft_weapons.gd` `_salvo_fire_count` |
...
| 最优目标选择（评分） | `aircraft.gd:3060` _select_best_missile_target |
| 射程包线检查 | `aircraft/aircraft_combat_tracking.gd:632` is_in_missile_envelope |
| 导弹阶段判定（接近/照射/保持） | `aircraft.gd:2275` _get_missile_phase |
| 是否应该用机炮 | `aircraft.gd:2302` _should_use_gun |
| Crank 状态查询 | `aircraft.gd:2313` is_cranking |
| 导弹射程（像素） | `aircraft.gd:2247` _missile_range_px |
| 有效导弹射程=min(导弹,雷达)像素 | `aircraft.gd:2257` _effective_missile_range_px |
| 有效射程（像素） | `aircraft.gd:2269` _effective_range_px |
...
| 规避态（底层 evasion_mode：导弹来袭 S 型 + 降高度；AI 自保 + 加力模式共用的底座）| `aircraft.gd:2724` _update_evasion |
| 规避态开关（调用方：AI 自保 enter_evade / 玩家 E 键经 AfterburnerCharge.toggle） | `aircraft.gd:2335` set_evasion_mode |
...
| 加力机炮 100% 闪避（绕 dodge cap 短路） | `aircraft.gd:3282` effective_dodge |
| 加力滚转甩导弹（90% → is_flare_jammed） | `missile_manager.gd:548` AB_MISSILE_DODGE |
| 加力速度地板 + 加速 ×3 | `aircraft/aircraft_physics.gd:741` AB_WINDOW_ACCEL_MULT |
...
| R 统一机动入口（眼镜蛇/J-Turn/胆大妄为） | `aircraft.gd:2678` try_manual_maneuver；`survivor/survivor_mode.gd:3483` KEY_R |
| 当前操控机手动 / AI 僚机自动分流 | `aircraft.gd:1088` is_manual_maneuver_controlled；`:1901` _update_cobra_skill；`:1963` _update_evasion_herbst_skill；`:1990` _update_manual_dodge_skill |
...
| 锁定免疫检查 | `aircraft.gd:3819` is_lock_immune |

## 伤害与击毁

| 功能 | 位置 |
|------|------|
| 受伤（导弹） | `aircraft.gd:3198` take_damage |
| 受伤（机炮，含闪避；返回 bool 表示是否实际结算） | `aircraft.gd:3251` take_bullet_damage |
| 内部伤害应用 | `aircraft.gd:3350` _apply_damage |
| └ 敌方护卫反应（被护送对象挨打 → 护卫同拍脱队扑攻击者） | `aircraft.gd:3515` _alert_escort_guards（`TS_DIRECTIVE` + `clear_formation` + `enter_engage_state`）|
| └ 护卫名单字段 | `aircraft.gd:366` escort_guards（登记方 `survivor/survivor_spawner.gd:2561` _spawn_flee_escort）|
| 地面撞击检查 | `aircraft.gd:3548` _check_ground_crash |
...
| 基类伤害 | `combat_unit.gd:296` take_damage |

## 雷达系统

| 功能 | 位置 |
|------|------|
| 雷达锥判定（飞机） | `aircraft.gd:3613` is_in_radar_cone |
...
| Simple AI 交战 | `ai_controller.gd:1961` _try_engage_simple |
...
| 巡逻逻辑 | `ai_controller.gd:2017` _process_patrol |
| 编队跟随逻辑 | `ai/squad_coordination.gd:28` process_squad_follow |
| └ 协同攻击触发（反应延迟） | `ai_controller.gd:639` 块内 |
| 掩护扫描（队友后方） | `ai/squad_coordination.gd:421` scan_leader_rear |
...
| **BOSS 登场→接战统一契约** | `events/boss_encounter_event.gd` — spawn 后从 `BossRegistry.banner_metadata_for()` 注入身份并播放 `<boss_id>_arrival`；先播系统入侵横幅，Wraith / CSG / Goose 退场后切主体，Black Star 因根机仍在 30km 隐藏而不切固定锚点特写；IRON SERPENT 在 PRE_STAGE 硬暂停下继续沿铁路驶入，横幅退场后镜头跟随第 7 节编组中部，420px 边界 inset 配合 0.26 广角保证首尾与 180 屏幕 px 安全区完整入画，完整入图才接战；镜头回玩家后立即 ENGAGED；缺序列或被 UI 转场覆盖均 fail-open 接战并亮血条；ENGAGED 后猎手持续追玩家（spec ui-transition §2.0 / boss-hunter-doctrine / armored-train §3.4） |
...
| └ 沙漠十四节超尺度武装列车 / 236×84px 重装车体 / 双转向架贴轨 / 铁路广角进场 / 全车可锁定 + 尾节递进打断脱离 / 侧舷锁定后电磁炮 AOE / 逃脱失败 | `resources/maps/desert_railway_preview.aglmap` `railways/iron_serpent_main`（`source=desert_basemap_primary_rail_corridor`，玩家可读金色双线）为路线 SSOT → `survivor/armored_train_unit.gd` `configure_route/arm_segments/begin_arrival_ingress/finish_arrival_ingress/is_arrival_fully_inside/_sample_route_pose/_sample_segment_pose/_refresh_segment_poses/_update_movement/on_segment_broken/railgun_side_for_position/_update_railgun/_begin_railgun_lock/_begin_railgun_charge/_fire_railgun_snapshot` → `survivor/armored_train_segment.gd` `set_train_pose/set_targetable/set_damageable/can_accept_new_hit/take_damage/_start_destroy/_detach_from_train/_draw_theme` → `survivor/armored_train_boss.gd` `train_route/spawn/engage/has_failed/get_hud_entries` → `events/boss_encounter_event.gd` `_spawn_custom/_enter_engaged/_update` → `survivor/survivor_mode.gd` `on_boss_failure`；进场时序在 `resources/presentation/sequences.json` `armored_train_arrival`（0.26 广角跟随第 7 节）；固定池与 Debug 在 `survivor/boss_registry.gd` / `survivor/boss_debug_select.gd`；focused `tests/test_armored_train.gd` / `tests/test_presentation.gd`，Visual `tests/boss_debug_select_visual_qa_runner.gd` |
| └ The Crucible 18 队 73 架接力混战 / Hound 雇佣兵背叛终局 / 玩家附近交战 / FFA 锁定 / 共享 Ace 中队条 | `survivor/the_crucible_boss.gd` `PROFILE_ORDER/CINEMATIC_SQUAD_COUNT/ACTIVE_SQUAD_CAP/OPENING_MELEE_DURATION_S/ENTRY_DISTANCE_PX/ENTRY_DISTANCE_MAX_PX/ENTRY_JOIN_RADIUS_PX/PLAYER_HUNTERS_PER_SQUAD/BACKSTAB_MAX_RANGE_PX/entry_relative_bearing_deg_for/entry_position_for/_entry_spawn_position_for/is_opening_melee_active/aggro_score/rear_attack_opportunity_score/visible_battle_score/spawn/engage/update/_spawn_next_squad/_activate_squad/_retarget_all/_assign_player_hunters/_try_acquire_player/_acquire_target/_is_pressuring_player/_force_reentry_if_strayed/_complete_reentry/_active_squad_count/_target_candidates/_live_engaged_members/get_hud_entries` → `ai_controller.gd` `directive_suspends_combat/prepare_parent_directive_control` → `aircraft/control_intent.gd` `SOURCE_DIRECTIVE` → `survivor/ace_squad_profiles.gd` `hound/boss_grade/crucible_only` → `survivor/ace_squad_theme_controller.gd` `_apply_hound_betrayal` → `survivor/ace_battle_bar.gd` `set_info`；首发为 2NDWAVE / GIMMICK / GOOFIGHTERS，MARATHON 延后到第 6 批；首发三队以 -110°/0°/+110° 为首选方位，在玩家 3600–6000px 外按当前镜头外推/侧搜，前 8s 只互选 Ace，之后每支活跃队仅 1 名稳定猎手压玩家，其余成员继续 FFA，并在 2600px 内从后半球机会主义抢攻正在缠斗玩家的敌方 Ace；每轮每目标最多 1 名背刺认领者；后续槽均铺在排除六点钟 ±45° 的 270° 环带；初次入场进到 3000px 后才启用 3600px 脱战三层环返场；focused `tests/test_the_crucible.gd` + 真实 SceneTree `tests/desert_theater_smoke_runner.gd`；全员 Headless 诊断审计 73 架状态、退场、目标、弹丸与返场 |
| └ 独立陆地航母 / 边界内向心进场 / 嵌入式停机 / 舰载机起飞 | `survivor/land_carrier_unit.gd` `configure_patrol/spawn_parked_aircraft/launch_parked_aircraft/arm_mounts/_draw_ground_icon` → `survivor/land_carrier_boss.gd` `spawn/engage/update/get_hud_entries` → `survivor/boss_registry.gd` / `survivor/boss_debug_select.gd`；甲板驻机由 `aircraft.gd` 在 `parent_carrier` meta 存续期间隐藏世界数据牌，起飞摘 meta 后恢复；focused `tests/test_land_carrier.gd` |
| **沙漠 RTS 地面战线** | `survivor/desert_front_controller.gd` `setup/on_zone_captured/_update_production/_update_helicopter_waves/_update_artillery_fire`；正式入口 `survivor/survivor_map_select.gd`；攻克沿用 `survivor/survivor_mode.gd` `_on_zone_mission_completed`，不再有沙漠专属支线时间改写；全战区气氛 `survivor/zone_atmosphere_combat.gd` `DECISIVE_MAP_IDS` |
...
| └ MQ-X 绕圈根治 + 三路武器 + F-22 级隐形 | `survivor/mother_goose_boss.gd` `configure_mqx_ai`统一注入 joust 550/2600px 攻击跑 → `resources/enemy_uav_mqx.tres` 聚合 `mqx_pulse_cannon.tres` / `uav_mqx_missile.tres` / `mqx_intercept_laser.tres` 与 `sensor_stealth_enabled`；点射枪口闪光由 `aircraft/aircraft_weapons.gd` `_fire_gun_round` 的真实出弹短窗驱动，FFA 也吃敌机一梭门；回归 `tests/test_joust.gd` `_test_mqx_attack_run_and_loadout` + `tests/test_gun_burst.gd`（bench `joust` / `gun_burst`） |
...
| **王牌编成 profile 注册表 / TTK 预算** | `survivor/ace_squad_profiles.gd` AceSquadProfiles — 19 份单点登记（17 支轮换 + ORION 宿敌 + HOUND 熔炉专属）；`pool_at` 排除后两类，`scheduled_wave_count` 给出两个固定槽，`defeat_units` / `estimated_ttk_s` 统一量化。**加新王牌中队 = 本表加一行** |
| └ 非 BOSS 王牌编成与主题（profile 驱动） | `survivor/ace_support_squad.gd` AceSupportSquad — element 解析 + profile 专属机炮/无人机参数资源；`survivor/ace_squad_theme_controller.gd` 中队级 2Hz 主题；`gun_lancer` 给 WhiteTea 配逐机 joust、4×5 受控短梭与单次 flare 后 J-turn |
...
| └ 王牌线框徽章 | `meta/ace_emblem_icon.gd` AceEmblemIcon — 19 份 profile 独立几何图形（含扩编 11 队、ORION 与 HOUND；血条代号旁 + 图鉴页）；静态绘制仅变更时重画 |
...
| 交战主逻辑 | `ai_controller.gd:2071` _process_engage |
...
| 舰载 Flak hover 包线 / 挂点符号 | `scripts/naval/naval_unit.gd:822 _draw_flak_envelopes` / `scripts/naval/naval_unit.gd:936 _draw_alive_mount` |
...
| Aircraft.apply_status 覆写（UAV 滤 FEAR / OVERLOAD 钩子）| `aircraft.gd:3107` apply_status |
...
| 功能 | 位置 |
|------|------|
| 相机跟随插值 | `survivor/survivor_mode.gd:3940` _process |
| 物理主循环（总入口） | `survivor/survivor_mode.gd:3995` _physics_process |
| 选中列表清理 | `survivor/survivor_mode.gd:4291` _cleanup_references |
| 飞机列表同步 | `survivor/survivor_mode.gd:4298` _update_aircraft_list |

#### 雷达锁定
| 功能 | 位置 |
|------|------|
| 全局锁定计算 | `survivor/survivor_mode.gd:4374` _update_radar_locks |
...
| FPS 采样与动态上限调整 | `survivor/survivor_spawner.gd:375` update_fps_sampling |
| 平均 FPS 查询 | `survivor/survivor_spawner.gd:383` _get_avg_fps |
| 屏幕外 AI/物理降频 | `survivor/survivor_mode.gd:4739` _update_offscreen_lod |
...
| 远距清理（释放 Token） | `survivor/survivor_spawner.gd:3660` _update_far_cleanup |

#### 猎手系统
| 功能 | 位置 |
|------|------|
| 猎手指派主循环 | `survivor/survivor_spawner.gd:3762` _update_hunters |
| BOSS 世界边界物理硬护栏（不接管战术/火控） | `survivor/survivor_spawner.gd:3871` enforce_boss_world_boundary |
| 空闲敌机航点围绕玩家 | `survivor/survivor_spawner.gd:3965` _update_enemy_waypoints |
| 获取 AI 控制器 | `survivor/survivor_mode.gd:4884` _get_ai |
| 导弹上限查询（飞向玩家数）| `survivor/survivor_mode.gd:5056` _count_missiles_targeting_player |
| 筛选未发射敌机 | `survivor/survivor_mode.gd:5065` _get_enemies_without_active_missile_at_player |
| 敌人数统计 | `survivor/survivor_spawner.gd:4276` _count_enemies |

#### 刷怪 & Token 烈度控制
| 功能 | 位置 |
|------|------|
| 刷怪主逻辑（每波间隔/FPS闸/Token 预算）| `survivor/survivor_spawner.gd:682` _update_spawner |
| 当前 Token 预算（随等级增长）| `survivor/survivor_spawner.gd:425` _get_token_budget |
| 重算场景 Token 占用 & 每类数量 | `survivor/survivor_spawner.gd:439` _recalc_token_usage |
| 指定类型是否可生成（预算+实例上限）| `survivor/survivor_spawner.gd:451` _can_spawn_type |
| 按等级选敌机类型（概率 + Token 约束）| `survivor/survivor_spawner.gd:477` _pick_enemy_type |
| └ BOSS/事件专属机型排除（后期随机桶，F-47 / F-14 Poltergeist）| `survivor/survivor_spawner.gd:646` BOSS_ONLY_TYPES（表在 `survivor/survivor_data.gd:3515` BOSS_ONLY_TYPES）|
| 敌人作战高度分档（按机型查权重表，替代均匀 1/3 随机）| `survivor/survivor_spawner.gd:3087` pick_altitude_tier → `survivor/survivor_data.gd:3521` ENEMY_ALTITUDE_WEIGHTS + `survivor/survivor_data.gd:3567` pick_altitude_tier |
| └ 巡逻高度跟随抽到的档（经 Situation.combat_altitude_m 影响战术层交战高度）| `survivor/survivor_spawner.gd:3098` patrol_altitude_for_tier → `survivor/survivor_data.gd:3556` TIER_PATROL_ALTITUDE + `survivor/survivor_data.gd:3562` patrol_altitude_for_tier |
...
| 单机生成（J-7 / MiG-31 / F-4E 35%）| `survivor/survivor_spawner.gd:1528` _spawn_single |
| 编队生成（MiG-29 / F-86 / MiG-23 / F-100 / A-7 / Q-5 / MQ-109 / MQ-110 / F-4E）| `survivor/survivor_spawner.gd:1574` _spawn_squad |
| 指挥 UAV 小队生成（Sentinel + MQ-109 僚机）| `survivor/survivor_spawner.gd:1829` _spawn_commander_squad |
...
| 创建敌机实体（参数/AI/缩放/Token meta）| `survivor/survivor_spawner.gd:2792` _create_enemy |
...
| 无效分队清理 | `survivor/survivor_spawner.gd:4307` _cleanup_squads |
...
| 功能 | 位置 |
|------|------|
| 击杀检测 & 经验奖励 & 回血（×xp_mult×sig×机体 xp_gain_mult）| `survivor/survivor_spawner.gd:4063` _detect_kills |
...
| 玩家死亡 | `survivor/survivor_mode.gd:5743` _on_player_died |
...
| 入档守卫（bench/boss_debug 排除） | `survivor/survivor_mode.gd:6982` archive_enabled |
| 空中击坠入档（归因过滤+enemy_type 键） | `survivor/survivor_spawner.gd:4063` _detect_kills（空中/地面两分支各一处 record 调用） |
| 停机计数 | `survivor/survivor_mode.gd:6839` _on_dock_docked 头部 |
| BOSS 接战/击败入档 | `survivor/survivor_mode.gd:7094` on_boss_engaged / `:7166` on_boss_victory |
| BOSS 轮换 + 通关次数 history 注入 | `survivor/survivor_mode.gd:7071` _update_boss_phase → `events/boss_encounter_event.gd` `_start`（spawn 前注入 `defeat_counts`） |
...
| 忠诚僚机奖池门控（构造时注入；缺键 fail-closed） | `survivor/zone_data.gd:861 _assign_reward`（武器子池过滤）+ `survivor/survivor_mode.gd:6968 _build_reward_roll_context`；回归 `tests/test_zone_rewards.gd:189 _test_achievement_reward_gate` |
| 成就 toast | `survivor/survivor_mode.gd:6987 _on_achievement_unlocked` |
...
| AWACS 正式局权益消费点 | `survivor/survivor_mode.gd:6438 _update_ally_events` |
| 起手机门控 | `survivor/survivor_select.gd` `_effective_list` / `_unlock_hint_for`（正常八卡：既有四架沿用原门控，新增四架读取局外采购；Boss Debug 改读 T4 参考节点） |
| 停靠送僚机门控 + 首停上新 toast | `survivor/survivor_mode.gd:6839` _on_dock_docked 内 |
...
| 功能 | 位置 |
|------|------|
| ESC 分流（开面板 / 结算态直退 / 选卡中不响应） | `survivor/survivor_mode.gd:3416` _unhandled_input |
| 创建 + 接线（bench 跳过） | `survivor/survivor_mode.gd:496` |
| 确认退出回调（不结算功勋） | `survivor/survivor_mode.gd:5760` _on_pause_quit_to_menu |
| 退出序列（clear_all + stop_music + 切场景） | `survivor/survivor_mode.gd:5767` _quit_to_main_menu |
...
| 战区临时支援（已购授权后：每局首次 `air/squadron` 派 2/3/4 架 F-86、首次 `ground` 派 2 架 A-10；两项额度分账；ALLY、短标签、战区 leash、统一物理撤离） | `survivor/zone_mission.gd:1913 _start_air_support_if_needed` / `survivor/zone_mission.gd:2009 _try_spawn_air_support` / `survivor/zone_mission.gd:2082 _create_a10_support` / `survivor/zone_mission.gd:2134 _begin_air_support_egress` / `ai_controller.gd:989 acquire_target` 的 `air_targets_only` + `ground_targets_only` 门；测试 `tests/test_zone_air_support.gd:53 run` |
| 王牌截击支援（已购授权后：每局首次非 BOSS 王牌轮换派 Hound-1/2 两架 F-15；友军实例雷达 3000m、固定双句无线电，只对空并优先本事件王牌；事件终态物理撤离） | `events/ace_reinforcement_event.gd:190 _spawn_ally_support` / `events/ace_reinforcement_event.gd:308 _maintain_ally_support_targets` / `events/ace_reinforcement_event.gd:352 _begin_ally_support_egress` / `events/ace_reinforcement_event.gd:381 _tick_ally_support_egress`；测试 `tests/test_zone_air_support.gd:53 run` |
| └ 第三方 ALLY 击杀收益隔离（任务销毁照常；不给玩家 XP/击杀数/回血/连击/教程进度） | `survivor/survivor_spawner.gd:4063 _detect_kills` 的 `third_party_kill` |
...
| └ 对舰最坏压力样本 | `survivor/survivor_mode.gd:2347 _bench_force_naval_zone`（`--bench=naval_zone_stress`：39 架飞机 + 3★ 六舰 + 18 挂点代理，持续覆盖 VLS/CIWS/Flak） |
| └ 王牌截击压力样本 | `survivor/survivor_mode.gd:7281 _bench_force_ace_support`（`--bench=ace_support_stress`：Lv15 + 31 敌 + Sentinel 完整机群 + MARATHON×5 + F-15×2） |
...
| CH-47 受击散开（scatter_on_damage meta + flock_members 串联）| `survivor/survivor_spawner.gd:2625` spawn_heli_flee |
...
| 机场解放战区（海岸线默认 3 机场；其它地图按稳定 id 覆盖本局名称/坐标；敌占→解放→一次性补给点；友军基础 AA×2，授权追加 SAM×1；难度=热度）| `survivor/zone_data.gd`（AIRFIELD_IDS / apply_map_zone_overrides / get_zone_definitions / is_airfield / liberate_airfield / set_airfield_difficulty）+ `survivor_mode.gd`（MapDocument.zones 注入 / _liberate_airfield）+ `zone_mission.gd`（_spawn_airfield_ground / _airfield_difficulty_from_heat）+ `resources/maps/desert_railway_preview.aglmap`（Pilbara 三机场 SSOT）+ `meta/meta_shop.gd`（is_airfield_sam_entitled）|
...
| 王牌中队交战血条（分段命条） | `survivor/ace_battle_bar.gd` `set_info` → `survivor/survivor_hud.gd` `_build_ace_panel/_update_ace_panel` |
| The Crucible 多王牌中队条（最多三条 / 仅已入场存活成员 / 击杀事件即时移除） | 与普通地图精确共用 `survivor/ace_battle_bar.gd` `set_info` → `survivor/survivor_hud.gd` `_set_crucible_ace_entries/_update_boss_panel/_on_kill_recorded` |
...
| 队存活成员枚举 | `survivor/survivor_mode.gd:5355` _squad_members_alive |
| 单机品类身份（meta profile_id） | `survivor/survivor_mode.gd:5368` _class_identity_of |
| 队品类并集（卡池门控） | `survivor/survivor_mode.gd:5377` _squad_present_classes |
| 升级归属分流入口 | `survivor/survivor_mode.gd:5387` _distribute_upgrade |
| 普通技能一次性动作（忠诚僚机立即部署） | `survivor/survivor_mode.gd` _dispatch_regular_oneshot |
| "+1 轴进度"发放点 | `survivor/survivor_mode.gd:5494` _grant_milestone_plus |
| 生效子集 meta 重建 | `survivor/survivor_mode.gd:5505` _refresh_squad_effective_stacks |
| 王牌字段技切控迁移 | `survivor/survivor_mode.gd:5518` _migrate_ace_field_upgrades |
| 新僚机入队补挂 build | `survivor/survivor_mode.gd:5534` _apply_build_to_new_member |
...
| 僚机阵亡 watcher（0.5s 沿检测） | `survivor/survivor_mode.gd:5555` _tick_squad_watch |
| 复仇之战/刺客复仇/黑匣子分发 | `survivor/survivor_mode.gd:5608` _on_squad_member_down |
| 奖励升级队列/呈现 | `survivor/survivor_mode.gd:5625` _queue_bonus_upgrade |
...
| 阵地转移/保卫阵地/座舱护甲减伤 | `aircraft.gd:3350` _apply_damage 720 批段 |
| 撤离/防守物理注入 | `aircraft/aircraft_physics.gd:598` _g_buff_mult 与 EVAC_SHIFT_SPRINT_BONUS |
...
| 功能 | 位置 |
|------|------|
| R 统一机动（眼镜蛇/J-Turn/胆大妄为） | `aircraft.gd:2678` try_manual_maneuver |
| 胆大妄为动作（i-frame + 滚转 + 投焰） | `aircraft.gd:2705` do_manual_dodge |
| R 键输入入口 | `survivor/survivor_mode.gd:3483` KEY_R 分支 |
...
| 技能杂项 tick（CD/VIFFing/近太空/三发推力/超速截击） | `aircraft.gd:1802` _update_sig_skills |
| 超速截击选目标（机头前半球 + 当前雷达锥双硬门） | `aircraft.gd:1863` _sig_mig31_pick_target |
| STEALTH 上升沿装填（先敌开火） | `aircraft.gd:3153` _sig_f22_reload_all |
| 隐身多锁齐射消费点 | `aircraft.gd:3169` effective_max_locks |
| 致死拦截（钛浴缸/复活判序） | `aircraft.gd:3431` _try_sig_death_save |
| 负面状态免疫早退（电战预算） | `combat_unit.gd:175` apply_status（头部 sig_status_immune 早退） |
| 全频段压制流速（被锁敌负面 ×0.6） | `status_effects.gd:113` sig_x13_active + tick 内 x13_suppress |
| 锁定管线集中注入（6 技 + viggen 出锥 grace） | `survivor/survivor_mode.gd:4374` _update_radar_locks（722 段在 in_cone/出锥两分支） |
| 一次性特判（f47/x02/ax00） | `survivor/survivor_mode.gd:5432` _dispatch_sig_oneshot |
| 签名 drone 生成（f47/x90，不进离屏 despawn 体系） | `survivor/survivor_mode.gd:5474` _sig_spawn_loyal_drone |
| 联合突击差量重算 | `survivor/survivor_mode.gd:5584` _update_sig_gcap |
| 奖励僚机生成体（双子星复用；尾部 build 补挂） | `survivor/survivor_mode.gd:6709` _spawn_reward_wingman |
...
| └ 注入点（注 `_squad_members_alive`） | `survivor/survivor_mode.gd:634` milestone_targets_provider |
...
| └ 新僚机入队调用点 | `survivor/survivor_mode.gd:5548` apply_all_milestones_to |
| 清账重挂（换型；**每机恰好一次**，重复调用会叠两遍） | `survivor/survivor_player.gd:303` reapply_all_milestones_to |
| └ 进化换型调用点（对 `_squad_members_alive()` 逐机） | `survivor/survivor_mode.gd:6269` reapply_all_milestones_to |
...
| 关注点 | 位置 |
|------|------|
| **硬规则**：no_pilot 永不说话 | `aircraft.gd:439` can_speak_on_radio |
| 等级门字段 | `aircraft.gd:435` has_radio_voice |
...
| 字段声明 | `survivor/survivor_mode.gd:181` _radio |
...
| BOSS 交战 | `survivor/survivor_mode.gd:7094` on_boss_engaged |
| 击坠回报 / 弹射 / 减员计数 | `survivor/survivor_mode.gd:7116` _on_radio_kill_recorded |
| break 规避呼叫（真·躲导弹） | `survivor/survivor_mode.gd:7141` _on_radio_evasion_started |
| 加力冲刺呼叫（玩家主动加力，非躲导弹） | `survivor/survivor_mode.gd:7149` _on_radio_afterburner_engaged |
| 僚机归队 | `survivor/survivor_mode.gd:7158` _on_radio_wingman_joined |
...
| 关注点 | 位置 |
|------|------|
| 播放入口 | `audio/audio_manager.gd:720` play_radio |
| 音效 id → 路径 | `audio/audio_manager.gd:74` RADIO_FILES |
...
| 四曲随机 one-shot（沙漠可追加 `Ace`）、曲终、Boss/Game Over 守卫、地图歌单断点恢复 | `survivor/survivor_mode.gd` `begin_ace_battle_music` / `_on_music_track_finished` / `end_ace_battle_music` / `_play_default_battle_music`；素材 id/path 在 `audio/audio_manager.gd` `MUSIC_FILES` |
...
| 关注点 | 位置 |
|------|------|
| 自动测试音频隔离 | `bench/bench_runner.gd:212` `_ready`：识别 `--bench` 后 mute Master；`tests/test_deadair.gd` `_test_automation_audio_gate` 运行时断言 |
...
| 沙漠正式入口冒烟 | `scripts/tests/desert_theater_smoke_runner.gd` + `scenes/tests/desert_theater_smoke.tscn`；真实构造 3v3 地面战线、据点产兵、4 架 AH-64，断言支线不改 600 秒时间轴，并逐一解放三座机场生成三个独立补给点；bench `desert_theater` |
...
| 悬停检测 | `camera_controller.gd` `hover_radius_world_for_zoom/update_hover`（main.gd 调用；30 屏幕像素命中区不随 zoom 缩水） |
...
### 隔离三维地图与 Mother Goose 实验

| 功能 | 位置 |
|---|---|
| 实验入口与成对正式负载 | `bench/bench_runner.gd` `VOLUME_SCENARIOS` / `_swap_to_volume_experiment` / `bench_finish`；仅经 `bench/run.cmd`，不修改发布入口 |
| 单视图投影、批绘与终态 | `experiments/volume_world_probe.gd:45 setup` / `:108 _refresh_members` / `:117 world_point` / `:121 camera_basis` / `:126 _sync_camera` / `:144 _process` / `:220 _set_body_override` / `:230 _submit` / `:242 _sync_goose_mounts` / `:258 _capture` / `:267 summary` / `:274 _exit_tree` |
| 可重建几何、静态地图 | `experiments/volume_mesh_factory.gd:9 triangle` / `:18 quad` / `:23 prism` / `:50 box` / `:60 loft` / `:72 mesh` / `:84 material` / `:94 aircraft` / `:147 map_mesh` / `:173 ribbon` / `:181 points_from` |
| 首批专属机型与来源许可 | `experiments/volume_aircraft_catalog.gd` `supported_keys` / `build` / `metadata_for` / `_build_f14` / `_build_a10` / `_build_f47` / `_planform` / `_closed_loft` / `_cap_ring` / `_fin` / `_outward_triangle` |
| 沙漠地图生产网格 | `experiments/volume_map_factory.gd` `build` / `palette` / `face` / `stroke_edges` / `stroke` / `sleeper_poses` / `airport_polygon` / `_add_runway_markings` / `_add_roof`；同源地理与铁路、地表范围、建筑高度和静态预算 |
| 独立观察与模型导出 | `scenes/tests/volume_3d_lab.tscn` → `experiments/volume_3d_lab.gd` `_ready` / `_add_mesh` / `_set_view` / `_set_map_view` / `_set_aircraft_view` / `_set_map_location` / `_unhandled_key_input` / `_capture_sequence` / `_save_view` / `_export_mesh` |
| F6 / 观察场 B 手动战斗 | `scenes/tests/volume_3d_combat.tscn` → `experiments/volume_3d_combat.gd` `_ready` / `_run_volume_smoke`；真实输入与重开退出：`tests/volume_combat_smoke.gd` `run` / `_wait_for_scene_release` / `_key` / `_mouse` / `_check` |
| 验证入口 | `tests/test_volume_world.gd:9 run`；`tests/volume_world_lifecycle_case.gd:15 run` → 默认 `tests/lifecycle_gauntlet_runner.gd` `_run` |
| 首批几何/地图验收 | `tests/test_volume_aircraft_catalog.gd` `run` / `_valid_triangles` / `_symmetric` / `_forward_nose` / `_profile_signature` / `_expect`；`tests/test_volume_map_factory.gd` `run` / `_check`；并入 `volume_world` |

### 正式二维绘制与共用反馈

| 功能 | 位置 |
|------|------|
| 飞机绘制入口 | `aircraft.gd:3633` _draw |
...
| 滚转体积投影（暗色壳层 + 顶面/机腹；PNG 最多两次纹理提交；legacy 不归零） | `aircraft_renderer.gd:102 bank_volume_projection_for` / `:112 bank_volume_face_alpha_for` / `:1304 draw_aircraft_icon` → `aircraft_silhouette_catalog.gd:126 volume_thickness_for` / `:164 draw_icon`；回归 `test_aircraft_bank_volume.gd`；Visual/Perf `aircraft_bank_volume_visual` |
| 统一机型尺寸幂律 + 高度倍率 | `aircraft_renderer.gd:550 altitude_base_scale` / `:557 visual_model_scale`；真实尺寸字段 `aircraft_params.gd` |
| 指挥型图标 | `aircraft_renderer.gd:1561 draw_commander_icon` |
| 雷达锥绘制 | `aircraft_renderer.gd:783 draw_radar_cone` |
...
| 战斗单位统一矢量状态栏（飞机/无人机/地面/SAM/雷达/战略目标/舰船/导弹；共享英文 HDG/SPD/ALT/RNG/HP 数据契约；唯一屏幕空间覆盖层逐帧持有最终变换，单位不重绘也绝不随单位、父节点或镜头转动/缩放；玩家/僚机受伤整块蓝白反相；加力激活飞机显示 `AB`） | `ui/world_unit_status_overlay.gd` `_process` / `_draw` → `aircraft_renderer.gd` `install_status_overlay` / `clear_unit_status_panel` / `status_panel_overlay_transform_for` / `draw_registered_status_panels` / `draw_unit_status_panel` / `status_label_entries` / `status_heading_text` / `status_speed_knots_text` / `status_altitude_text` / `status_range_text` / `status_hp_text` / `english_status_identity`；地面/SAM/雷达内容 `_status_label_lines`，舰船 `_draw_status_label`，导弹 `missile.gd` `_draw_data_label`；格式与屏幕轴断言 `tests/test_presentation.gd`，不主动重绘单位的动态旋转 Visual `unit_status_label_visual` |
...
| 机头闪光 | `aircraft_renderer.gd:1118` draw_muzzle_flash |
| 加力火焰 | `aircraft_renderer.gd:1133` draw_afterburner_glow |
| 热诱弹粒子 | `aircraft_renderer.gd:1168` draw_flare_particles |
| 目标连线（玩家小队统一蓝，与机体 `icon_color/wing_color` 及当前操控权解耦；双击突击=独立黄线） | `aircraft_renderer.gd` `combat_target_line_color` / `draw_target_line` |
...
| 飞机旁状态栏：FLR 弹尽装填行红/琥珀双色闪烁 | `aircraft_renderer.gd:598` _wpn_color（FLR_RELOAD） |
...
| 战术提示弹窗 | `aircraft_renderer.gd:2358 draw_tactic_popup` |
| 悬停雷达/机炮范围与机炮意图锥（任意阵营 hover 参考锥 / 敌方含 FFA 威胁锥 / 当前操控机急刹真实射界；炮艇为 360° 环；仅 Mother Goose 蜂群隐藏） | `aircraft_renderer.gd` `should_show_hover_radar_range` / `should_show_enemy_gun_threat` / `should_show_gun_reference` / `draw_gun_cone` → `aircraft.gd` `_update_gun_threat_indicator` |
...
| └ 每帧推进（威胁条件成立时按 is_firing 增减） | `aircraft.gd:3849` _update_gun_threat_indicator |
| └ 威胁条件中断复位（保留"立即归零防抖"语义） | `aircraft.gd:3879` _reset_gun_threat |
...
- **Encounter Director / 压力账本**：survivor/encounter_director.gd（2Hz Presence/PR、Recovery、Global admission、Lethal Slot；`opening_garrison_size_for` 令 N≥4 的唯一开局驻防稳定为三机完整 Element）→ survivor/survivor_spawner.gd `_try_spawn_global_package`（Package→Element→Squad 原子生成、无 Spawn Debt）/ `_spawn_opening_garrison`；任务/ACE 终态在 survivor_mode / ace_reinforcement_event 转发恢复窗。
- **编队优先攻击**：ai/squad_coordination.gd `process_squad_follow` 保留 `SQUAD_FOLLOW` 并标记 `TACTIC_FORMATION_ASSAULT`；aircraft/aircraft_weapons.gd `update_formation_opportunity_weapons` 在阵位复用严格机炮/导弹许可；普通敌机最终攻击在 `_emit_missile` / `update_gun` 查询 Lethal Slot。
- **主题遭遇包**：survivor/survivor_spawner.gd `_spawn_snowblind_squad/_spawn_deadair_squad` 为支援核心+独立双机 `ESCORT_SCREEN`，`_spawn_commander_squad/_ensure_sentinels_escorted` 固定验证 Sentinel `1+5+1`；focused `spawn_pool` 真实实例化对拍。
