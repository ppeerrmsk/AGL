...
| 垂直越过俯仰投影（纵轴收缩/尾焰锚点） | `aircraft_renderer.gd:571 draw_muzzle_flash` / `aircraft_renderer.gd:582 draw_afterburner_glow` / `aircraft_renderer.gd:742 draw_aircraft_icon` |
...
| R 统一机动入口（眼镜蛇/J-Turn/胆大妄为） | `aircraft.gd:2397` try_manual_maneuver；`survivor/survivor_mode.gd:1998` KEY_R |
...
| 功能 | 位置 |
|------|------|
| 相机跟随插值 | `survivor/survivor_mode.gd:2386` _process |
| 物理主循环（总入口） | `survivor/survivor_mode.gd:2412` _physics_process |
| 选中列表清理 | `survivor/survivor_mode.gd:2575` _cleanup_references |
| 飞机列表同步 | `survivor/survivor_mode.gd:2582` _update_aircraft_list |

#### 雷达锁定
| 功能 | 位置 |
|------|------|
| 全局锁定计算 | `survivor/survivor_mode.gd:2637` _update_radar_locks |
...
| 屏幕外 AI/物理降频 | `survivor/survivor_mode.gd:2969` _update_offscreen_lod |
| 已坠毁敌机清理 | `survivor/survivor_mode.gd:3105` _cleanup_destroyed_enemies |
...
| 获取 AI 控制器 | `survivor/survivor_mode.gd:3112` _get_ai |
| 导弹上限查询（飞向玩家数）| `survivor/survivor_mode.gd:3276` _count_missiles_targeting_player |
| 筛选未发射敌机 | `survivor/survivor_mode.gd:3285` _get_enemies_without_active_missile_at_player |
...
| 玩家升级回调（普通三轴 + 可选专属第四槽）| `survivor/survivor_mode.gd:3389` _on_player_leveled_up → `:3549` _append_signature_offer |
| 升级选中回调（选任意卡结算当前专属机会）| `survivor/survivor_mode.gd:3902` _on_upgrade_selected |
| 玩家死亡 | `survivor/survivor_mode.gd:3986` _on_player_died |

#### 4 级金卡软 pity

| 功能 | 位置 |
|------|------|
| 倍率 `1+2m` / 三卡后清零或累加 / 奖励与第四槽隔离 | `survivor/survivor_data.gd:54` CLASSIFIED_PITY_WEIGHT_PER_MISS；`:2642` classified_pity_weight_multiplier；`:2647` classified_pity_next_misses；`:3115` pick_card_for_axis；`survivor/survivor_mode.gd:89` _classified_pity_misses；`:3457` _roll_axis_cards |

#### 生涯档案（spec career-archive，2026-07-26）
| 功能 | 位置 |
|------|------|
| 档案 AutoLoad（schema/记录 API/成就/写盘） | `meta/career_archive.gd` 全文件 |
| 入档守卫（bench/boss_debug 排除） | `survivor/survivor_mode.gd:5035` archive_enabled |
| 空中击坠入档（归因过滤+enemy_type 键） | `survivor/survivor_spawner.gd:3536` _detect_kills（空中/地面两分支各一处 record 调用） |
| 停机计数 | `survivor/survivor_mode.gd:4946` _on_dock_docked 头部 |
| BOSS 接战/击败入档 | `survivor/survivor_mode.gd:5168` on_boss_engaged / `:5239` on_boss_victory |
...
| 忠诚僚机奖池门控（构造时注入；缺键 fail-closed） | `survivor/zone_data.gd:823 _assign_reward`（武器子池过滤）+ `survivor/survivor_mode.gd:5021 _build_reward_roll_context`；回归 `tests/test_zone_rewards.gd:98 _test_achievement_reward_gate` |
| 成就 toast | `survivor/survivor_mode.gd:5040 _on_achievement_unlocked` |
...
| AWACS 正式局权益消费点 | `survivor/survivor_mode.gd:4543 _update_ally_events` |
| 起手机门控 | `survivor/survivor_select.gd` _effective_list / _unlock_hint_for（boss debug 放行） |
| 停靠送僚机门控 + 首停上新 toast | `survivor/survivor_mode.gd:4946` _on_dock_docked 内 |
...
| 功能 | 位置 |
|------|------|
| ESC 分流（开面板 / 结算态直退 / 选卡中不响应） | `survivor/survivor_mode.gd:1942` _unhandled_input |
| 创建 + 接线（bench 跳过） | `survivor/survivor_mode.gd:496` |
| 确认退出回调（不结算功勋） | `survivor/survivor_mode.gd:4003` _on_pause_quit_to_menu |
| 退出序列（clear_all + stop_music + 切场景） | `survivor/survivor_mode.gd:4010` _quit_to_main_menu |
...
| └ 三支援最坏压力样本 | `survivor/survivor_mode.gd:1087 _bench_force_zone_support`（`--bench=zone_support_stress`：Lv15 + 31 敌 + Sentinel 完整机群 + 8 F-86 + 2 A-10） |
| └ 对舰最坏压力样本 | `survivor/survivor_mode.gd:1170 _bench_force_naval_zone`（`--bench=naval_zone_stress`：39 架飞机 + 3★ 六舰 + 18 挂点代理，持续覆盖 VLS/CIWS/Flak） |
| └ 王牌截击压力样本 | `survivor/survivor_mode.gd:5324 _bench_force_ace_support`（`--bench=ace_support_stress`：Lv15 + 31 敌 + Sentinel 完整机群 + MARATHON×5 + F-15×2） |
...
| 友军航母（南入北上/甲板 DockPoint/限 2 次/友军专属 300 hull/击沉清零；敌方 CV 资源仍 1200）| `survivor_mode.gd:4656 _summon_reward_carrier` / `survivor_mode.gd:4730 _depart_friendly_carrier` |
...
| E/G/F/Q/T 键、高度两态与缺失武器跳过 | `survivor/survivor_mode.gd:1942` _unhandled_input / `_cycle_player_altitude_preference` / `_cycle_player_weapon_preference` |
...
| **玩家队装配 + 登记进 spawner 队表**（唯一入口，幂等）| `survivor/survivor_mode.gd:1683` _ensure_player_squad |
...
| 队存活成员枚举 | `survivor/survivor_mode.gd:3590` _squad_members_alive |
| 单机品类身份（meta profile_id） | `survivor/survivor_mode.gd:3603` _class_identity_of |
| 队品类并集（卡池门控） | `survivor/survivor_mode.gd:3612` _squad_present_classes |
| 升级归属分流入口 | `survivor/survivor_mode.gd:3622` _distribute_upgrade |
| "+1 轴进度"发放点 | `survivor/survivor_mode.gd:3701` _grant_milestone_plus |
| 生效子集 meta 重建 | `survivor/survivor_mode.gd:3712` _refresh_squad_effective_stacks |
| 王牌字段技切控迁移 | `survivor/survivor_mode.gd:3739` _migrate_ace_field_upgrades |
| 新僚机入队补挂 build | `survivor/survivor_mode.gd:3755` _apply_build_to_new_member |
...
| 僚机阵亡 watcher（0.5s 沿检测） | `survivor/survivor_mode.gd:3781` _tick_squad_watch |
| 复仇之战/刺客复仇/黑匣子分发 | `survivor/survivor_mode.gd:3834` _on_squad_member_down |
| 奖励升级队列/呈现 | `survivor/survivor_mode.gd:3851` _queue_bonus_upgrade |
...
| R 键输入入口 | `survivor/survivor_mode.gd:1998` KEY_R 分支 |
...
| 锁定管线集中注入（6 技 + viggen 出锥 grace） | `survivor/survivor_mode.gd:2637` _update_radar_locks（722 段在 in_cone/出锥两分支） |
| 一次性特判（f47/x02/ax00） | `survivor/survivor_mode.gd:3639` _dispatch_sig_oneshot |
| 签名 drone 生成（f47/x90，不进离屏 despawn 体系） | `survivor/survivor_mode.gd:3681` _sig_spawn_loyal_drone |
| 联合突击差量重算 | `survivor/survivor_mode.gd:3791` _update_sig_gcap |
| 奖励僚机生成体（双子星复用；尾部 build 补挂） | `survivor/survivor_mode.gd:4830` _spawn_reward_wingman |
...
| **普通随机池排除统一谓词** | `survivor/survivor_data.gd:2697` is_normal_random_candidate；消费点 `survivor_mode.gd:3433` _roll_upgrade_choices / `survivor_mode.gd:3457` _roll_axis_cards / `zone_data.gd:754` _nextgen_candidates |
| **每机每局第四槽调度** | `survivor/survivor_mode.gd:121` SignatureOfferState 账本 / `:3540` _current_evolution_node_id / `:3549` _append_signature_offer / `:3902` _on_upgrade_selected |
...
| └ 新僚机入队调用点 | `survivor/survivor_mode.gd:3774` apply_all_milestones_to |
| 清账重挂（换型；**每机恰好一次**，重复调用会叠两遍） | `survivor/survivor_player.gd:303` reapply_all_milestones_to |
| └ 进化换型调用点（对 `_squad_members_alive()` 逐机） | `survivor/survivor_mode.gd:4388` reapply_all_milestones_to |
...
| BOSS 交战 | `survivor/survivor_mode.gd:5168` on_boss_engaged |
| 击坠回报 / 弹射 / 减员计数 | `survivor/survivor_mode.gd:5189` _on_radio_kill_recorded |
| break 规避呼叫（真·躲导弹） | `survivor/survivor_mode.gd:5214` _on_radio_evasion_started |
| 加力冲刺呼叫（玩家主动加力，非躲导弹） | `survivor/survivor_mode.gd:5222` _on_radio_afterburner_engaged |
| 僚机归队 | `survivor/survivor_mode.gd:5231` _on_radio_wingman_joined |
...
| 统一机型尺寸幂律 + 高度倍率 | `aircraft_renderer.gd:106 altitude_base_scale` / `:111 visual_model_scale`；真实尺寸字段 `aircraft_params.gd` |
| 指挥型图标 | `aircraft_renderer.gd:930` draw_commander_icon |
| 雷达锥绘制 | `aircraft_renderer.gd:266` draw_radar_cone |
| 跨帧玩家机引用净化（HUD / 锁定线 / 绘制消费者） | `aircraft_renderer.gd:14 safe_aircraft_ref` / `:23 safe_player_ref`；`survivor/survivor_hud.gd:16 _safe_player_aircraft`；终局断引用 `survivor/survivor_mode.gd:3986 _on_player_died` |
...
| 数据标签（真实开局 0.35/0.40 简略档 + Alt 临时完整；当前操控机显示纯机型编号 + 完整呼号，长机名后缀缩首字母） | `aircraft_renderer.gd` `next_compact_label_state` / `should_draw_compact_label` / `draw_data_label_compact` / `aircraft_model_designation` / `compact_aircraft_name` / `controlled_identity_label`；地面单位 `ground_unit.gd` `_should_draw_compact_data_label` |
| 机头闪光 | `aircraft_renderer.gd:571` draw_muzzle_flash |
| 加力火焰 | `aircraft_renderer.gd:582` draw_afterburner_glow |
| 热诱弹粒子 | `aircraft_renderer.gd:617` draw_flare_particles |
| 目标连线（普通=当前操控机 icon_color；双击突击=独立黄线；单层中细线） | `aircraft_renderer.gd:1905` draw_target_line |
...
| 飞机旁状态栏：FLR 弹尽装填行红/琥珀双色闪烁 | `aircraft_renderer.gd:154` _wpn_color（FLR_RELOAD） |
...
| 预测轨迹 | `aircraft_renderer.gd:2014` draw_predicted_path |
| 战术提示弹窗 | `aircraft_renderer.gd:1885` draw_tactic_popup |
| 机炮意图锥（敌方威胁锥 / 友方 hover 参考锥；仅 Mother Goose 蜂群隐藏） | `aircraft_renderer.gd:430` should_show_enemy_gun_threat / `:436` draw_gun_cone |
