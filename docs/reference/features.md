...
- **进化树**：50 节点宝可梦式机型进化（原四张选机卡保留；另有四架局外采购 T0 → 三架低位 T1 / 四架标准 T1 → T5 原创超凡），带 LV + 三轴属性双门槛
- **三轴卡片制**：每升 3 级基础三选一（斗士 / 骑士 / 策士各一张），选卡 = 技能 + 该轴 1 点；商店购买“机体战术适配”后有 15% 概率追加当前机体身份轴的普通第四卡
- **技能系统**：数量以自动生成的 `skill-table.md` 为准；43 条既有机型签名技按原流程取得，七架新增机只显示不可购买、不可授予、无效果的专属槽占位
...
- **EventLogger**：300 秒 O(1) 环形事件队列，F9 导出战斗日志
