...
- **每升 3 级**触发卡片三选一：斗士 / 骑士 / 策士**各一张**（家系池内按稀有度+流派引导抽）。
...
- **激活期全队强 buff**（挂 `afterburner_window_active`，长机 + squad.leader 僚机快照）：
...
- **底层复用**：内部仍走既有 `set_evasion_mode(true)` 链路（planner EVADE、僚机 `escort_cover_active` 护卫广播、§1.2 技能钩子全保留）。AI 自保规避（僚机被咬 / 敌机 EVADE）走**旧弱 buff**（+20% 闪避），**不触发加力强 buff、不消耗能量**。
...
> 键于本模式的技能（全部随激活期存续，按实际烧的时长算，不再是固定 6s）：超频加力 / 蓄势狂暴 / 弹仓过载 / 雾隐机动 / 危机赫尔贝特 / 眼镜蛇机动 / 检讨 / 强化加力 / 适应 / 加力供弹 / su34 鸭嘴兽厨房 / mig31 超速截击 等。
