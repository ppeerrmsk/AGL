...
# 追加（2026-07-05）：MRM 命中率仲裁仿真——"AI 还是导弹性能"定案

用户质疑参数归因（"感觉是瞄准位置不对，导弹追不上前置点"），要求自动测试裁决。
新增 `--bench=missile_env`：真实 missile.gd 物理步进，距离 2/5/8/11km × 尾追直线/
横穿6G × 离轴 0~40°，OLD(fov60/g35) vs NEW(fov90/g45) 双参数集。

## 裁决结果
- **导弹性能无问题（用户正确）**：0° 离轴发射全场景全中（含 11km 尾追、8km 横穿 6G，
  min_dist 17~28m）——速度/过载/PN 都够。
- **真凶 = 发射离轴与导引头视场的边界带**：玩家发射质量门允许 ≤22° 离轴（radar_half
  40°×0.55），OLD FOV60（半角 30°）在 20° 离轴 + 目标 6G 机动叠加时**末段出视场丢锁**
  （min_dist=34m，差 4 米脱靶）——"合法发射窗口内的必丢锁带"。用户观察到的"追不上
  前置点"即此：弹一路在追、临门一脚出 FOV 直线飞走。
- **FOV 60→90 恰好补掉这条带**（20° 离轴 OLD miss → NEW hit）——此前"参数贡献+0"
  是极端矩阵（0°/40°）的误判，中间带实测 +1 且正是实战高发区（crank 后转回/机动中
  发射都落在 10~22° 离轴）。参数上调**保留**，有数据背书。
- 28°/40° 离轴两套参数都丢锁：玩家方被质量门挡住不受影响；敌方无门但用 V 系弹
  （FOV 同 60）→ 敌方命中率天然低 = 难度设计，不动。

## 结论
主责在"AI 发射纪律（离轴门）× 导引头视场"的耦合边界，非单纯弹性能——用户方向正确；
修复落点选在弹侧（FOV90 给合法窗口全覆盖 + 15° 余量），门侧无需再收紧
（22° < 45°×0.7 安全带内）。裁决断言已固化进 missile_env 防回归。
