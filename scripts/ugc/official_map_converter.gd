...
static func build() -> MapDocument:
	...
	# ── 陆地层：OSM mask + 手画平滑地块（is_on_land 的两路合一）──
	# 两路互相重叠 → 做无损布尔并集（面积不变），保证偶奇判定/栅格化自洽。
	# 注意：并集会改写重叠处顶点（gameplay 等价、非逐顶点保留），道路/建筑等仍逐顶点直通。
	var land: Array = []
	for poly in MapGeographyData.LAND_MASK_POLYGONS:
		land.append(poly)
	for poly in MapGeography.get_land_polygons():
		land.append(poly)
	land = ContourBaker.merge_overlapping(land)
	doc.layer_polygons["land"] = land

	# ── 城区层（OSM 块间互叠，同样并集消重叠）──
	doc.layer_polygons["urban"] = ContourBaker.merge_overlapping(MapGeographyData.URBAN_POLYGONS)
