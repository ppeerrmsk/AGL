...
func _ready() -> void:
	...
	# 开局看全图（编辑器缩放范围自管，绕过 CameraController 的战斗用下限）
	_cam_ctrl.target_zoom = 0.065
	_camera.zoom = Vector2(0.065, 0.065)
...
func _zoom(factor: float) -> void:
	_cam_ctrl.target_zoom = clampf(_cam_ctrl.target_zoom * factor, EDITOR_ZOOM_MIN, EDITOR_ZOOM_MAX)
