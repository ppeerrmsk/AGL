class_name Tier3SiegeFlak
extends AirburstAAUnit
...
func _draw() -> void:
	...
	draw_circle(Vector2.ZERO, 8.0, Color(0.22, 0.30, 0.38))
	draw_arc(Vector2.ZERO, 8.0, 0.0, TAU, 16, color, 1.5)
	draw_line(Vector2.ZERO, Vector2(sin(turret_heading - rotation),
		-cos(turret_heading - rotation)) * 15.0, color, 3.0)
