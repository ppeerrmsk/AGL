...
var _building_renderer: BuildingRenderer
var _ugc_doc: MapDocument = null  ## UGC 试飞中的地图文档（null = 官方图）
var _map_preview_only := false     ## 图2/图3空地图试飞：只保留玩家、地图与Tab导航
...
var _brake_steer_active: bool = false
var _brake_steer_anchor_screen: Vector2 = Vector2.ZERO
var _brake_steer_accumulated_dx: float = 0.0
var _brake_mouse_mode_before: int = Input.MOUSE_MODE_VISIBLE
var _brake_mouse_captured: bool = false
...
var _battlefield_flow = BattlefieldFlowScript.new()
## 兼容子系统与 Debug 的公开属性；唯一存储位在 BattlefieldFlow。
var game_time: float:
	get: return _battlefield_flow.game_time
	set(value): _battlefield_flow.set_game_time(value)
...
## ── 阶段制（2026-05-09）──
## 兼容 HUD / Debug / 天气的公开属性；规则层持有可被生涯商店延长的真实时长。
var WARZONE_PHASE_DURATION: float:
	get: return _battlefield_flow.warzone_duration_s
	set(value): _battlefield_flow.set_warzone_duration(value)
...
## 4 级金卡软 pity：只统计自然等级升级产生的本轮普通候选（含适配第四卡）；机场专属技能/奖励升级不读写。
...
## 顶部只能显示一条 persistent；字典保留完整来源状态，未来即使测试 seam 构造多源也不会误清。
var _active_tier3_threat_text_by_zone: Dictionary = {}
...
var _bench_zone_atmosphere_near: bool = false
var _bench_map_prewarming: bool = false
var _bench_wall_started_msec: int = 0
var _bench_last_heartbeat_msec: int = 0
var _bench_pause_started_msec: int = 0
var _bench_render_frames: int = 0
var _bench_render_delta_sum: float = 0.0
var _bench_render_delta_max: float = 0.0
var _bench_render_frames_below_60: int = 0
var _bench_frame_trace_started: bool = false
...
func _exit_tree() -> void:
	...
	# RefCounted/static 局内状态不会随场景树自动终止；退局统一断开跨场景引用。
...
func _ready() -> void:
	...
	# 每次进入都重建局内流程；会话级“连续两局首支王牌不重复”由 BattlefieldFlow 自己保留。
	_battlefield_flow.reset_run()
	# 新局入口不信任上局是否走过正常终态，统一清理跨场景 static 并启用生存模式目标上下文。
	...
	# 逐帧追踪从 3 秒预热后才开启，与正式性能统计窗口完全对齐。
	PerfBuckets.configure_frame_trace(false)
	...
	if get_tree().has_meta("map_preview_only"):
		_map_preview_only = bool(get_tree().get_meta("map_preview_only"))
	...
	if (not _boss_debug_mode or boss_debug_map_render) and (not _bench_mode or bench_map_render):
		...
		if _ugc_doc != null:
			...
			_map_features.ugc_palette = _ugc_doc.style.get("palette", {}).duplicate(true)
	...
	if (not _boss_debug_mode or boss_debug_map_render) and (not _bench_mode or bench_map_render):
		...
		if _ugc_doc != null:
			# 预览图跨度大；压低远距视差，避免远处建筑侧墙被拉成巨型方块。
			_building_renderer.perspective_k = 0.015
		add_child(_building_renderer)
		_building_renderer.setup(camera)
	...
	if _ugc_doc != null and _ugc_doc.spawn.get("pos", []) is Array:
		var spawn_pos: Array = _ugc_doc.spawn.get("pos", [])
		if spawn_pos.size() >= 2:
			player_aircraft.position = Vector2(float(spawn_pos[0]), float(spawn_pos[1]))
	# 相机初始对准玩家起始点 + 启用跟随
	...
	if _ugc_doc != null:
		player_aircraft.heading = deg_to_rad(float(_ugc_doc.spawn.get("heading_deg", 0.0)))
	...
	# T0 比标准起手机多走一次进化：开局按档案结算一项机场等价礼包。
	# 只在 _ready 读取起手机时调用；EvolutionSystem.evolve 不会经过这里，因此同型进化不重复送。
	_grant_starting_benefit(profile)
	...
	# 通用战区系统 + F6 Debug 面板；空地图预览不构造任何战区入口。
	if not _map_preview_only:
	...
	if not _map_preview_only and not _boss_debug_mode and not _bench_mode:
		...
		_zone_mission.mission_spawn_announced.connect(_on_zone_mission_spawn_announced)
	...
	if not _map_preview_only and not _boss_debug_mode and not _bench_mode and SurvivorTutorial.should_show():
...
## pending 分支守住未来渲染时序变化导致 ZoneHint 尚未创建的情况。
...
func _setup_bench_scenario() -> void:
	...
	if not survivor_player or not is_instance_valid(player_aircraft):
		...
		return
	process_mode = Node.PROCESS_MODE_ALWAYS
	if _bench_scenario == "map_vector_detail_stress" and _map_features != null:
		_bench_map_prewarming = true
		var detail_prewarm := MapDetailTileCache.new()
		add_child(detail_prewarm)
		var detail_result: Dictionary = await detail_prewarm.bake_cache()
		if bool(detail_result.get("ok", false)):
			var stress_center := camera.global_position
			detail_result = await detail_prewarm.bake_region(
				Rect2(stress_center - Vector2(4000.0, 3000.0), Vector2(8000.0, 6000.0)))
		detail_prewarm.queue_free()
		camera.zoom = Vector2(0.80, 0.80)
		if not bool(detail_result.get("ok", false)) or not _map_features.attach_cached_detail() \
				or not _map_features.set_vector_preview_enabled(true):
			push_error("[Bench] map_vector_detail_stress could not enable cached detail renderer")
		else:
			_sync_shared_map_gameplay_layers()
	elif _bench_scenario == "map_vector_stress" and _map_features != null:
	...
	if _bench_scenario in MAP_PREVIEW_BENCH_SCENARIOS:
		...
		EventLogger.log_event("BENCH", "MapPreviewLoaded",
			"scenario=%s map=%s land=%d roads=%d zones=%s" % [
				_bench_scenario, _ugc_doc.display_name if _ugc_doc != null else "missing",
				MapGeography.get_land_mask_polygons().size(), MapGeography.HIGHWAYS.size(),
				"none" if _zone_data == null and _zone_mission == null else "unexpected",
			])
		return
	if _bench_scenario == "evolution_growth":
	...
	if _spawner:
		if _bench_scenario == "survivor_death":
		...
		elif _bench_scenario.begins_with("battlefield_atmosphere_stress_") \
				or _bench_scenario == "performance_hud_visual":
			_bench_force_battlefield_atmosphere_stress()
		elif _bench_scenario == "boss_mother_goose":
			_bench_force_spawn_boss_mg()
		elif _bench_scenario == "boss_wraith_stress":
			...
			_bench_force_spawn_boss_hyper_a_arrival()
		...
		elif _bench_scenario == "bomber_escort_zone":
			_bench_force_bomber_escort_zone()
		elif _bench_scenario == "zone_atmosphere_formal":
...
## 真实 5Hz 控制器会同时扫描 CombatUnit 与在途 Missile 注册表；不添加测试专用 process。
func _bench_force_deadair_stress() -> void:
	player_aircraft.global_position = Vector2.ZERO
	_spawner._dynamic_enemy_cap = SurvivorData.MAX_ENEMIES_HARD
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.DEADAIR, 1,
		Vector2(0.0, -800.0), 180.0)
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.MIG23, 2,
		Vector2(-500.0, -950.0), 180.0)
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.F22, 3,
		Vector2(0.0, -2500.0), 180.0)
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.GRIPEN_E, 3,
		Vector2(2600.0, 0.0), 270.0)
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.RAFALE, 2,
		Vector2(-2600.0, 0.0), 90.0)
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.SU57, 2,
		Vector2(0.0, 2600.0), 0.0)
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.J20, 3,
		Vector2(2200.0, -2200.0), 225.0)
	_bench_spawn_enemy_squad_at(SurvivorSpawner.EnemyType.F15_REGULAR, 3,
		Vector2(-2200.0, 2200.0), 45.0)
	for unit in CombatUnit.all_units:
		if unit is Aircraft and is_instance_valid(unit):
			var ac := unit as Aircraft
			ac.invulnerable = true
			ac.set_meta("skip_far_cleanup", true)
	print("[Bench] deadair_stress: 9 direct friendlies vs DEADAIR package + 16 enemies")


## 真实生存场景中的海陆空气氛样本；三个入口均可用 Visual 模式肉眼观察。
func _bench_force_battlefield_atmosphere(kind: String = "air") -> void:
	_bench_set_camera_zoom(0.35)
	...
	_bench_atmosphere_experiment = experiment
...
func _bench_force_battlefield_atmosphere_stress() -> void:
	...
	# 容量测试只保留气氛演员；正式 spawner 自然补刷会让成员数漂移，也可能用导弹一击移除地面演员。
	_spawner._dynamic_enemy_cap = 0
	var config_by_scenario := {
		"performance_hud_visual": Vector2i(36, 8),
		"battlefield_atmosphere_stress_24": Vector2i(24, 8),
		"battlefield_atmosphere_stress_36": Vector2i(36, 8),
		"battlefield_atmosphere_stress_40": Vector2i(40, 8),
		"battlefield_atmosphere_stress_44": Vector2i(44, 8),
		"battlefield_atmosphere_stress_48": Vector2i(48, 8),
		"battlefield_atmosphere_stress_52": Vector2i(52, 8),
		"battlefield_atmosphere_stress_54": Vector2i(54, 8),
		"battlefield_atmosphere_stress_56": Vector2i(56, 8),
		"battlefield_atmosphere_stress_60": Vector2i(60, 8),
		"battlefield_atmosphere_stress_64": Vector2i(64, 8),
		"battlefield_atmosphere_stress_68": Vector2i(68, 8),
		"battlefield_atmosphere_stress_72": Vector2i(72, 8),
		"battlefield_atmosphere_stress_96": Vector2i(96, 8),
		"battlefield_atmosphere_stress_48_24km": Vector2i(48, 24),
		"battlefield_atmosphere_stress_48_48km": Vector2i(48, 48),
	}
	var config: Vector2i = config_by_scenario.get(_bench_scenario, Vector2i(48, 8))
	if _bench_scenario == "performance_hud_visual":
		hud.set_performance_panel_visible(true)
	var span_px := float(config.y) * 500.0
	...
	if bench_player_ai != null:
		bench_player_ai.enable_combat = false
		bench_player_ai.release_target(AIController.TargetSource.TS_SCORED,
			"battlefield atmosphere stress observer")
	_bench_atmosphere_experiment = BattlefieldAtmosphereExperimentScript.new()
	_bench_atmosphere_experiment.name = "BattlefieldAtmosphereStressBench"
	add_child(_bench_atmosphere_experiment)
	_bench_atmosphere_experiment.setup(self, _spawner)
	if not _bench_atmosphere_experiment.launch_stress_battle(config.x, span_px):
		push_error("[Bench] %s failed to launch" % _bench_scenario)
		return
	# 保留场景启动时已存在的少量正式飞机作为固定基线成本，但关闭其武器，避免一击移除地面样本。
	for child in get_children():
		if not (child is Aircraft) or child == player_aircraft \
				or child.is_in_group(&"battlefield_atmosphere_actor"):
			continue
		var baseline_ac := child as Aircraft
		baseline_ac.invulnerable = true
		var baseline_ai: AIController = baseline_ac._get_ai_controller()
		if baseline_ai != null:
			baseline_ai.enable_combat = false
			baseline_ai.release_target(AIController.TargetSource.TS_SCORED,
				"battlefield atmosphere fixed baseline")
	print("[Bench] %s launched members=%d span=%dkm zoom=%.3f" % [
		_bench_scenario, config.x, config.y, fit_zoom])
...
func _bench_force_zone_support() -> void:
	...
	for child in get_children():
		if child is Aircraft and child.has_meta("zone_support"):
			support_count += 1
	# 正式合同：fighter/attack 每种支援每局只派一支；3★ F-86×4 + A-10×2 = 6。
	print("[Bench] zone_support_stress spawned support=%d (expected 6)" % support_count)

## 可失败移动战区的真实场景验证：A 区强制改为护送任务，选择后由场外任务包入场。
func _bench_new_isolated_zone_data() -> ZoneData:
...
func _bench_force_bomber_escort_zone() -> void:
	...
	_zone_data = _bench_new_isolated_zone_data()
	_zone_mission = ZoneMission.new()
	add_child(_zone_mission)
	_zone_mission.setup(self, _zone_data, player_aircraft,
		_sam_scene, _sam_params, _aa_scene, _aa_params,
	...
	_zone_mission.mission_failed.connect(_on_zone_mission_failed)
	_spawner.set_zone_mission(_zone_mission)
	_zone_data.debug_set_available(&"A")
	_zone_data.debug_set_mission_type(&"A", "bomber_escort")
	_zone_mission.debug_force_respawn_zone(&"A")
	var ingress := _zone_data.get_zone_center(&"A")
	var objective := _zone_data.get_objective_center(&"A")
	_zone_data.select_zone(&"A")
	var bench_ai := player_aircraft._get_ai_controller()
	if bench_ai != null:
		bench_ai.enable_combat = false
	print("[Bench] bomber_escort_zone prepared ingress=%s objective=%s outside=%s player_clearance=%.0f" % [
		ingress, objective, str(MapBoundary.distance_to_edge(ingress) < 0.0),
		ingress.distance_to(player_aircraft.global_position)])


## 正式战区氛围层真实场景：A=陆、C=空、E=海，玩家留在远距观察零杀伤持续演出。
## 专项替换 ZoneMission 前必须连同旧控制器持有的演员一起退场；只 free 控制器会留下
## 无主 TGT/驻守，制造正式局不可能出现的重复战区人口并污染性能结论。
func _bench_dispose_live_zone_mission() -> void:
	if _zone_mission == null or not is_instance_valid(_zone_mission):
		return
	var old_mission := _zone_mission
	var owned_units: Dictionary = {}
	for roster_value in old_mission._spawned_zones.values():
		var spawned_roster: Array = roster_value as Array
		for unit_value in spawned_roster:
			if typeof(unit_value) == TYPE_OBJECT and unit_value != null \
					and is_instance_valid(unit_value) and unit_value is CombatUnit:
				owned_units[unit_value] = true
	for roster_value in old_mission._garrison_zones.values():
		var garrison_roster: Array = roster_value as Array
		for unit_value in garrison_roster:
			if typeof(unit_value) == TYPE_OBJECT and unit_value != null \
					and is_instance_valid(unit_value) and unit_value is CombatUnit:
				owned_units[unit_value] = true
	if old_mission._zone_atmosphere != null \
			and is_instance_valid(old_mission._zone_atmosphere):
		for unit in old_mission._zone_atmosphere.retire_all():
			if is_instance_valid(unit):
				owned_units[unit] = true
	for value in owned_units.keys():
		if not is_instance_valid(value):
			continue
		old_mission._retire_tier3_unit(value, true)
		if is_instance_valid(value) and not value.is_queued_for_deletion():
			CombatUnit.release_target_refs(value)
			value.queue_free()
	old_mission._spawned_zones.clear()
	old_mission._garrison_zones.clear()
	old_mission.set_physics_process(false)
	old_mission.queue_free()


func _bench_force_zone_atmosphere() -> void:
	_bench_dispose_live_zone_mission()
	_zone_data = _bench_new_isolated_zone_data()
	_zone_mission = ZoneMission.new()
	add_child(_zone_mission)
	_zone_mission.setup(self, _zone_data, player_aircraft,
	...
	_spawner._spawn_commander_squad(5)
	for config in [[&"A", "ground"], [&"C", "squadron"], [&"E", "naval"]]:
		var zid: StringName = config[0]
		_zone_data.debug_set_available(zid)
		_zone_data.debug_set_mission_type(zid, String(config[1]))
		_zone_mission.debug_force_respawn_zone(zid)
	var atmosphere: Node = _zone_mission._zone_atmosphere
	print("[Bench] zone_atmosphere_formal active=%d ground_actors=%d air_actors=%d naval_actors=%d terrain_violations=%d" % [
		atmosphere.active_zone_count(), atmosphere.actor_count(&"A"),
		atmosphere.actor_count(&"C"), atmosphere.actor_count(&"E"),
		atmosphere.terrain_violation_count()])


## 3★ 四种正式来源的可复现专项：走 ZoneData → ZoneMission 真生成路径并强开气氛层。
## 玩家无敌但不关闭来源武器；因此超级巨炮、VLS、JAM、坦克挂点与地面对打都是真实活跃负载。
func _bench_force_tier3_zone(profile: String) -> void:
	_bench_dispose_live_zone_mission()
	_zone_data = _bench_new_isolated_zone_data()
	_zone_mission = ZoneMission.new()
	add_child(_zone_mission)
	...
	for value in _zone_mission._spawned_zones.get(zone_id, []):
		if value is CombatUnit and is_instance_valid(value) \
				and bool(value.get_meta(&"tier3_threat_source", false)):
			source_count += 1
			source_anchor += (value as CombatUnit).global_position
	if source_count > 0:
		source_anchor /= float(source_count)
	else:
	...
	match profile:
		...
		"siege_tank":
			player_aircraft.global_position = source_anchor + Vector2(1200.0, 0.0)
			_bench_set_camera_zoom(0.45)
		"deadair":
			player_aircraft.global_position = source_anchor + Vector2(900.0, 0.0)
			_bench_set_camera_zoom(0.35)
		"long_range_vls":
	...
	if _bench_duration <= visual_duration_limit:
		...
		camera.global_position = source_anchor.lerp(player_aircraft.global_position, 0.5)
	print("[Bench] tier3_%s zone=%s mission=%s sources=%d targets=%d atmosphere=%d" % [
		profile, zone_id, mission_type, source_count,
		(_zone_mission._spawned_zones.get(zone_id, []) as Array).size(),
		_zone_mission._zone_atmosphere.actor_count(zone_id)])
...
## Wraith 最坏正式层：四机本体 + 首败后两架 YF-23 支援，与 12 架常规友军同窗。
func _bench_force_spawn_boss_wraith_stress() -> void:
	if _spawner == null:
		return
	_spawner._dynamic_enemy_cap = 0
	_bench_set_camera_zoom(CameraController.ZOOM_MIN)
	for squad_idx in range(3):
		var angle := TAU * float(squad_idx) / 3.0
		var leader_pos := player_aircraft.global_position + Vector2.from_angle(angle) * 1800.0
		var heading_deg := rad_to_deg(atan2(-leader_pos.x, leader_pos.y))
		_bench_spawn_friendly_squad_at(
			[SurvivorSpawner.EnemyType.SU27, SurvivorSpawner.EnemyType.MIG31,
			SurvivorSpawner.EnemyType.MIG][squad_idx], 4, leader_pos, heading_deg)
	var encounter := BossRegistry.instantiate("WRAITH_SQUADRON")
	if encounter == null or not (encounter is F47AceSquad):
		push_error("[Bench] WRAITH registry entry missing or wrong type")
		return
	encounter.configure_progression(1)
	_spawner._spawn_boss(encounter, player_aircraft.global_position + Vector2.UP * 2600.0, true)
	encounter.hud_visible = true
	encounter.engage()
	for member in encounter.get_display_members():
		if member is Aircraft:
			(member as Aircraft).invulnerable = true
	print("[Bench] boss_wraith_stress: Wraith/4 + YF-23/2 + 12 friendlies")


## CSG 最坏正式层：首败后 13 舰强化编成、接战 F/A-18，以及真实沉航母后四机
## Poltergeist 弹射。护航舰拉高耐久只为 15s 窗口持续覆盖 VLS/CIWS/Flak。
func _bench_force_spawn_boss_csg_stress() -> void:
	if _spawner == null:
		return
	# 性能 A/B 必须复现同一齐射、Flak 与目标选择时间线。
	seed(8202026)
	_spawner._dynamic_enemy_cap = 0
	_bench_set_camera_zoom(CameraController.ZOOM_MIN)
	for squad_idx in range(3):
		var angle := TAU * float(squad_idx) / 3.0
		var leader_pos := player_aircraft.global_position + Vector2.from_angle(angle) * 1900.0
		var heading_deg := rad_to_deg(atan2(-leader_pos.x, leader_pos.y))
		_bench_spawn_friendly_squad_at(
			[SurvivorSpawner.EnemyType.SU27, SurvivorSpawner.EnemyType.MIG31,
			SurvivorSpawner.EnemyType.F100][squad_idx], 4, leader_pos, heading_deg)
	var encounter := BossRegistry.instantiate("CARRIER_STRIKE_GROUP")
	if encounter == null or not (encounter is CarrierStrikeGroup):
		push_error("[Bench] CSG registry entry missing or wrong type")
		return
	var csg := encounter as CarrierStrikeGroup
	csg.configure_progression(1)
	_spawner._spawn_boss(csg, player_aircraft.global_position + Vector2.UP * 900.0, true)
	csg.hud_visible = true
	csg.engage()
	for ship in csg.get_all_ships():
		...
		if ship != csg._cv:
			...
			for mount in ship.mounts:
				if mount != null:
					mount.hp = 1.0e9
	if csg._cv != null and is_instance_valid(csg._cv):
		csg._cv.take_damage(1.0e9, null, "bench_phase2")
		csg.update(0.0)
	print("[Bench] boss_csg_stress: reinforced 13-ship CSG + F/A-18/2 + Poltergeist/4 + 12 friendlies")


## 直接写 Camera2D.zoom 会在下一帧被 CameraController 的基础 zoom 覆盖；性能 bench 必须
## 同步三份状态，并限制在玩家正式可达的缩放范围内。
func _bench_set_camera_zoom(requested_zoom: float) -> void:
	var stable_zoom := clampf(requested_zoom, CameraController.ZOOM_MIN, CameraController.ZOOM_MAX)
	_camera_ctrl.target_zoom = stable_zoom
	_camera_ctrl._base_zoom = stable_zoom
...
## 正式事件链登场：身份横幅 → 玩家附近真实下降 → 状态栏 / 根机自报。
func _bench_force_spawn_boss_hyper_a_arrival() -> void:
	if _spawner == null or _event_director == null or player_aircraft == null:
		return
	seed(813)
	_spawner._dynamic_enemy_cap = 0
	player_aircraft.invulnerable = true
	player_aircraft.set_meta("skip_far_cleanup", true)
	var event_anchor := player_aircraft.global_position + Vector2.UP * 2200.0
	var event := BossEncounterEvent.new(event_anchor, 0.0, _map_id, "BLACK_STAR", {}, "full")
	_event_director.start(event)
	print("[Bench] boss_hyper_a_arrival: formal banner → player-local descent")
...
## Bench 自动选技能：按等级从稳定池序确定性取一张（剔除 evolved），无视 max_stacks/requires/exclusive。
## 性能 A/B 必须同 build；不得让此前流程消耗全局 RNG 的细微差异改变技能与 draw/弹丸负载。
func _bench_auto_pick_upgrade() -> void:
	...
	if pool.is_empty():
		...
		return
	var pick_index: int = (survivor_player.level * 37 + 11) % pool.size()
	var pick: Dictionary = pool[pick_index]
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo and event.keycode == KEY_R:
		if player_aircraft and is_instance_valid(player_aircraft) and not is_paused_for_upgrade:
			_cancel_afterburner_for_manual_control()
		...
		MOUSE_BUTTON_RIGHT:
			if event.pressed:
				...
				if _command_wheel and _command_wheel.abort_if_pending():
					return
				_on_right_click()
				_begin_hard_brake(event.global_position)
			else:
				_set_hard_brake(false)
		MOUSE_BUTTON_MIDDLE:
...
func _handle_mouse_motion(event: InputEventMouseMotion) -> void:
	if _brake_steer_active:
		_update_hard_brake_steer(event.screen_relative.x)
	if is_dragging:
	...
	# 捕获鼠标期间 global_position 固定在窗口中心，不能拿它误刷战场 hover。
	if not _brake_steer_active:
...
func _on_left_press(screen_pos: Vector2) -> void:
	# 世界点击在按下沿立即终止加力并进入充能；松开后原移动/攻击命令照常执行。
	_cancel_afterburner_for_manual_control()
...
func _execute_left_click(world_pos: Vector2, enemy: CombatUnit, is_double_click: bool) -> void:
	# focused/调试可直接调用本入口，不能依赖一定先经过 _on_left_press。
	_cancel_afterburner_for_manual_control()
...
func _on_wheel_command(context: int, slot_id: String, world_pos: Vector2, target: CombatUnit, option: String) -> void:
	if _wheel_command_steers_aircraft(slot_id):
		_cancel_afterburner_for_manual_control()
	match slot_id:
		# ── 小队命令轮盘：位置命令 ──
		"regroup":
...
static func _wheel_command_steers_aircraft(slot_id: String) -> bool:
	return slot_id in ["regroup", "evac_area", "guard_area", "standoff", "assault"]


func _cancel_afterburner_for_manual_control() -> bool:
	return afterburner_charge != null and afterburner_charge.cancel_for_manual_command()
...
## 持续到松开右键；不拖动时保持航向，左右拖动按各机真实 G/滚转/失速能力请求坡度。
func _begin_hard_brake(screen_pos: Vector2) -> void:
	_brake_steer_active = true
	_brake_steer_anchor_screen = screen_pos
	_brake_steer_accumulated_dx = 0.0
	# 正式场景使用相对输入：系统鼠标锁定/隐藏，虚拟摇杆承担可见反馈。
	# 脱树 focused 夹具不改全局鼠标模式，避免测试进程污染桌面输入。
	if is_inside_tree() and not _brake_mouse_captured:
		_brake_mouse_mode_before = Input.mouse_mode
		_brake_mouse_captured = true
		Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
	_set_hard_brake(true)
	if hud:
		hud.begin_brake_steering(screen_pos)


func _update_hard_brake_steer(relative_dx: float) -> void:
	if not _brake_steer_active:
		return
	_brake_steer_accumulated_dx = clampf(
		_brake_steer_accumulated_dx + relative_dx,
		-AircraftPhysics.BRAKE_STEER_FULL_PX,
		AircraftPhysics.BRAKE_STEER_FULL_PX)
	var steer := AircraftPhysics.brake_steer_input_from_dx(_brake_steer_accumulated_dx)
	if hud:
		hud.update_brake_steering(_brake_steer_anchor_screen, steer)
	for ac in selected_aircraft:
		if is_instance_valid(ac) and not ac.is_destroyed and ac.hard_brake:
			ac.brake_steer_input = steer


func _set_hard_brake(active: bool) -> void:
	if not active:
		_brake_steer_active = false
		_brake_steer_accumulated_dx = 0.0
		if hud:
			hud.end_brake_steering()
		if _brake_mouse_captured:
			var restore_mode := _brake_mouse_mode_before
			_brake_mouse_captured = false
			Input.mouse_mode = restore_mode
			if restore_mode != Input.MOUSE_MODE_CAPTURED:
				Input.warp_mouse(_brake_steer_anchor_screen)
	for ac in selected_aircraft:
		if is_instance_valid(ac) and not ac.is_destroyed:
			ac.hard_brake = active
			ac.brake_steer_input = 0.0
...
func _on_nav_point_selected(world_pos: Vector2) -> void:
	_cancel_afterburner_for_manual_control()
	if _map_features != null and _map_features.vector_preview_enabled:
		await _prewarm_nav_detail(world_pos)
	if _squad_cmd:
		_squad_cmd.command_move(world_pos)


func _prewarm_nav_detail(world_pos: Vector2) -> void:
	if _map_features == null or _tactical_map == null:
		return
	var region := MapDetailTileCache.viewport_region(world_pos, get_viewport_rect().size)
	_tactical_map.set_detail_prepare_in_progress(true)
	EventLogger.log_event("MAP_DETAIL", "NavPrewarmStart", "point=%s region=%s" % [world_pos, region])
	var result: Dictionary = await _map_features.prewarm_detail_regions([region])
	if bool(result.get("ok", false)):
		EventLogger.log_event("MAP_DETAIL", "NavPrewarmDone",
			"point=%s cached=%d baked=%d batch_ms=%d" % [
				world_pos,
				int(result.get("cached_tile_count", 0)),
				int(result.get("last_batch_baked", 0)),
				int(result.get("last_batch_ms", 0)),
			])
	else:
		EventLogger.log_event("MAP_DETAIL", "NavPrewarmFallback",
			"point=%s error=%s" % [world_pos, result.get("error", "unknown")])
	_tactical_map.set_detail_prepare_in_progress(false)
...
func _process(delta: float) -> void:
	var frame_trace_scenario := _bench_tracks_render_performance()
	if frame_trace_scenario and _bench_elapsed >= 3.0 and not _bench_frame_trace_started:
		_bench_frame_trace_started = true
	...
	if frame_trace_scenario and _bench_elapsed >= 3.0:
		_bench_render_frames += 1
		_bench_render_delta_sum += delta
		_bench_render_delta_max = maxf(_bench_render_delta_max, delta)
		if delta > 1.0 / 60.0:
			_bench_render_frames_below_60 += 1
		_bench_stress_frame_deltas.append(delta)
	...
	if is_instance_valid(_tutorial) and (
		...
		_tutorial.notify_pan()
	_cleanup_references()
	# 虚拟摇杆只在右键期间读取当前操控机的失速边界；HUD 内部仅在布尔值变化时重绘。
	if _brake_steer_active and hud:
		var brake_ac := AircraftRenderer.safe_aircraft_ref(player_aircraft)
		var steer_locked := brake_ac != null and (
			brake_ac.is_stalled or brake_ac._stall_recovery_timer > 0.0)
		hud.set_brake_steering_stall_locked(steer_locked)
		if brake_ac != null:
			var gun: GunParams = brake_ac.params.gun if brake_ac.params != null else null
			var gun_range_m := brake_ac.effective_gun_range_m() if gun != null else 0.0
			var gun_max_ammo: int = gun.max_ammo if gun != null else 0
			hud.set_brake_steering_flight_data(brake_ac.speed * 3.6, gun_range_m,
				brake_ac.ammo, gun_max_ammo,
				brake_ac.infinite_ammo or brake_ac.bloodlust_gun_ammo_free())
...
func _physics_process(delta: float) -> void:
	...
	# BattlefieldFlow 独占净时间轴；BOSS/预热冻结与时间修正不再散落写字段。
	var time_before := game_time
	_battlefield_flow.advance_time(delta, _bench_map_prewarming, _zone_data, _boss_spawned)
	if game_time > time_before:
		_update_delayed_zone_unlocks()
	if _weather:
		_weather.set_game_time(game_time, WARZONE_PHASE_DURATION)
	# 雷达锁定累积放在物理帧（固定 60Hz）：失焦/最小化时不会因 _process 大 delta 暴涨。
	# 必须在所有 AI/武器子系统之前跑，因为 update_missile 读 radar_targets 决定开火。
	var perf_root_capture := PerfBuckets.enabled \
		and (PerfBuckets.frame_trace_enabled or PerfBuckets.runtime_panel_enabled)
	var perf_section_t0 := Time.get_ticks_usec() if perf_root_capture else 0
	...
	if perf_root_capture:
		PerfBuckets.tick("survivor_cache", Time.get_ticks_usec() - perf_section_t0)
	...
	# 动态性能控制
	if perf_root_capture:
		perf_section_t0 = Time.get_ticks_usec()
	_spawner.update_fps_sampling(delta)
	_update_offscreen_lod()
	_update_friendly_squad_lod()
	if perf_root_capture:
		PerfBuckets.tick("survivor_lod", Time.get_ticks_usec() - perf_section_t0)
	...
	if not _map_preview_only and not _boss_debug_mode and not _bench_map_prewarming:
		...
		if perf_root_capture:
			PerfBuckets.tick("survivor_spawner", Time.get_ticks_usec() - perf_section_t0)
	...
	if _bench_demo:
		...
		if _bench_scenario == "boss_hyper_a_arrival":
			bench_visual_capture_at = 3.0
		elif _bench_scenario == "boss_hyper_a_dash":
		...
		if _bench_scenario == "zone_atmosphere_formal" and not _bench_zone_atmosphere_near \
				and _bench_elapsed >= _bench_duration * 0.5:
			_bench_zone_atmosphere_near = true
			player_aircraft.global_position = _zone_data.get_zone_center(&"A")
		...
		if _bench_elapsed >= _bench_duration:
			...
			if _bench_tracks_render_performance():
				...
				var p1_fps := average_fps
				if not _bench_stress_frame_deltas.is_empty():
					var sorted_deltas := _bench_stress_frame_deltas.duplicate()
					sorted_deltas.sort()
					var p99_index := mini(int(floor(float(sorted_deltas.size() - 1) * 0.99)),
						sorted_deltas.size() - 1)
					p1_fps = 1.0 / maxf(sorted_deltas[p99_index], 0.0001)
			...
			if _bench_scenario == "battlefield_atmosphere_naval" \
	...
	var warzone_remaining: float = _battlefield_flow.remaining_s()
	hud.set_warzone_remaining(warzone_remaining, _is_in_boss_phase(), game_time)


func _bench_tracks_render_performance() -> bool:
	return _bench_scenario in MAP_BENCH_SCENARIOS \
		or _bench_scenario in COMBAT_PERF_BENCH_SCENARIOS \
		or _bench_scenario == "performance_hud_visual" \
		or _bench_scenario.begins_with("battlefield_atmosphere_stress_")
...
func _update_aircraft_list() -> void:
	...
	_all_aircraft_cache = all_aircraft
	...
	PerfBuckets.set_value("boss.phase", 1 if _is_in_boss_phase() else 0)
...
func _update_radar_locks(delta: float) -> void:
	...
	if _radar_lock_accum < RADAR_LOCK_INTERVAL:
		return
	PerfBuckets.mark_frame_event("radar_tick")
	...
	for unit in all_units:
		...
		for target in unit.radar_targets:
			...
			# AI↔AI 的 radar_targets 仍供武器正常交战，但不写玩家可见/可反应的被锁状态。
...
func _cached_is_in_cloud(unit: CombatUnit) -> bool:
	...
	unit._cloud_cache_result = _weather.is_obscured(pos, unit.altitude) \
		if _weather.has_method("is_obscured") else \
		(unit.get_altitude_tier() == CombatUnit.AltitudeTier.HIGH and _weather.is_in_cloud(pos))
	return unit._cloud_cache_result
...
func _update_offscreen_lod() -> void:
	...
	# 复用本物理帧已经为雷达/武器生成的飞机缓存；禁止再次 get_children()，也不为
	# 每架飞机分配 {ac, ai, dist} 临时 Dictionary。
	var simple_enemies: Array[Aircraft] = []

	for ac in _all_aircraft_cache:
		if not is_instance_valid(ac) or ac == player_aircraft:
			continue
		if ac.is_destroyed:
			continue
		...
		var ai_node: AIController = ac._ai_ref
		...
		ac.visible = not bool(ac.get_meta(&"force_hidden_visual", false)) \
			and not bool(ac.get_meta(Aircraft.META_PRESENTATION_FORCE_HIDDEN_VISUAL, false))
		ac.lod_level = 0
	...
	for i in range(simple_enemies.size()):
		var ac := simple_enemies[i]
		ac.set_physics_process(true)
		var ai_node: AIController = ac._ai_ref
		if ai_node == null:
			continue
		ai_node.set_physics_process(true)
		ai_node.ai_tick_divisor = 3 if i < SIMPLE_AI_FULL_TICK_BUDGET else 6
...
func _update_friendly_squad_lod() -> void:
	...
	player_aircraft.visible = not bool(player_aircraft.get_meta(
		Aircraft.META_PRESENTATION_FORCE_HIDDEN_VISUAL, false))

	var cam_pos := camera.global_position
	var vp_size := get_viewport_rect().size / camera.zoom
	var half := vp_size / 2.0 + Vector2(OFFSCREEN_MARGIN, OFFSCREEN_MARGIN)

	# 与敌方 LOD 共用同帧飞机缓存，避免第二次遍历 survivor_mode 场景树。
	for ac in _all_aircraft_cache:
...
func _on_player_leveled_up(_new_level: int) -> void:
	...
	# 基础三轴卡（spec evolution-attribute-gates §2.2）+ 稀有机体适配第四卡：
	# 每 3 级触发；选任一卡 = 得技能 + 该卡所属轴 +1 属性点。
...
## 从升级池按现行规则（专属排除/doctrine 门控/稀有度分槽/pity）抽三选一。
...
func _roll_upgrade_choices() -> Array[Dictionary]:
	...
	var available: Array[Dictionary] = SurvivorSkillCatalogScript.normal_candidates(
		SurvivorData.UPGRADES, _player_profile_id, p, upgrade_stacks,
		_squad_present_classes(), Callable(MetaShop, "is_upgrade_gated"))
...
## 每 3 级基础三轴各抽一张；自然升级可由机体战术适配权益稀有追加第四张普通轴卡。
...
func _roll_axis_cards(apply_classified_pity: bool = false) -> Array[Dictionary]:
	...
	var available: Array[Dictionary] = SurvivorSkillCatalogScript.normal_candidates(
		SurvivorData.UPGRADES, _player_profile_id, p, upgrade_stacks,
		_squad_present_classes(), Callable(MetaShop, "is_upgrade_gated"))
	var by_axis: Dictionary = SurvivorSkillCatalogScript.candidates_by_axis(available)
	...
	for a in SurvivorData.AXES:
		...
		cards.append(c)
	# 机体战术适配只接在自然升级后：第四张仍走普通池与同一权重器，绝不回流专属技能。
	if apply_classified_pity and MetaShop.is_airframe_affinity_entitled(archive_enabled()):
		var offer_roll := randf()
		if SurvivorData.airframe_affinity_offer_hit(offer_roll):
			var identity: Array = _class_identity_of(player_aircraft)
			var bonus_axis := SurvivorData.airframe_affinity_axis(identity, randf())
			var bonus_pool: Array = []
			if bonus_axis != &"":
				bonus_pool = SurvivorData.airframe_affinity_candidates(
					by_axis[bonus_axis] as Array, cards, service_tag)
			var bonus_debt_mult: float = SurvivorData.terminal_debt_weight(service_misses) \
				if service_tag != "" and not force_terminal else 1.0
			var bonus_card := SurvivorData.pick_card_for_axis(
				bonus_pool, upgrade_stacks, lvl, classified_mult,
				focus_tag, int(build_affinity.get(focus_tag, 0)), service_tag, bonus_debt_mult)
			if not bonus_card.is_empty():
				bonus_card = bonus_card.duplicate(true)
				bonus_card["airframe_bonus_offer"] = true
				bonus_card["airframe_bonus_axis"] = String(bonus_axis)
				cards.append(bonus_card)
				EventLogger.log_event("UPGRADE_AIRFRAME_AFFINITY", "Player",
					"roll=%.4f axis=%s card=%s" % [
						offer_roll, String(bonus_axis), str(bonus_card.get("id", ""))])
			else:
				EventLogger.log_event("UPGRADE_AIRFRAME_AFFINITY", "Player",
					"roll=%.4f axis=%s empty_pool" % [offer_roll, String(bonus_axis)])
	if service_tag != "":
...
## HUD 只在“真有可进化出口且有可用停靠点”时亮提示，避免玩家去机场扑空。
func has_ready_aircraft_evolution() -> bool:
	if survivor_player == null:
		return false
	var has_active_dock := false
	for dock in _dock_points:
		if is_instance_valid(dock) and not dock.is_spent():
			has_active_dock = true
			break
	if not has_active_dock:
		return false
	var current_id := _current_evolution_node_id()
	return current_id != &"" and EvolutionSystem.has_available_exit(
		current_id, survivor_player.level, survivor_player.axis_points)
...
func _refresh_squad_effective_stacks() -> void:
	for m in _squad_members_alive():
		var eff: Dictionary = SurvivorSkillCatalogScript.effective_stacks_for_machine(
			SurvivorData.UPGRADES, upgrade_stacks, _class_identity_of(m),
			m == player_aircraft)
		m.set_meta("upgrade_stacks", eff)
		SurvivorData.recompute_category_bonuses(m, eff)
		SkillHooks.sync_ghost_buster_team_hp_bonus(m)
	SurvivorSkillRuntimeScript.sync_team_state(upgrade_stacks, afterburner_charge)
...
func _apply_build_to_new_member(ac: Aircraft) -> void:
	if survivor_player == null or ac == null or not is_instance_valid(ac):
		return
	var replay_layers: Array[Dictionary] = SurvivorSkillCatalogScript.replay_layers_for_machine(
		SurvivorData.UPGRADES, upgrade_stacks, _class_identity_of(ac),
		ac == player_aircraft)
	for layer in replay_layers:
		for i in int(layer["stacks"]):
			survivor_player.apply_upgrade_to(ac, layer["upgrade"])
	# squad_once 的逐机成分：数据链的僚机雷达加成（T0 排查缺口②——晚入队补挂）
	if int(upgrade_stacks.get("data_link", 0)) > 0 and ac != player_aircraft and ac.params:
		var data_link: Dictionary = SurvivorData.upgrade_by_id("data_link")
		ac.params.radar_range *= (1.0 + float(data_link.get("value", 0.0)))
...
## 应用一条升级（归属分流 + 记栈 + 技能自带 milestone_plus + 生效子集重建）。
## 本函数刻意不发普通选卡的 +1 轴点：等级卡由 _on_upgrade_selected 另行发放，机场专属技能不发。
## 返回值保留给等级卡与机场留机两个入口的既有调用契约；技能进化链已退役，恒为 ""。
...
func _replay_player_upgrades() -> void:
	...
	for m in _squad_members_alive():
		...
		if fa18_hp > 0.0 and m.params:
			...
			m.hp += fa18_hp
	var replayed: int = 0
	var replay_layers: Array[Dictionary] = SurvivorSkillCatalogScript.owned_replay_layers(
		SurvivorData.UPGRADES, upgrade_stacks)
	for layer in replay_layers:
		var stacks: int = int(layer["stacks"])
		for i in stacks:
			_distribute_upgrade(layer["upgrade"])
		replayed += stacks
	_refresh_squad_effective_stacks()
	if replayed > 0:
...
func _on_player_died() -> void:
	...
	# 终局立即切断跨帧玩家机缓存；飞机稍后 free 时 HUD/敌方锁定线不得再接触旧实例。
	survivor_player.aircraft = null
...
func _on_supply_confirmed() -> void:
	...
	var time_before := game_time
	_battlefield_flow.apply_time_cost(SUPPLY_TIME_COST)
	# 机头转向地图中心，玩家自然飞回内部
	_turn_player_inward()
...
## 正式局节奏门：先让玩家完成一轮战斗/升级，再开放奖励目标与可选护送任务。
func _update_delayed_zone_unlocks() -> void:
	if _zone_data == null or _battlefield_flow.phase != BattlefieldFlowScript.Phase.WARZONE \
			or survivor_player == null:
		return
	var level := survivor_player.level
	var released_initial_this_tick := false
	if not _zone_data.initial_reward_zones_released() \
			and ZoneData.initial_reward_unlock_ready(game_time, level):
		if _zone_data.release_initial_reward_zones():
			released_initial_this_tick = true
			if _zone_hint:
				_zone_hint.show_persistent(tr("ZONE_HINT_NEW_OPENED"))
			if _tactical_map:
				_tactical_map.refresh_zone_state()
			EventLogger.log_event("ZONE", "InitialRewardsReleased",
				"time=%.1f level=%d" % [game_time, level])
	if not released_initial_this_tick and _zone_data.initial_reward_zones_released() \
			and ZoneData.optional_mission_unlock_ready(game_time, level) \
			and _zone_data.get_optional_mission_assigned_count() == 0:
		var optional_id := _zone_data.release_first_optional_mission()
		if optional_id != &"":
			if _zone_hint:
				_zone_hint.show_persistent(tr("ZONE_HINT_NEW_OPENED"))
			if _tactical_map:
...
func _on_zone_selected(zone_id: StringName) -> void:
	...
	var cruise_edge := _zone_cruise_edge(zone_id)
	if _map_features != null and _map_features.vector_preview_enabled:
		await _prewarm_zone_detail(zone_id, cruise_edge)
	# RTS 巡航：把小队目标设到战区圆的"近边缘点"，飞过去即进圈触发 ZoneMission
	if _squad_cmd and cruise_edge != Vector2.INF:
		_squad_cmd.command_move(cruise_edge)


func _prewarm_zone_detail(zone_id: StringName, cruise_edge: Vector2) -> void:
	if _zone_data == null or _map_features == null or _tactical_map == null:
		return
	var zone: Dictionary = _zone_data.get_zone_by_id(zone_id)
	if zone.is_empty():
		return
	var center: Vector2 = _zone_data.get_objective_center(zone_id) \
		if _zone_data.get_mission_type(zone_id) == "bomber_escort" \
		else _zone_data.get_zone_center(zone_id)
	var radius := _zone_data.get_zone_radius(zone_id)
	if center == Vector2.INF or radius <= 0.0 or cruise_edge == Vector2.INF:
		return
	var regions := _detail_path_view_regions(cruise_edge, center, get_viewport_rect().size)
	_tactical_map.set_detail_prepare_in_progress(true)
	EventLogger.log_event("MAP_DETAIL", "ZonePrewarmStart",
		"zone=%s views=%d" % [zone_id, regions.size()])
	var result: Dictionary = await _map_features.prewarm_detail_regions(regions)
	if bool(result.get("ok", false)):
		EventLogger.log_event("MAP_DETAIL", "ZonePrewarmDone",
			"zone=%s cached=%d baked=%d batch_ms=%d" % [
				zone_id,
				int(result.get("cached_tile_count", 0)),
				int(result.get("last_batch_baked", 0)),
				int(result.get("last_batch_ms", 0)),
		...
		EventLogger.log_event("MAP_DETAIL", "ZonePrewarmFallback",
			"zone=%s error=%s" % [zone_id, result.get("error", "unknown")])
	_tactical_map.set_detail_prepare_in_progress(false)

## 计算战区圆边缘上离玩家最近的抵达点；预热与巡航必须复用同一个结果。
func _zone_cruise_edge(zone_id: StringName) -> Vector2:
	if not _zone_data or not player_aircraft or not is_instance_valid(player_aircraft):
		return Vector2.INF
	var z := _zone_data.get_zone_by_id(zone_id)
	if z.is_empty():
		return Vector2.INF
	var center: Vector2 = _zone_data.get_zone_center(zone_id)
	var radius: float = _zone_data.get_zone_radius(zone_id)
	if center == Vector2.INF or radius <= 0.0:
		return Vector2.INF
	if _zone_data.get_mission_type(zone_id) == "bomber_escort":
		# 编队真实位置可以在地图外；自动巡航只送玩家到边界内会合点，避免触发出界补给。
		return MapBoundary.clamp_inside(center, 600.0)
	var dir := (player_aircraft.global_position - center)
	return center + (dir.normalized() if dir.length() > 1.0 else Vector2.UP) * radius


static func _detail_corridor_region(center: Vector2, radius: float, cruise_edge: Vector2) -> Rect2:
	if center == Vector2.INF or cruise_edge == Vector2.INF or radius <= 0.0:
		return Rect2()
	var corridor_center := (center + cruise_edge) * 0.5
	var half_extent := radius * 0.5 + 600.0
	var extent := Vector2.ONE * half_extent
	return Rect2(corridor_center - extent, extent * 2.0)


## V37：按 Detail 完全显现 zoom 的真实视口覆盖抵达点、路径中点与圆心。
## 三个矩形的非空瓦片并集必须整体装进 12 格 LRU，否则完整回退 Operational。
static func _detail_path_view_regions(start: Vector2, finish: Vector2,
		viewport_pixels: Vector2) -> Array:
	var regions: Array = []
	for center in [start, start.lerp(finish, 0.5), finish]:
		regions.append(MapDetailTileCache.viewport_region(center, viewport_pixels))
	return regions
...
func _tier3_profile_name(profile: StringName) -> String:
	match profile:
		&"super_cannon": return tr("TIER3_PROFILE_SUPER_CANNON")
		&"siege_tank": return tr("TIER3_PROFILE_SIEGE_TANK")
		&"long_range_vls": return tr("TIER3_PROFILE_LONG_RANGE_VLS")
		&"deadair": return tr("TIER3_PROFILE_DEADAIR")
		_: return tr("TIER3_PROFILE_UNKNOWN")


func _on_tier3_threat_changed(zone_id: StringName, profile: StringName, active: bool) -> void:
	if _zone_hint == null:
		return
	var zone_label := _zone_label(zone_id)
	var profile_name := _tier3_profile_name(profile)
	if active:
		var text := tr("TIER3_THREAT_ACTIVE_FMT") % [zone_label, profile_name]
		_active_tier3_threat_text_by_zone[zone_id] = text
		_zone_hint.show_warning_banner(text)
		_zone_hint.show_persistent(text)
		return
	var old_text := String(_active_tier3_threat_text_by_zone.get(zone_id, ""))
	_active_tier3_threat_text_by_zone.erase(zone_id)
	if old_text != "":
		_zone_hint.hide_persistent(old_text)
	if not _active_tier3_threat_text_by_zone.is_empty():
		var remaining_text := String(_active_tier3_threat_text_by_zone.values()[0])
		_zone_hint.show_persistent(remaining_text)
	elif not _is_in_boss_phase():
...
func _on_zone_mission_completed(zone_id: StringName) -> void:
	...
	var reward_log := "bomber_xp_%d" % bomber_xp_reward if bomber_xp_reward > 0 \
		else String(reward.get("id", "-"))
	EventLogger.log_event("ZONE", "Cleared", "id=%s reward=%s" % [zone_id, reward_log])

	# 攻克 toast：奖励攻克即领（不再"待停靠"文案）
	var label := _zone_label(zone_id)
	if _zone_hint:
		var msg: String
		if bomber_xp_reward > 0:
			msg = tr("ZONE_CLEARED_BOMBER_XP_FMT") % [label, bomber_xp_reward]
		elif reward_name != "":
...
## 轰炸机护送成功的专属结算：直接进入共享经验池，不套击杀 XP 乘区或小队分摊。
## 难度必须在 mark_cleared 清缓存前读取；失败路径不调用本函数，因此严格为 0 XP。
func _grant_bomber_escort_xp(zone_id: StringName) -> int:
	if not _zone_data or not is_instance_valid(survivor_player):
		return 0
	var difficulty := _zone_data.get_difficulty(zone_id)
	var amount := ZoneData.bomber_escort_xp_reward(difficulty)
	survivor_player.add_xp(amount)
	if not _bench_mode and hud:
		hud.spawn_xp_gain(amount)
	EventLogger.log_event("XP", "BomberEscort",
		"zone=%s stars=%d awarded=%d" % [zone_id, difficulty, amount])
	return amount
...
## 打开停靠规划站：保留当前机并装备专属技能，或进化到新机体，严格二选一。
## 进化出口可能全锁（等级门槛）→ 仍显示灰卡；当前机专属技始终展示其许可/装备状态。
func _open_evolution_offer() -> void:
	# 停靠规划站会暂停战场并接管指针；必须先退出急刹捕获，即使随后因玩家引用
	# 异常提前返回，也不能把系统鼠标留在 MOUSE_MODE_CAPTURED。
	_set_hard_brake(false)
	...
	var signature: Dictionary = SurvivorData.signature_upgrade_for_aircraft(cur_id)
	if signature.is_empty():
		push_warning("机场规划站缺少机体专属技能映射：%s" % cur_id)
	var signature_license_owned: bool = not archive_enabled() \
		or MetaShop.is_signature_owned_for_aircraft(cur_id)
	var signature_id := String(signature.get("id", ""))
	var signature_equipped: bool = signature_id != "" \
		and int(upgrade_stacks.get(signature_id, 0)) >= int(signature.get("max_stacks", 1))
	if _evolution_ui == null:
		...
		_evolution_ui.signature_chosen.connect(_on_settlement_signature)
	...
	if hud:
		...
		hud.set_warzone_remaining(_remaining, _is_in_boss_phase(), game_time)
	var lvl: int = survivor_player.level if survivor_player else 1
	...
	if history.is_empty() and cur_id != &"":
		history = [cur_id]
	_evolution_ui.show_offer(EvolutionSystem.node_of(cur_id), exits, lvl, signature,
		signature_license_owned, signature_equipped, history,
		survivor_player.axis_points if survivor_player else {},
		survivor_player.milestone_bonus if survivor_player else {})

## 规划站·进化栏：ACE 手动进化 + 僚机自动跟随同款（spec ace-system §2.3）。成功后结束本次停靠。
func _on_settlement_evolution(node_id: StringName) -> void:
	var nd: Dictionary = EvolutionSystem.node_of(node_id)
	if nd.is_empty():
		_evolution_ui.reject_decision()
		return
	var current_id := _current_evolution_node_id()
	if not EvolutionSystem.node_of(current_id).get("exits", []).has(String(node_id)):
		_evolution_ui.reject_decision()
		return
	...
	EventLogger.log_event("DOCK", "EvolutionDecision", "%s→%s" % [current_id, node_id])
	_on_settlement_closed()

## 规划站·保留当前机：装备本机专属技能，不发普通 +1 轴点；技能自身 milestone_plus 仍照常兑现。
func _on_settlement_signature(upgrade: Dictionary) -> void:
	var current_id := _current_evolution_node_id()
	var expected := SurvivorData.signature_upgrade_for_aircraft(current_id)
	var uid := String(upgrade.get("id", ""))
	if expected.is_empty() or uid != String(expected.get("id", "")):
		_evolution_ui.reject_decision()
		return
	if archive_enabled() and not MetaShop.is_signature_owned_for_aircraft(current_id):
		_evolution_ui.reject_decision()
		return
	if int(upgrade_stacks.get(uid, 0)) >= int(upgrade.get("max_stacks", 1)):
		_evolution_ui.reject_decision()
		return
	_apply_upgrade_choice(upgrade)
	EventLogger.log_event("DOCK", "SignatureDecision", "%s/%s" % [current_id, uid])
	if player_aircraft and is_instance_valid(player_aircraft):
		player_aircraft.show_tactic_popup(tr("SETTLEMENT_SIGNATURE_EQUIPPED_POPUP_FMT") % tr(
...
func _spawn_airfield_ally_unit(kind: StringName, world_pos: Vector2,
	...
	if scene == null:
		return null
	var safe_pos := MapGeography.find_ground_spawn_near(world_pos, 500.0)
	if safe_pos == Vector2.INF:
		EventLogger.log_event("SUPPORT", "AirfieldGroundSpawnSkipped",
			"kind=%s anchor=%s no_safe_land" % [kind, world_pos])
		return null
...
func _prepare_ace_run_order() -> void:
	var rng := RandomNumberGenerator.new()
	rng.randomize()
	if _battlefield_flow.prepare_ace_run_order(rng):
		EventLogger.log_event("BALANCE", "AceRotation",
			"run_order=%s" % str(_battlefield_flow.ace_run_order()))
...
func _update_ace_support_event(_delta: float) -> void:
	...
	var rng := RandomNumberGenerator.new()
	rng.randomize()
	var pick := _battlefield_flow.claim_next_ace_profile(rng,
		_event_director.find_by_name("ace_support") != null)
	if pick.is_empty():
		return
...
func _update_orion_event() -> void:
	...
	if _boss_debug_mode or _bench_mode:
		return
	if not _battlefield_flow.claim_orion_if_due(OrionNemesisEvent.TRIGGER_S):
		return
...
## T0 起手机机场等价礼包。技能与武器都复用正式账本 / 奖励入口，避免另造平行状态。
func _grant_starting_benefit(profile: PlayableAircraft) -> void:
	if profile == null or profile.starting_benefit_id == &"":
		return
	var benefit := String(profile.starting_benefit_id)
	if benefit == "gun_multishot":
		var upgrade := SurvivorData.upgrade_by_id(benefit)
		if upgrade.is_empty():
			push_error("survivor_mode: 未找到 T0 起手技能 %s" % benefit)
			return
		_apply_upgrade_choice(upgrade)  # 机场技能语义：记栈并生效，但不发普通升级卡的轴点。
	else:
		_claim_weapon_reward(benefit)
	EventLogger.log_event("START_BENEFIT", String(profile.id), benefit)
...
func _claim_weapon_reward(weapon: String) -> void:
	...
	match weapon:
		...
		"rocket", "rocket_ffar":
			...
			# F-104C 的 T0 起手礼包显式使用 FFAR；普通战区 rocket 继续使用 Hydra。
			# 已持有（重复奖励，roll 侧已过滤——这里是 roll 后换机窗口的兜底）→ 降级补 QMAAM
			if p.rocket != null:
				_claim_weapon_reward("qmaam")
				return
			var rocket_path := "res://resources/rocket_ffar.tres" if weapon == "rocket_ffar" \
					else "res://resources/a10_rocket.tres"
			p.rocket = load(rocket_path).duplicate(true)
		...
		_:
			...
			if p.secondary_missile != null:
				...
				player_aircraft.secondary_missiles_remaining += q.max_count
			else:
				p.secondary_missile = q
				player_aircraft.secondary_missiles_remaining = q.max_count
	# 奖励在发放当刻进入局内库存；进化前快照仍会覆盖为强化后的同一资源引用。
	if survivor_player:
		survivor_player.record_special_weapons()
...
func begin_ace_battle_music() -> bool:
	...
	if not AudioManager.crossfade_music_interrupt(
			_active_ace_music_id, BATTLE_MUSIC_CROSSFADE_S, false):
		_active_ace_music_id = ""
		return false
	return true
...
func end_ace_battle_music() -> void:
	...
	if owned_id.is_empty():
		return
	if is_game_over or _is_in_boss_phase():
		AudioManager.discard_interrupted_music()
		return
	if AudioManager.current_music_id() != owned_id:
		AudioManager.discard_interrupted_music()
		return
	if not AudioManager.resume_interrupted_music(BATTLE_MUSIC_CROSSFADE_S):
...
func _is_in_boss_phase() -> bool:
	return _battlefield_flow.is_boss_phase(_zone_data, _boss_spawned)
...
func archive_enabled() -> bool:
	return not _bench_mode and not _boss_debug_mode and not _map_preview_only
...
## 与出界补给时间税（当前 +30s）同一注入语义、方向相反：game_time 向
...
func grant_time_extension(seconds: float) -> void:
	var granted := _battlefield_flow.grant_time_extension(seconds, _zone_data, _boss_spawned)
	if granted > 0.0:
		EventLogger.log_event("EVENT", "TimeExtension",
			"+%.0fs (game_time=%.1f)" % [granted, game_time])
...
func _check_warzone_phase_timeout() -> void:
	if not _battlefield_flow.close_warzone_if_due(_zone_data, _zone_mission):
		return
...
## F6 Debug：只把正式计时推进到 50%，不复制或触发战区/BOSS 解锁逻辑。
## 若已超过半局则不倒拨；现实速度沙带另由 WeatherSystem Debug override 定位到地图中心。
func debug_skip_to_midgame() -> void:
	if _is_in_boss_phase():
		return
	var target_time := WARZONE_PHASE_DURATION * 0.5
	game_time = maxf(game_time, target_time)
	if _weather:
		_weather.set_debug_midgame(game_time, WARZONE_PHASE_DURATION)
	EventLogger.log_event("ZONE", "DebugSkipMidgame", "game_time=%.1f" % game_time)
