...
#  规则（2026-07-12 用户订正）：**蓝 = 玩家直属**（玩家小队的丝带/尾迹/标签等一律蓝系，
#  沿用既有蓝）；**绿 = 中立/非直属友军**（第三方 ALLY、中立设施）；敌方一律暖色，
#  内部用"橙 → 红"表达威胁层级（常规 → 精英/BOSS）。
#  任何 HUD / kill feed / 雷达锥 / 尾迹 / 小地图 / 新增单位配色一律引用本表，
#  禁止再散写颜色字面量。team 语义：0=PLAYER 1=HOSTILE 2=ALLY（CombatUnit.TEAM_*）。
# ══════════════════════════════════════════════════════════════
const COL_FRIEND_PLAYER := Color(0.3, 0.6, 1.0)         ## #4D99FF 玩家直属 蓝（既有队伍蓝）
const COL_FRIEND_ALLY   := Color(0.35, 0.82, 0.42)      ## #59D16B 中立/第三方 绿
...
# ── 队伍尾迹色（飞机+导弹通用；玩家=既有蓝 / ALLY=绿）──
const TEAM_TRAIL_PLAYER := Color(0.3, 0.5, 1.0, 0.6)
const TEAM_TRAIL_ALLY   := Color(0.32, 0.78, 0.40, 0.6)
const TEAM_TRAIL_RED    := Color(1.0, 0.25, 0.25, 0.6)

# ── 队伍雷达锥色（地面单位扇形/圆形填充；玩家=既有蓝青 / ALLY=绿）──
const TEAM_RADAR_PLAYER := Color(0.2, 0.7, 0.8)   ## alpha 由调用处设置
const TEAM_RADAR_ALLY   := Color(0.28, 0.72, 0.35)
const TEAM_RADAR_RED    := Color(0.8, 0.2, 0.2)

# ── 队伍数据链色（雷达站虚线圆）──
const TEAM_DATALINK_PLAYER := Color(0.3, 0.6, 1.0, 0.15)
const TEAM_DATALINK_ALLY   := Color(0.35, 0.8, 0.4, 0.15)
const TEAM_DATALINK_RED    := Color(1.0, 0.3, 0.3, 0.15)

# ── 导弹信息标签背景/文字色（玩家=既有蓝 / ALLY=绿）──
const MISSILE_LABEL_BG_PLAYER   := Color(0.1, 0.2, 0.3, 0.75)
const MISSILE_LABEL_TEXT_PLAYER := Color(0.7, 0.85, 1.0)
const MISSILE_LABEL_BG_ALLY     := Color(0.06, 0.2, 0.1, 0.75)
const MISSILE_LABEL_TEXT_ALLY   := Color(0.75, 0.95, 0.78)
...
# ── 高射炮射程圈色（玩家=既有蓝青 / ALLY=绿 / 敌=橙，区别于 SAM 红）──
const AA_RANGE_PLAYER := Color(0.2, 0.7, 0.8)
const AA_RANGE_ALLY   := Color(0.28, 0.72, 0.35)
const AA_RANGE_ORANGE := Color(0.8, 0.5, 0.1)

# ── 飞机信息标签色（玩家=既有蓝 / ALLY=绿）──
const AIRCRAFT_LABEL_BG_PLAYER   := Color(0.1, 0.15, 0.35, 0.85)
const AIRCRAFT_LABEL_TEXT_PLAYER := Color(0.8, 0.9, 1.0)
const AIRCRAFT_LABEL_BG_ALLY     := Color(0.05, 0.20, 0.09, 0.85)
const AIRCRAFT_LABEL_TEXT_ALLY   := Color(0.78, 0.95, 0.80)
...
static func missile_label_colors(team: int) -> Array:
	...
	if team == 0:
		return [MISSILE_LABEL_BG_PLAYER, MISSILE_LABEL_TEXT_PLAYER]
	if team == 2:
		return [MISSILE_LABEL_BG_ALLY, MISSILE_LABEL_TEXT_ALLY]
