...
# （2026-07-05 扩图 30km→60km，spec map-expansion；bbox 为旧值 ×2。
#  ⚠ PNG 底图按 map-editor v4 决议属过渡资产，编辑器矢量渲染接管官方图后移除）
# bbox 必须对称围绕游戏中心，否则游戏内边界会和底图错位（见 2026-04-22 修复）
# 半宽略大于 ±30 km 留余量；瓦片对齐后实际范围会再扩一点
TARGET_BBOX = {
    "lat_min": 35.44 - 0.2924,   # 35.1476  (~32.5 km south)
    "lat_max": 35.44 + 0.2924,   # 35.7324  (~32.5 km north)
    "lon_min": 139.76 - 0.3460,  # 139.4140 (~31.3 km west)
    "lon_max": 139.76 + 0.3460,  # 140.1060 (~31.3 km east)
}
