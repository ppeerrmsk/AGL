...
##   ① 陆判等价铁律：转换图偶奇陆判 == MapGeography.is_on_land，全部 150×150 格心逐点一致
##   ② editor_cells 反推与陆判一致（涂格 == is_on_land）
##   ③ 直通保真：道路总数 == 官方 5 级之和且首条逐顶点相等；建筑数 == districts 数；
##      战区数/字段 == ZoneData.ZONES；海岸线/机场条数一致
##   ④ 存读闭环：save → load 无警告不拒载，陆地层多边形逐顶点相等
...
func _even_odd(p: Vector2, polys: Array) -> bool:
	var hits := 0
	for poly in polys:
...
func _initialize() -> void:
	...
	for cy in range(MapDocument.GRID_H):
		for cx in range(MapDocument.GRID_W):
			...
			if _even_odd(p, land) != MapGeography.is_on_land(p):
				mismatch += 1
